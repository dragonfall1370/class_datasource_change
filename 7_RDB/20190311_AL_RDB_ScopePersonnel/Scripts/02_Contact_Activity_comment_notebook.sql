...
--3	CCT	Contact
--4	PER	Person
declare @objType int = 3

drop table if exists VC_ConActivityCommentNotebook
...
, a.CreatedOn
, b.CreatedBy
, iif(@objType = 2, f.ObjectId, null) as ComObjId
, iif(@objType = 3, i.FakeConId, null) as ConObjId
, iif(@objType = 1, f.ObjectId, null) as CanObjId
, f.JobId
, f.ClientID as ComId
, e.FileExtension
, e.Memo
...
into VC_ConActivityCommentNotebook
...
left join (select ClientContactId as FakeConId, ContactPersonId as ConId from [RF_Contacts_Complete]) i on h.ObjectId = ConId
...
select * from VC_ConActivityCommentNotebook
