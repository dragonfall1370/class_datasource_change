...
, Person_Attachments AS (
	SELECT
	f.contactId
	, STRING_AGG(NULLIF(f.[fileName],''), ',') AS docs
	FROM (
		SELECT
		pa.contactId, pa.[fileName]
		FROM
		CandidatesIndex p
		LEFT JOIN [dbo].[person.attachment] pa ON p.contactId = pa.contactId
	) f
...
, TRIM(',' FROM COALESCE(pat.docs, '')) AS [candidate-resume]
...
FROM
CandidatesIndex ci
LEFT JOIN [dbo].[person] p ON ci.contactId = p.contactId
--LEFT JOIN Person_Tags pt ON p.contactId = pt.contactId
LEFt JOIN Person_Emails_Flatened pe ON p.contactId = pe.contactId
LEFT JOIN Person_Addresses pa ON p.contactId = pa.contactId
LEFT JOIN Person_Phones_Flatened pp ON p.contactId = pp.contactId
