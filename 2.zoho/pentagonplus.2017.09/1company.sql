...
with dup as (SELECT ClientId,ClientName,ROW_NUMBER() OVER(PARTITION BY CC.ClientName ORDER BY CC.ClientId ASC) AS rn FROM clients CC ) --where name like 'Azurance'
--select * from dup
...
select --top 1000
	...
	Stuff( 
	                  Coalesce('About: ' + NULLIF(cast(c.About as varchar(max)), '') + char(10), '')
                        ...
                        + Coalesce('Remarks: ' + NULLIF(c.Remarks_egsellingpointofclient, '') + char(10), '')
                , 1, 0, '') as 'company-note',
        doc.docs as 'company-document'
from clients c
left join dup on dup.ClientId = c.ClientId
left join users u on u.userid = c.AccountManagerId
left join (SELECT b.parentid, STUFF((SELECT DISTINCT ',' + replace(replace(a.FileName,',',''),'''','') from attachments a left join clients c on a.parentid = c.ClientId WHERE a.parentid = b.parentid FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '')  AS docs FROM attachments AS b GROUP BY b.parentid) doc on doc.parentid = c.clientID
...
--select count(*) --565
from notes t
left join clients c on c.ClientId = t.parentid
...
--select count(*) --41
from calls t
left join clients c on c.ClientId = t.RelatedTo
left join (select userid, email, firstname, lastname from users) u1 on u1.userid = t.CreatedBy
left join (select userid, email, firstname, lastname from users) u2 on u2.userid = t.ModifiedBy
where c.ClientId is not null
--and t.ParentId = 'Zrecruit_139304000000055051'
