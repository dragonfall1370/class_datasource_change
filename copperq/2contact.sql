...
select 
	 #cp.ID as 'contact-externalId'
	  concat(cp.ID,'copperq') as 'contact-externalId'
	...
	,concat(cl.cliID,'copperq') as 'contact-companyId'
	...
	, ltrim(rtrim(replace(replace(cp.Email,char(9),""),'\r\n',""))) as 'contact-email'
	...
	#, concat(cp.mobile,case when (cp.Telephone = '' OR cp.Telephone is NULL) THEN '' ELSE concat(', ',cp.Telephone) END) as 'contact-phone1'
	#, left(concat(ltrim(cp.mobile),",",ltrim(cp.Telephone),",",ltrim(cp.Telephone)),1) as 'contact-phone2'1
	, if(
	       left(concat(ltrim(cp.mobile),",",ltrim(cp.Telephone),",",ltrim(cp.Telephone)),1)  in (",",":","="),
	       substring(concat(ltrim(cp.mobile),",",ltrim(cp.Telephone),",",ltrim(cp.Telephone)),2),
	       concat(ltrim(cp.mobile),",",ltrim(cp.Telephone),",",ltrim(cp.Telephone))
	       ) as 'contact-phone'
...
from copperq.clipeople cp
left join copperq.client cl on cp.CliID = cl.CliID
left join copperq.clipeoclass cpl on cp.Class1 = cpl.ID
left join copperq.users u ON cl.Owner = u.userid
...
select * from copperq.clipeople cp
*/