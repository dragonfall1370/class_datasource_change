select
          cl.cliID as 'company-externalId(origin)'
        ,concat(cl.cliID,'copperq') as 'company-externalId'
        , IF(ISNULL(cl.cliname),"No CompanyName",ltrim(rtrim(replace(replace(cl.cliname,char(9),""),'\r\n',"")))) as 'company-name'
...
#       , CC.companyDescription
        ...
        , cli.filename as 'company-document'
#select cl.cliID, cl.cliname, cli.filename # select count(*)
from copperq.client cl
...
# select cl.cliID, cli.filename,length(cli.filename) as len_filename,'COMPANY' as entity_type, 'legal_document' as document_type  from copperq.client cl left join (SELECT cliID, CONCAT(ID,'.',FileName) as filename FROM clifiles where FileName not like '%.msg') cli on cli.CliID = cl.cliID
