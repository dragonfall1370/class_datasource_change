...
job (position_externalId,company_externalId,position_title,position_startDate,rn) as (
	SELECT    a.position_externalId as position_externalId
		, b.company_externalId as company_externalId
	...
	-- select count(*) --1973
	from JobImportAutomappingTemplate a
...
select --top 100
	...
	, a.position_type as 'position-type' /* This field only accepts PERMANENT, INTERIM_PROJECT_CONSULTING,TEMPORARY,CONTRACT, TEMPORARY_TO_PERMANENT */
        ...
        , a.Currency as 'position-currency'
...
-- select distinct employmentType --select distinct Type -- select distinct status -- select distinct customtext1 --select count(*) --1380 -- select top 10 * -- select distinct convert(varchar,position_owners)
from JobImportAutomappingTemplate a
