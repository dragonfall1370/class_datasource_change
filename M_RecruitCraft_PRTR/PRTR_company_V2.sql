...
/* THIS PART REMOVED IN VER2 TO BE INJECTED AS CUSTOM SCRIPT 
--COMPANY ADDRESS
...
, iif(dup.rn > 1, concat(dup.company_name, ' - ', dup.rn), coalesce(c.company_name,'No company name')) as 'company-name'
...
, case when c.company_tel in ('','x','xxx','-','NA','n/a','tbc') then NULL
	else c.company_tel end as 'company-switchBoard'
, concat_ws(','
	, coalesce(nullif(json_value(c.company_custom_field,'$.SubDetail.dxTextBox[0].Value'),''),NULL) --get value from json
	, coalesce(nullif(json_value(c.company_custom_field,'$.SubDetail.dxTextBox[1].Value'),''),NULL) --get value from json
	) as 'company-phone'
...
, concat_ws(char(10),concat('Company external ID: ',c.company_id)
	...
	-- , coalesce('Telephone 3: ' + nullif(json_value(c.company_custom_field,'$.SubDetail.dxTextBox[1].Value'),''),NULL) --get value from json
