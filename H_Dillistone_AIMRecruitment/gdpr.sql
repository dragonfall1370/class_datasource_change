WITH cte_candidate AS (
	...
	concat(
		...
		CASE WHEN NULLIF(TRIM(REPLACE(cl.email, '\x0d\x0a', '')), '') IS NULL THEN '' ELSE concat('Email: ', REPLACE(cl.email, '\x0d\x0a', ' '), E'\n') END,
		CASE WHEN NULLIF(TRIM(REPLACE(pct.value, '\x0d\x0a', '')), '') IS NULL THEN '' ELSE concat('Person communication type: ', REPLACE(pct.value, '\x0d\x0a', ' '), E'\n') END,
		CASE WHEN NULLIF(TRIM(REPLACE(cl.error_description, '\x0d\x0a', '')), '') IS NULL THEN '' ELSE concat('Error description: ', REPLACE(cl.error_description, '\x0d\x0a', ' '), E'\n') END,
		CASE WHEN NULLIF(TRIM(REPLACE(cl.error_code, '\x0d\x0a', '')), '') IS NULL THEN '' ELSE concat('Error code: ', REPLACE(cl.error_code, '\x0d\x0a', ' '), E'\n') END,
		...
		CASE WHEN NULLIF(TRIM(REPLACE(cl.first_name, '\x0d\x0a', '')), '') IS NULL THEN '' ELSE concat('First name: ', REPLACE(cl.error_code, '\x0d\x0a', ' '), E'\n') END,
		CASE WHEN NULLIF(TRIM(REPLACE(cl.last_name, '\x0d\x0a', '')), '') IS NULL THEN '' ELSE concat('Last name: ', REPLACE(cl.last_name, '\x0d\x0a', ' '), E'\n') END,
		CASE WHEN NULLIF(TRIM(REPLACE(cl.email_template, '\x0d\x0a', '')), '') IS NULL THEN '' ELSE concat('Email template: ', REPLACE(cl.email_template, '\x0d\x0a', ' '), E'\n') END,
		CASE WHEN NULLIF(TRIM(REPLACE(cl.result, '\x0d\x0a', '')), '') IS NULL THEN '' ELSE concat('Result: ', REPLACE(cl.result, '\x0d\x0a', ' '), E'\n') END,
	...
	LEFT JOIN person_communication_type pct ON cl.id_person_communication_type = pct.id_person_communication_type
)
SELECT
candidate_id,
gdpr_note
