...
---COMPANY CUSTOM FIELDS (Global Solutions)
...
, dup as (SELECT CompanyID, Name, ROW_NUMBER() OVER(PARTITION BY [Name] ORDER BY CompanyID ASC) AS rn 
FROM Company)

---MAIN SCRIPT
select * from (
select concat('GS',C.CompanyID) as 'company-externalId'
, iif(C.CompanyID in (select CompanyID from dup where dup.rn > 1)
	, iif(dup.name = '' or dup.name is NULL,concat('Default Company-',dup.CompanyID),concat(dup.name,'-DUPLICATE-',dup.CompanyID))
	, iif(C.Name = '' or C.Name is null,concat('Default Company-',C.CompanyID),C.Name)) as 'company-name'
, C.Name as '(CompanyNameOrigin)'
...
, concat(iif(CA.Name = '' or CA.Name is NULL,'',concat(CA.Name,': ')),iif(CA.Line1 = '' or CA.Line1 is NULL,'',concat(CA.Line1,', ')),iif(CA.Suburb = '' or CA.Suburb is NULL,'',concat(CA.Suburb,', '))
	,iif(CA.State = '' or CA.State is NULL,'',concat(CA.State,', ')),iif(CA.Postcode = '' or CA.Postcode is NULL,'',CA.Postcode),iif(CA.Country = '' or CA.Country is NULL,'',concat(', ',CA.Country))) as 'company-locationAddress'
...
, C.OwnerUserID, C.UpdatedBy
, iif(U.Email like '%deleted%','',U.Email) as 'company-owners'
, left(concat('Company External ID: ',C.CompanyID,char(10)
	, iif(C.DateCreated = '' or C.DateCreated is NULL,'',concat('Date created: ',convert(varchar(10),C.DateCreated,120),char(10)))
...
--left join CompanySkill CSk on CSk.CompanyID = C.CompanyID
...
--where C.CompanyID in (129815,129409,131570,121630,121631,129661) | --where C.CompanyID = 116825 --> Those companies with empty company name

UNION ALL

select 'GS9999999','Default Company','','','','','','','','','','','','','','','This is default company from data import',''
) as FullCompany
