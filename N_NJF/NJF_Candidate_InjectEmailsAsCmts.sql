select concat('NJFS',ac.intCandidateId) as CanExternalId, -10 as userId
		...
		, concat('-----MIGRATED FROM EMAILS-----',
				...
				iif(ac.dtInserted = '' or ac.dtInserted is null, '', concat(char(10),'Inserted Date: ',convert(varchar(20),ac.dtInserted,120))),
				concat(char(10),'Email ID: ', ac.intAttachmentId)
				) as commentContent
