...
BEGIN
  ...
  SELECT @ImageData = (SELECT convert(varbinary(max), binDocbytes, 1) FROM dCandidateCVBinaryDocuments WHERE intCandidateCVId = @DocumentID);

  DECLARE @Path nvarchar(1024);
  SELECT @Path = 'E:\OneDrive - HRBoss\DataImport\NJF\CVFiles'; --> Only affects on localserver
 ...
 iif(right(vchCVName,4)=vchFileType,concat('_',replace(vchCVName,'/','.')),iif(vchFileType= '.', concat('_',vchCVName,'.docx'),concat('_',replace(vchCVName,'/','.'),vchFileType)))) FROM dCandidateCV WHERE intCandidateCVId = @DocumentID);
