...
, tempWorkHistory1 as (select intPersonId, 
		concat(iif(intWorkHistoryId is NULL,'',concat('-----Employer ',rn,': ',vchCompanyName,char(10)))
			...
			, case
				 when tintJobType = 1 then concat('Job Type: Fixed Contract (1)',char(10))
				 when tintJobType = 2 then concat('Job Type: Permanent (2)',char(10))
...
select concat('NJF',intCompanyTierContactId) as ContactExternalId, -10 as userId
		, CURRENT_TIMESTAMP as InsertTimeStamp, -10 as AssignedUserId, 'comment' as category, 'contact' as type
		, WorkHistory as commentContent
		from temp
