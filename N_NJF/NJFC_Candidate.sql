...
, case	when rn = 1 then email
		else concat('NJFC_',rn,'_',email) end as CandidateEmail
, rn
...
select concat('NJFC', c.intCandidateId) as 'candidate-externalId', p.intPersonId  as PersonId--just for reference afterward
...
, iif(cme.CandidateEmail = '' or cme.CandidateEmail is NULL,concat('NJFC_ID',c.intCandidateId,'@noemail.com'),cme.CandidateEmail) as 'candidate-email'
...
, left(
	concat('Candidate External ID (NJF Contract): NJFC',c.intCandidateId
...
from dCandidate c
				...
				left join CanDocuments cd on c.intCandidateId = cd.intCandidateId
