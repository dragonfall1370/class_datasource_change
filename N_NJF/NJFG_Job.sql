...
select 
case 
	when (ctc.intCompanyTierContactId = '' or ctc.intCompanyTierContactId is NULL) and j.intCompanyId in (select CompanyId from ContactMaxID) then concat('NJFGTP',cm.ContactMaxID)
	when (ctc.intCompanyTierContactId = '' or ctc.intCompanyTierContactId is NULL) and j.intCompanyId not in (select CompanyID from ContactMaxID) then 'NJFGTP9999999'
	when ctc.intCompanyTierContactId is NULL and j.intCompanyId is NULL then 'NJFGTP9999999'
	else concat('NJFGTP',ctc.intCompanyTierContactId) end as 'position-contactId'
, j.intCompanyId as 'CompanyID'
, j.intMainContactId as 'MainContactId'
, ctc.intCompanyTierContactId as 'ContactID'
, concat('NJFGTP',j.intJobId) as 'position-externalId'
, j.vchClientJobTitle as 'position-title(old)'
, iif(j.intJobId in (select intJobId from dup where dup.rn > 1)
	, iif(dup.vchClientJobTitle = '' or dup.vchClientJobTitle is NULL,concat('NJF GTP - No job title - ID',dup.intJobId),concat(dup.vchClientJobTitle,' - ', dup.intJobId))
	, iif(j.vchClientJobTitle = '' or j.vchClientJobTitle is null,concat ('NJF GTP - No job title - ID',j.intJobId),j.vchClientJobTitle)) as 'position-title'
...
, left(
	concat('Job External ID: NJFGTP',j.intJobId,char(10)
