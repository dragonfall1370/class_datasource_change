...
---Main script
select case 
when J.ContactID is NULL and J.CompanyID is not NULL then iif(CM.ContactMaxID = '' or CM.ContactMaxID is NULL,'GS9999999',concat('GS',CM.ContactMaxID)) --Company has maxContactID
when J.ContactID is not NULL then concat('GS',J.ContactID)
else 'GS9999999' end as 'position-contactId'
, J.ContactID
, J.CompanyID
, concat('GS',J.JobOrderID) as 'position-externalId'
, iif(J.JobTitle = '' or J.JobTitle is NULL,concat('No job title - ',J.JobOrderID),J.JobTitle) as 'position-title'
...
, iif(U.Email like '%deleted%','',U.Email) as 'position-owners'
...
, left(replace(replace(replace(replace(JNF.URLList,'&lt;','<'),'&gt;','>'),'&amp;','&'),'#x0D;',''),32000) as 'position-comment'
