...
when FileType = 'application/vnd.openxmlformats-officedocument.word' then '.docx'
...
---Candidate skills to be combined from Category and Subcategory
, CandCg_SCg as (select CS.ContactID, CS.CategoryID, Cg.Name as CategoryName, CS.SubCategoryID, SCg.Name as SubCategoryName
...
, CandCombinedSubCat as (SELECT
     ...
     STUFF(
         (SELECT ', ' + SubCategoryName
          ...
          FOR XML PATH (''))
          , 1, 1, '')  AS URLList
FROM CandCg_SCg as a
GROUP BY a.ContactID, a.CategoryID)

, CandCategory as (select CCg.ContactID, CCg.CategoryID, Cg.Name as CategoryName, CCg.URLList as SubCategories
from CandCombinedSubCat CCg
left join Category Cg on CCg.CategoryID = Cg.CategoryID)

, CandCombinedCategory as (SELECT
     ContactID,
     STUFF(
         (SELECT char(10) + CategoryName + ': ' + SubCategories
          ...
          , 1, 1, '')  AS URLList
FROM CandCategory as a
GROUP BY a.ContactID)

---Candidate custom fields (Global Solutions)
...
---Main script
select concat('GS',C.ContactID) as 'candidate-externalId'
...
, C.WorkTypeID
, CCC.URLList as 'candidate-skills'
...
, concat(coalesce(CAF.FileName2 + ',',''),coalesce(CAF.FileName2,'')) as 'candidate-resume'
, iif(CO.Email like '%deleted%','',CO.Email) as 'candidate-owners'
, left(replace(replace(replace(replace(concat('Candidate External ID: ',C.ContactID,char(10)
	...
	--,iif(CCC.URLList = '' or CCC.URLList is NULL,'',concat('Candidate skills: ',CCC.URLList,char(10)))
	,iif(CCFV.CustFieldValue = '' or CCFV.CustFieldValue is NULL,'',concat('Candidate custom field: ',char(10),CCFV.CustFieldValue,char(10)))
	,iif(CSU.ProcessedText = '' or CSU.ProcessedText is NULL,'',concat('Candidate Resume: ',CSU.ProcessedText,char(10)))
	),'&lt;','<'),'&gt;','>'),'&amp;','&'),'&#x0D;',''),32000) as 'candidate-note'
, left(replace(replace(replace(replace(CNF.CandNote,'&lt;','<'),'&gt;','>'),'&amp;','&'),'&#x0D;',''),32000) as 'candidate-comments'
from Candidate C
...
left join CandCombinedCategory CCC on CCC.ContactID = C.ContactID
left join CombinedCustFieldValue CCFV on CCFV.ContactID = C.ContactID
where C.ContactID between 955467 and 1035467

where C.ContactID between 1035468 and 1055467
where C.ContactID between 1055468 and 1085467
where C.ContactID between 1085468 and 5194507