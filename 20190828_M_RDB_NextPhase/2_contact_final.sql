...
, mobile as (select ObjectID
		...
		from mobilephone
		group by ObjectID
		) ---select * from mobile where ObjectID = 6551
, homephone as (
		select p.ObjectID,p.PhoneId,p.CreatedOn
				, case when patindex('%[a-zA-Z]%',Num) > 0 then left(trim(num), patindex('%[a-zA-Z]%',Num) - 1) 
				else trim(Num) end as phone
        ...
        from dbo.Phones p
        where p.CommunicationTypeId in (79) --Phone
		)
, contact_homephone as (select ObjectID
		, string_agg(nullif(phone,''),',') as contact_homephone
        from homephone
        --where rn=1
		group by ObjectID
		)
, otherphone as (--ALL OTHER PHONES as HOME PHONE
...
, otherphone_group as (select ObjectID, CommunicationTypeId
				, string_agg(nullif(phone, ''),', ') as otherphone_group
				from otherphone
...
, contact_doc as (select cc.ClientContactId --Stored Documents
		--, cc.ContactPersonId
		--, t.TemplateName
		--, td.FileExtension
		--, td.Document
		--, t.CreatedOn
		, string_agg(cast(concat_ws('_','NP_S', t.TemplateId, concat(cc.ClientContactId, td.FileExtension)) as varchar(max)),',') 
			within group (order by t.TemplateId desc) as contact_doc
		from ClientContacts cc
		...
		and t.TemplateTypeId = 53 --221 rows | TemplateTypeId = 53 --stored documents
...
select concat('NP', cc.ClientContactId) as [contact-externalId]
        ...
        , coalesce(nullif(trim('\-?. ' FROM pi.Surname),''), concat('No Lastname - ',cc.ClientContactId)) as [contact-lastName]
        , cc.JobTitle as [contact-jobTitle]
        , pi.Title as [contact-title]
		, pre.PrimaryEmail as [contact-email]
		, m.mobile --#inject later
		, m.mobile as [contact-phone] --mobile as primary phone
		, opg.otherphone_group_all as homephone --#inject later
		...
		, concat_ws(char(10)
				...
				, coalesce('Employment Term: ' + nullif(et.Description,''),NULL)
				, coalesce('Employment Start Date: ' + nullif(convert(nvarchar(10),cc.EmploymentStartDate,120),''),NULL)
				, coalesce('Employment End Date: ' + nullif(convert(nvarchar(10),cc.EmploymentEndDate,120),''),NULL)
				, coalesce('Fax: ' + nullif(fc.fax_contact,''),NULL)
		...
		, cd.contact_doc as [contact-document]
from ClientContacts cc
...
left join mobile m on m.ObjectId = cc.ContactPersonId --mobile only | as primary phone
...
left join EmploymentTypes et on et.EmploymentTypeId = cc.EmploymentTypeId --employment terms
left join contact_doc cd on cd.ClientContactId = cc.ClientContactId --documents
where 1=1
