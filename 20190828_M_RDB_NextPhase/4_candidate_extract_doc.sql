DECLARE CURSOR_DocumentIds CURSOR FOR (select DocumentID from CVSendDocuments where CVId is NULL) --CV already parsed
...
BEGIN
  ...
  SELECT @Path = 'G:\NextPhase\Doc'; --> Only affects on localserver where database is create
---D:\EXEC\Documents
  ...
  SELECT @Filename = (select concat_ws('_','NP_D', d.DocumentID, concat(aa.ApplicantId, dc.FileExtension))
		...
		--and cvs.CVId is NULL --10321 --CV already parsed
