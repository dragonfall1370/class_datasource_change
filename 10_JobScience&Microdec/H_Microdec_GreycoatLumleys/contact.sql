-- DROP TABLE IF EXISTS _02_contact_sample;
-- CREATE TABLE _02_contact_sample AS
WITH contact_record_status AS (
	SELECT
		ROW_NUMBER() OVER(PARTITION BY p.person_ref ORDER BY CASE WHEN l.code = 'C' THEN 1 ELSE 2 END) rn,
		CASE
		...
		p.*
	FROM position p
-- 	LEFT JOIN person_type pt ON p.person_ref = pt.person_ref AND pt.type NOT IN ('A', 'C')
	LEFT JOIN lookup l ON p.record_status = l.code
	WHERE code_type = '132'
-- 	AND NULLIF(p.person_ref, '0') IS NOT NULL
),
current_contact AS (
	SELECT *,
...
COALESCE(NULLIF(TRIM(p.first_name), ''), '[No first name]') AS "contact-firstName",
COALESCE(NULLIF(TRIM(p.last_name), ''), '[No last name]') AS "contact-lastName",
...
CASE
	WHEN cc.rn_email > 1 THEN OVERLAY(cc.contact_email PLACING CONCAT('[DUP', CASE WHEN rn_email = 2 THEN '' ELSE (rn_email - 1)::text END, ']') from LENGTH(cc.contact_email) + 1 for 0)
...
cc.displayname AS "contact-jobTitle",
co.email_address AS "contact-owners",
CONCAT_WS(
	E'\n',
	CASE WHEN p.displayname IS NOT NULL THEN CONCAT('Contact display name: ', p.displayname) END,
	...
	CASE WHEN p.compliance_status = 'Y' THEN 'Compliance status: Yes' ELSE 'Compliance status: No' END,
	CASE WHEN coca.description IS NOT NULL THEN CONCAT('Compliance category: ', coca.description) END,
	CASE WHEN p.compliance_expiry IS NOT NULL THEN CONCAT('Compliance expiry: ', p.compliance_expiry) END,
...
LEFT JOIN compliance_category coca ON p.compliance_category = coca.code