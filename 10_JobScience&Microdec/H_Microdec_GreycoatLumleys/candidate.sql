...
-- CREATE TABLE _04_candidate_sample
-- AS
...
COALESCE(NULLIF(TRIM(cde.first_name), ''), '[No first name]') AS "candidate-firstName",
COALESCE(NULLIF(TRIM(cde.last_name), ''), '[No last name]') AS "candidate-lastName",
CASE
...
cc.country_code AS "candidate-Country",
CONCAT_WS(
	', ',
...
CASE
	WHEN cde.email_address IS NULL OR cde.email_address NOT LIKE '%@%' THEN concat(cde.first_name, '_', cde.last_name, '@no_email.com')
	WHEN rn_email > 1 THEN OVERLAY(cde.email_address PLACING CONCAT('[DUP', CASE WHEN rn_email = 2 THEN '' ELSE (rn_email-1)::text END, ']') from LENGTH(cde.email_address) + 1 for 0)
...
-- cde.income_required AS "candidate-desiredSalary",
-- CASE
...
-- 			 END-- DROP TABLE IF EXISTS _04_candidate_sample;
-- CREATE TABLE _04_candidate_sample
-- AS
WITH candidate_type AS (
	SELECT
...
-- 				ELSE NULLIF(RPAD(REPLACE(REGEXP_REPLACE(cde.income_required, '[a-zA-Z!#$%&*,-./:;<=>?@[\]^_`{|}~£()+]','','g'), ' ', ''), 5, '0'), '00000')
-- 			 END
-- END AS "candidate-desiredSalary",
CONCAT_WS(
	E'\n',
	CASE WHEN cde.displayname IS NOT NULL THEN CONCAT('Candidate display name: ', cde.displayname) END,
