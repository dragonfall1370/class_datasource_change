...
, note as (
	...
	, Stuff( Coalesce('Client Corporation ID: ' + NULLIF(cast(CC.clientCorporationID as varchar(max)), '') + char(10), '')
                        ...
                        + Coalesce('Company Description: ' + NULLIF([dbo].[fn_ConvertHTMLToText](CC.notes), '') + char(10), '')
                        + Coalesce('Company Overview: ' + NULLIF([dbo].[fn_ConvertHTMLToText](CC.companyDescription), '') + char(10), '')
                        + Coalesce('Standard Perm Fee (%): ' + NULLIF(cast(CC.feeArrangement as varchar(max)), '') + char(10), '')
                        + Coalesce('Trading Terms: ' + NULLIF(cast(CC.customtext3 as varchar(max)), '') + char(10), '')
                        + Coalesce('Operations / Sites / Project Offices: ' + NULLIF(cast(CC.customtextblock1 as varchar(max)), '') + char(10), '')
                        + Coalesce('Purchase Order Required: ' + NULLIF(cast(CC.customtext4 as varchar(max)), '') + char(10), '')
                        ...
                        --+ Coalesce('Custom Component 1: ' + NULLIF(CC.CustomComponent1, '') + char(10), '')
                        + Coalesce('Culture / Perks: ' + NULLIF(cast(CC.culture as varchar(max)), '') + char(10), '')
                        /*
...
select --top 100 
	...
	, CC.phone as 'company-switchboard' --as 'company-phone'
...
-- select count (*) --2127 -- select distinct CC.ownership -- select top 10 *
from bullhorn1.BH_ClientCorporation CC
