...
----------------------------
select 
	 Cl.userID as 'contact-externalId'
	--,UC.userID
	...
	--, case when (cast(tmp_businessSectorIDList3.name as varchar(max)) = '' OR tmp_businessSectorIDList3.name is NULL) THEN '' ELSE REPLACE(REPLACE(tmp_businessSectorIDList3.name, '&amp;', '&'), 'amp;', '') END as businessSectorIDListName
