...
, tmp_note(Userid, Notes) as (SELECT
     Userid,
     STUFF(
         --(SELECT '    ' + char(10) + 'Action: ' + action + ' || ' + convert(varchar(10), dateAdded, 120) + ': ' + cast(comments as varchar(max))-- + char(10)
		 (SELECT char(10)  + 'Date added: ' + convert(varchar(10), dateAdded, 120) + ' || ' + 'Action: ' + action + ' || ' + cast(comments as varchar(max))
          ...
          WHERE Userid = a.Userid order by dateAdded
...
, tmp_addednote as (SELECT
	 ...
	 concat('Date Added: ',convert(varchar(10),dateAdded,120),char(10)
		...
		--,concat('Skills: ',SN.SkillName,char(10)),concat('Business Sector: ',BS.BusinessSector)
		) as AddedNote
	 --,CA.desiredLocations
