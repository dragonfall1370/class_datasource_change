...
--select top 100 * from oe3 where email like '%waiyin@alphasearch%'
*/
...
, t as (SELECT [BH_Candidate].[candidateID] ,[BH_Candidate].[userID] ,[BH_SkillList].name 
...
, SkillName(userId, SkillName) as (SELECT userId, STUFF((SELECT DISTINCT ', ' + name from t WHERE t.name <> '' and userId = a.userId FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 2, '') AS name FROM t as a where a.name <> '' GROUP BY a.userId)
--select top 100 * from SkillName
...
, Education as (select EG.userID, UE.dateAdded, UE.userEducationID, UE.school, convert(varchar(10),UE.graduationDate,110) as graduationDate, UE.degree, UE.major, UE.comments from EducationGroup EG left join bullhorn1.BH_UserEducation UE on EG.userEducationID = UE.userEducationID)
-- Education Summary
...
+ Coalesce('Work Mobile: ' + NULLIF(convert(varchar(max),CA.phone2), '') + char(10), '')
...
+ Coalesce('Contract Rate: ' + NULLIF(cast(CA.hourlyRateLow as varchar(max)), '') + char(10), '')
...
+ Coalesce('LTD Company Country: ' + NULLIF(cast(tc.country as varchar(max)), '') + char(10), '') --CA.secondaryCountryID
...
+ Coalesce('CV: ' + NULLIF(UW.description_truong, '') , '')
  ...
  /*                       --+ Coalesce('Other email: ' + NULLIF(cast(oe3.email as varchar(max)), '') + char(10), '')
                       ...
                       + Coalesce('Pay Unit: ' + NULLIF(cast(CA.customText13 as varchar(max)), '') + char(10), '')
                        ...
                        , 1, 0, '') as note
	-- select top 10 * -- select referredBy, referredByUserID
	...
	left join tmp_country tc on tc.code = CA.secondaryCountryID
	...
	--left join BusinessSector BS on CA.userID = BS.userId
        ...
        left join (SELECT userid, STUFF((
                        ...
                        from bullhorn1.BH_UserWork where userid = a.userid FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '')  AS description_truong
                        FROM (   select userid, description_truong
                                        from bullhorn1.BH_UserWork) AS a GROUP BY a.userid 
                        ) uw on uw.userid = ca.userid            
	where CA.isPrimaryOwner = 1 ) --and CA.userid in (164043,164083) )
...
, EmploymentHistory(userId, eh) as (
       SELECT userId, STUFF(( 
                     ...
                     stuff(
                            ...
                            + Coalesce('Job Posting: ' + NULLIF(cast(jobPostingID as varchar(max)), '') + char(10), '')
...
--select * from EmploymentHistory where userid in (164043);
	...
	select --top 200
		...
		, e2.email as 'candidate-workemail' --PersonalEmail' --<<
	...
	left join EmploymentHistory EH on EH.userid = C.userid
