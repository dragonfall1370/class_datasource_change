...
, BusinessSector(jobPostingID, businessSector) as (
       SELECT jobPostingID
              , STUFF((
                     SELECT DISTINCT ', ' + BSL.name 
                     from bullhorn1.BH_JobPostingBusinessSector BS left join bullhorn1.BH_BusinessSectorList BSL ON BS.businessSectorID = BSL.businessSectorID 
                     WHERE BS.businessSectorID <> '' and jobPostingID = a.jobPostingID 
                     FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 2, '')  AS URLList 
       FROM bullhorn1.BH_JobPostingBusinessSector as a where a.businessSectorID <> '' GROUP BY a.jobPostingID )
...
/*
  ...
  */
	        , 1, 0, '') as note
        -- select count(*) -- select businessSectorID
        ...
        --where JP.customText10 is not null 
        )
...
select --top 100 
	...
	, case when job.title in (null,'') then 'No JobTitle' when job.rn > 1 then concat(job.title,' ',rn) else job.title end as 'position-title'
	...
	, case when a.employmentType is null then 'PERMANENT'
	       ...
	       when a.employmentType = 'Temporary - Regular' then 'TEMPORARY'
	       else ''
	...
	, a.skills as 'Skills / Keywords' -- <<
       , bs.businessSector as 'Industry'
-- select distinct employmentType -- select distinct Type -- select distinct status -- select distinct customtext1 -- select count(*) --2574 -- select top 100 * -- select publicDescription, description, startDate
from bullhorn1.BH_JobPosting a --where publicDescription <> description  --where a.jobPostingID = 2539
left join bullhorn1.BH_Client b on a.clientUserID = b.userID
...
left join BusinessSector bs on bs.jobPostingID = a.jobPostingID
--where bs.jobPostingID is not null
where b.isPrimaryOwner = 1 --> add isPrimaryOwner = 1 to remove 1 userID having more than 1 clientID
--and job.title <> ''
