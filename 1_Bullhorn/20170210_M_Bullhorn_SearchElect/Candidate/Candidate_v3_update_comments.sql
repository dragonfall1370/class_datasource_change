with t3(candidateUserID, name) as (select a.candidateUserID, a.name
...
, tmp_note2 (UserID, Notes2) as (SELECT
	C.userID, concat('Status: ',C.status,char(10),
	iif(C.referredBy = '' or C.referredBy is NULL,'',concat('Referred by: ',C.referredByUserID,' - ',referredBy,char(10)))
	,iif(C.phone2 = ''or C.phone2 is NULL,'',concat('Phone 2: ',C.phone2,char(10)))
	,iif(cast(C.desiredLocations as varchar(max)) = '' or C.desiredLocations is NULL,'',concat('Desired Locations: ',cast(tmp_country.COUNTRY as varchar(max)),char(10)))
	,tmp_note.Notes)
          from bullhorn1.Candidate C
		  left join tmp_note on C.userID = tmp_note.Userid
		  left join tmp_country on cast(C.desiredLocations as varchar(max)) = ABBREVIATION)
...
left join tmp_note2 on C.userID = tmp_note2.Userid
