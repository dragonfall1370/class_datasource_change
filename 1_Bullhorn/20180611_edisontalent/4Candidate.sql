...
--select top 10 * from placementfiles
	...
	select --top 5
              ...
              --, C.userID as '#userID'
		, case C.namePrefix when 'Mr.' then 'MR' when 'Mrs.' then 'MRS' else '' end as 'candidate-title'
		, case C.namePrefix when 'Mr.' then 'MALE' when 'Mrs.' then 'FEMALE' else '' end as 'candidate-gender'
		...
		, concat(files.ResumeId, ',' , p.placementfile) as 'candidate-resume'
	...
	-- select count (*) -- select distinct gender --employmentPreference -- select skillset, skillIDlist, customTextBlock1 --select top 10 * -- select distinct namePrefix
