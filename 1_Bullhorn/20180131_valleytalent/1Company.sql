...
with note as (
	...
	, Stuff( Coalesce('Client Corporation ID: ' + nullif(convert(varchar(max),CC.clientCorporationID), '') + char(10), '')
                        --+ coalesce('Stats: ' + nullif(convert(varchar(max),CC.clientCorporationRatios, '') + char(10), '')
                        /*+ Coalesce('Company Description: ' + nullif(ltrim(rtrim([dbo].[udf_StripHTML]( 
                                replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(
                                        convert(varchar(max),CC.companyDescription)
                                ,'&nbsp;','') ,'&ndash;','') ,'&amp;',''), '&hellip;','') ,'&#39;','') ,'&gt;','') ,'&lt;','') ,'&quot;','') ,'&rsquo;',''), '&ldquo;',''), '&rdquo;','') ,'&reg;','') ,'&euro;','')  ) )), '') + char(10), '') */
                        + coalesce('Company Description: ' + NULLIF([dbo].[fn_ConvertHTMLToText](CC.companyDescription), '') + char(10), '')
                        + coalesce('Date Last Visit: ' + nullif(convert(varchar(max),CC.customDate1), '') + char(10), '')
                        + coalesce('Fecha de Creación: ' + nullif(convert(varchar(max),CC.customDate2), '') + char(10), '')
                        + coalesce('CIF: ' + nullif(convert(varchar(max),CC.customText2), '') + char(10), '')
                        + coalesce('Consultant: ' + nullif(convert(varchar(max),CC.customText8), '') + char(10), '')
                        + Coalesce('Date Added: ' + nullif(convert(varchar(10),CC.dateAdded,120), '') + char(10), '')
                        + coalesce('Date Last Modified: ' + nullif(convert(varchar(max),lm.DateLastModified), '') + char(10), '')
                        + coalesce('Company Comments: ' + nullif(convert(varchar(max),CC.notes), '') + char(10), '')
                        --+ coalesce('Opportunities: ' + nullif(convert(varchar(max),CC.opportunityTable, '') + char(10), '')
                        --+ coalesce('Parent Company: ' + nullif(convert(varchar(max),CC.parentClientCorporationID), '') + char(10), '')
                        + Coalesce('Parent Company: ' + NULLIF(cast(pc.name as varchar(max)), '') + char(10), '') --parentClientCorporationID
                        + coalesce('Revenue: ' + nullif(convert(varchar(max),CC.revenue), '') + char(10), '')
                        + coalesce('Status: ' + nullif(convert(varchar(max),ltrim(CC.status)), '') + char(10), '')
                        ...
                        --+ Coalesce('Notes: ' + NULLIF(ltrim(rtrim(cast(CC.notes as varchar(max)))), '') + char(10), '')
        ...
        left join bullhorn1.View_ClientCorporationLastModified lm on lm.ClientCorporationID = CC.clientCorporationID
...
, t as (
        ...
        , UC.action, UC.comments
                ...
                where 
                UC1.clientCorporationID is not null
...
--select distinct authoremail from owner where rn = 1 
...
select --top 100 
	...
	, o.authoremail as 'company-owners' --, CC.ownership as 'company-owners'
...
-- select count (*) --560 -- select distinct customText8 --CC.ownership -- select *
from bullhorn1.BH_ClientCorporation CC
...
left join (select clientCorporationID,authoremail from owner where rn = 1) o on o.clientCorporationID = CC.clientCorporationID
--where CC.ClientCorporationID in (177,3227)
