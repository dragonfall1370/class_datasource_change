...
, note as (
	SELECT CA.userID
		 , Stuff(  
                                ...
                                + coalesce('Masters: ' + NULLIF(convert(varchar(max),CA.customText1), '') + char(10), '')
                                + coalesce('Functions: ' + NULLIF(convert(varchar(max),CA.customText9), '') + char(10), '')
                                + coalesce('Specializations: ' + NULLIF(convert(varchar(max),CA.customText13), '') + char(10), '')
                                + coalesce('Communication: ' + NULLIF(convert(varchar(max),CA.customText11), '') + char(10), '')
                                + coalesce('Appearance: ' + NULLIF(convert(varchar(max),CA.customText10), '') + char(10), '')
                                + coalesce('Languages: ' + NULLIF(convert(varchar(max),CA.customText5), '') + char(10), '')
                                + coalesce('Current Social Benefits: ' + NULLIF(convert(varchar(max),CA.customText7), '') + char(10), '')
                                + coalesce('Desired Social Benefits: ' + NULLIF(convert(varchar(max),CA.customText8), '') + char(10), '')
                                + coalesce('Interview Date: ' + NULLIF(convert(varchar(max),CA.customDate1), '') + char(10), '')
                                + coalesce('Shift Availability: ' + NULLIF(convert(varchar(max),CA.willRelocate), '') + char(10), '') --CustomComponent3
                                --+ coalesce('ButtonTest: ' + NULLIF(convert(varchar(max),CA.CustomComponent2), '') + char(10), '')
                                + coalesce('Desired Annual Salary: ' + NULLIF(convert(varchar(max),CA.salary), '') + char(10), '')
                                + coalesce('Opted Out: ' + NULLIF(convert(varchar(max),CA.massMailOptOut), '') + char(10), '')
                                + coalesce('Referred by: ' + NULLIF(convert(varchar(max),CA.referredByUserID), '') + char(10), '')
                                ...
                                + coalesce('Latest Comment: ' + NULLIF(convert(varchar(max),lc.comments), '') + char(10), '') --CA.latestComment
                                --+ coalesce('Submissions: ' + NULLIF(convert(varchar(max),CA.openSubmissions), '') + char(10), '')
                                 + coalesce('CV: ' + NULLIF([dbo].[fn_ConvertHTMLToText](UC1.description), '') + char(10), '') --CA.description
                        , 1, 0, '') as note
...
--select * from Education
	...
	select --top 200
		...
		, C.Nickname as 'Preferred Name' --NO
		, CONVERT(VARCHAR(10),C.dateOfBirth,120) as 'candidate-dob'
		, CASE WHEN (tc.abbreviation = 'NONE' OR tc.abbreviation = 'NULL' OR tc.abbreviation = 'ZR') THEN '' ELSE tc.abbreviation END as 'candidate-citizenship' --NO
		...
		, e2.email as 'candidate-workEmail' -- NO
		, C.mobile as 'candidate-phone'
		, C.mobile as 'candidate-mobile'
		, C.phone as 'candidate-homePhone' --NO
		, C.workPhone as 'candidate-workPhone' --NO
		, 'PERMANENT' as 'candidate-jobTypes' --NO
		...
		--, cast(C.salary as int) as 'candidate-desiredSalary' --,C.customTextBlock2 --NO
		, c.companyURL as 'candidate-Linkedinprofile'
		...
		, SN.SkillName as 'candidate-skills' --NO
	...
	-- select count (*) -- select distinct employmentPreference -- select companyURL -- select top 10 *
