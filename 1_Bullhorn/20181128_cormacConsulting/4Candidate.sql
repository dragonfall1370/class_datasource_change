...
, note as (
	SELECT CA.userID
		 , Stuff( Coalesce('Candidate ID: ' + NULLIF(cast(CA.userID as varchar(max)), '') + char(10), '')  
                        + Coalesce('Title: ' + NULLIF(convert(varchar(max),CA.namePrefix), '') + char(10), '')
                        ...
                        + Coalesce('Date Available: ' + NULLIF(convert(varchar(max),CA.dateAvailable), '') + char(10), '')
                        + Coalesce('Available Until: ' + NULLIF(convert(varchar(max),CA.dateAvailableEnd), '') + char(10), '')
                        + Coalesce('Desired Daily Rate: ' + NULLIF(convert(varchar(max),CA.dayRate), '') + char(10), '')
                        + Coalesce('Current Daily Rate: ' + NULLIF(convert(varchar(max),CA.dayRateLow), '') + char(10), '')
                        
                        + Coalesce('Employee Payment Type: ' + NULLIF(cast(ca.EmployeeType as varchar(max)), '') + char(10), '')
                        + Coalesce('Desired Hourly Rate: ' + NULLIF(convert(varchar(max),CA.hourlyRate), '') + char(10), '')
                        + Coalesce('Current Hourly Rate: ' + NULLIF(convert(varchar(max),CA.hourlyRateLow), '') + char(10), '')
