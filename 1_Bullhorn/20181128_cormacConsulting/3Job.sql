...
, note as (
	...
	, Stuff(  
              ...
              + Coalesce('Primary Approver: ' + NULLIF(cast(JP.correlatedCustomText1 as varchar(max)), '') + char(10), '')
              + Coalesce('Secondary Approver: ' + NULLIF(cast(JP.correlatedCustomText2 as varchar(max)), '') + char(10), '')
              + Coalesce('Perm Fee (%): ' + NULLIF(cast(JP.feeArrangement as varchar(max)), '') + char(10), '')
              + Coalesce('Open / Closed: ' + NULLIF( cast( iif(JP.isOpen in (null,0),'Closed','Open') as varchar(max)), '') + char(10), '') --+ Coalesce('isOpen: ' + NULLIF(cast(JP.isOpen as varchar(max)), '') + char(10), '')
...
select --top 100
	...
	, b.clientID as 'position-contactId'
       --, a.clientUserID as '#UserID', uc.firstname as '#ContactFirstName', uc.lastname as '#ContactLastName'
	...
	, case when a.employmentType is null then 'PERMANENT'
	       ...
	       when a.employmentType = 'Contract To Hire' then 'PERMANENT'
	       when a.employmentType = 'Retained' then 'INTERIM_PROJECT_CONSULTING'
	...
	, a.correlatedCustomText3 as 'position-currency'
