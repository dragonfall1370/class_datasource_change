  	...
  	select JP.jobPostingID as JobID
  		, JP.title as JobTitle
              , case when JP.employmentType is null then 301
                     when JP.employmentType in ('Permanent','Retained') then 301
...
when JR.status = '3+ Interview' then 'SECOND_INTERVIEW'
...
when JR.status = 'Consultant Rejected' then 'SHORTLISTED'-- > Rejected'
when JR.status = 'Covered By Other Agency' then 'SHORTLISTED'-- > Rejected'
when JR.status = 'HR Interview' then 'FIRST_INTERVIEW'
when JR.status = 'Interview Scheduled' then 'FIRST_INTERVIEW'
when JR.status = 'Line Mgr Interview' then 'FIRST_INTERVIEW'
--when JR.status = 'Longlisted' then 'SHORTLISTED'
--when JR.status = 'New Application' then 'SHORTLISTED'
--when JR.status = 'New Lead' then ''
--when JR.status = 'New Submission' then 'SENT'
when JR.status = 'Offer Accepted' then 'OFFERED' --'PLACED PERMANENT - PLACED CONTRACT'
when JR.status = 'Offer Extended' then 'OFFERED'
when JR.status = 'Offer Rejected' then 'OFFERED'-- > Rejected'
when JR.status = 'Placed' then 'PLACED' --'OFFERED' --'PLACED PERMANENT - PLACED CONTRACT'
...
when JR.status = 'To Be Contacted' then 'SHORTLISTED'
--when JR.status = 'undefined' then ''
--when JR.status = 'Web Response' then 'SHORTLISTED'
