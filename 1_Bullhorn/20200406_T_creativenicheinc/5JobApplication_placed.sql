...
with
  JPInfo as (
	...
	where 1=1  and (JP.isdeleted <> 1 and JP.status <> 'Archive') --and JP.title <> ''
)
--select distinct JobType from JPInfo --order by JobID
...
when JR.status = 'Placed' then 'OFFERED' --PLACED
        ...
        where CAI.candidateID is not null and JPI.JobID is not null
UNION
       select 
...
--where ( [application-positionExternalId] = 16 and [application-candidateExternalId] = 3312)
...
--select distinct JobType, count(*) from ja1 group by JobType
