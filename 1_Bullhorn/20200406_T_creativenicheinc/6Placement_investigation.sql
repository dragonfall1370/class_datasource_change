...
--            + Coalesce('Invoice contact: ' + NULLIF(convert(nvarchar(max), pl.reportto), '') + char(10), '')
       ...
       , Stuff(
            + Coalesce('Commission Info: ' + NULLIF(convert(nvarchar(max), pc.note), '') + char(10), '')
              , 1, 0, '') as 'InvoiceNote'
...
-- select count(*) --2086  -- select customText1 --hoursPerDay --pl.billingUserID, pl.statementUserID -- select distinct pl.employmentType -- correlatedCustomText2 -- pl.salaryUnit -- statement.email -- select DATEADD(year, 1, pl.dateBegin) 
...
left join (
       select placementID,  STRING_AGG( note, char(10)) WITHIN GROUP (ORDER BY commissionID desc) note
       from ( 
              select pc.placementID,commissionID,
                      + Stuff(
                      ...
                      + Coalesce('Fee Split: ' + NULLIF(convert(nvarchar(max), pc.commissionPercentage*pl.fee*pl.salary), '') + char(10), '')--, pc.flatPayout as 'Split Amount'
...
--                      + Coalesce('Date Added: ' + NULLIF(convert(nvarchar(max), pc.dateAdded), '') + char(10), '')--, pc.dateAdded as 'N/A'                      
              ...
              left join bullhorn1.BH_Placement pl on pl.placementID = pc.placementID
              left join (select userID,name from bullhorn1.BH_UserContact) UC on UC.UserID = pc.userID 
--              where pc.placementID = 13374 
              ) t
       GROUP BY t.placementID
...
and PL.placementID in (13569)
--and JPI.JobID in (14123)
