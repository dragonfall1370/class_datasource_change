...
, placementnote as (
	...
	, Stuff((select  /* Add placement information under job posting */
	         Coalesce('Placement - Employee type: ' + NULLIF(cast(PL.employeeType as varchar(max)), '') + char(10), '')
	        + Coalesce('Placement - Fee(%): ' + NULLIF(cast(PL.fee as varchar(max)), '') + char(10), '')
+ Coalesce('Placement - Salary: ' + NULLIF(cast(PL.salary as varchar(max)), '') + char(10), '')
+ Coalesce('Placement - Pay Unit: ' + NULLIF(cast(PL.salaryUnit as varchar(max)), '') + char(10), '')
...
select --top 100 
	...
	--, cast(a.description as varchar(max)) as 'position-internalDescription'
       , stuff( Coalesce(NULLIF(cast(a.description as varchar(max)), '') + char(10), '')
               + Coalesce('Skills: ' + NULLIF(cast(a.skills as varchar(max)), '') + char(10), '')
...
-- select distinct employmentType -- select distinct Type -- select distinct status -- select distinct employmentType -- select count(*) --2574 -- select top 100 startDate  -- select *
from bullhorn1.BH_JobPosting a --where a.employmentType = 'Opportunity'
left join bullhorn1.BH_Client b on a.clientUserID = b.userID
