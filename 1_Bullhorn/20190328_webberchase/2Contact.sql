...
, note as (
	...
	, Stuff(  
	  Coalesce('BH Contact ID: ' + NULLIF(cast(UC.userID as varchar(max)), '') + char(10), '')
       ...
       + Coalesce('Home Phone: ' + NULLIF(UC.phone3, '') + char(10), '')
...
--       , UC.zip
--       , UC.countryID
       --, ltrim(Stuff( Coalesce(' ' + NULLIF(UC.workphone, ''), '') + Coalesce(', ' + NULLIF(UC.Phone2, ''), '') , 1, 1, '') ) as 'mobile_phone' --**
...
-- select count(*) --7487 -- select top 10 * -- select distinct UC.namePrefix
from bullhorn1.BH_Client Cl --where isPrimaryOwner = 1
left join bullhorn1.BH_UserContact UC ON Cl.userID = UC.userID
