...
--              + Coalesce('Status: ' + NULLIF(cast(CC.status as varchar(max)), '') + char(10), '')
              ...
              + Coalesce('Company Comments: ' + NULLIF(cast(CC.notes as varchar(max)), '') + char(10), '')
               ...
               --+ Coalesce('Billing Country: ' + NULLIF(tc.country, '') + char(10), '')
                , 1, 0, '') as note
       -- select  * -- select top 10 * -- select companyDescription
...
-- FILES
, doc0 (clientCorporationID,ResumeId) as (
        ...
        UNION select clientCorporationID, concat(clientCorporationID,'.html') as ResumeId  from bullhorn1.BH_ClientCorporation where cast(companyDescription as varchar(max)) <> ''
        )
, doc as (SELECT clientCorporationID, STUFF((SELECT ',' + ResumeId  from doc0 WHERE clientCorporationID = a.clientCorporationID FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '')  AS ResumeId FROM doc0 AS a GROUP BY a.clientCorporationID )
...
select --top 100 
       ...
       , iif(dup.rn > 1,concat(dup.name,' ',dup.rn), iif(dup.name is null,'No CompanyName',dup.name)) as 'company-name' --, iif(CC.clientCorporationID in (select clientCorporationID from dup where dup.rn > 1),concat(dup.name,' ',dup.rn), iif(CC.NAME = '' or CC.name is null,'No CompanyName',CC.NAME)) as 'company-name'
       ...
       , case when CC.city = '' then 'Tokyo' when (CC.city is null and tc.abbreviation <> 'US') then 'Tokyo' else CC.City end as 'company-locationCity'
       ...
       , CC.customTextBlock1 as 'Send Out Instructions'
-- select count (*) --560 -- select top 50 * -- select distinct CC.ownership
