...
, note as (
	select UC.userID
	, Stuff( 
	          Coalesce('BH Contact ID: ' + NULLIF(cast(Cl.userID as varchar(max)), '') + char(10), '')
	        + Coalesce('Title: ' + NULLIF(UC.namePrefix, '') + char(10), '') 
	        + Coalesce('Email 2: ' + NULLIF(e2.email, '') + char(10), '')
	        + Coalesce('Known as: ' + NULLIF(UC.nickName, '') + char(10), '')
	        + Coalesce('Type: ' + NULLIF(cast(Cl.type as varchar(max)), '') + char(10), '')
	        --+ Coalesce('Secondary Owners: ' + NULLIF(cast(UC.secondaryOwners as varchar(max)), '') + char(10), '')
	        + Coalesce('Num Owners: ' + NULLIF(cast(Cl.numowners as varchar(max)), '') + char(10), '')
...
-- select count(*) --10317 -- select top 10 *
...
------------
with comments as ( select Cl.clientID as 'contact_id'
...
--select count(*) from comments where externalId is not null --538216 > 563579
select * from comments where contact_id is not null --and externalId = 4142
*/
...
/*
with t as (
select  Cl.clientID as 'contact-externalId'
	, Cl.userID as 'BH UserID'
	, case when (ltrim(replace(UC.firstName,'?','')) = '' or  UC.firstName is null) then 'Firstname' else ltrim(replace(UC.firstName,'?','')) end as 'contact-firstName'
	, case when (ltrim(replace(UC.lastName,'?','')) = '' or  UC.lastName is null) then concat('Lastname-',Cl.clientID) else ltrim(replace(UC.lastName,'?','')) end as 'contact-Lastname'
	, UC.middleName as 'contact-middleName'
	, UC.Phone as 'Phone'
        ...
        , UC.Mobile as 'contact-phone'
        , ltrim(Stuff( Coalesce(' ' + NULLIF(UC.WorkPhone, ''), '')
                        + Coalesce(', ' + NULLIF(UC.Phone, ''), '')
                        + Coalesce(', ' + NULLIF(UC.Mobile, ''), '')
                        + Coalesce(', ' + NULLIF(UC.Phone2, ''), '')
                        + Coalesce(', ' + NULLIF(UC.Phone3, ''), '')
                , 1, 1, '') ) as 'contact-phone-final'         
from bullhorn1.BH_Client Cl --where isPrimaryOwner = 1
left join bullhorn1.BH_UserContact UC ON Cl.userID = UC.userID
left join bullhorn1.BH_UserContact UC2 on Cl.recruiterUserID = UC2.userID
where isPrimaryOwner = 1 )
select * from t where [contact-phone-final] is not null
*/