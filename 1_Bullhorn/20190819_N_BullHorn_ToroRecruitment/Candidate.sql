...
) , INDUSTRY0 as (-- BusinessSector >>> INDUSTRY
        SELECT userid, Split.a.value('.', 'VARCHAR(2000)') AS businessSectorID 
...
) , INDUSTRY as ( -- no records
        SELECT userid, STRING_AGG( ltrim(rtrim( convert(nvarchar(max),BSL.name) )),', ' ) WITHIN GROUP (ORDER BY BSL.name) name 
	from INDUSTRY0 left join bullhorn1.BH_BusinessSectorList BSL ON INDUSTRY0.businessSectorID = BSL.businessSectorID
	WHERE INDUSTRY0.businessSectorID <> '' GROUP BY userid
) , CATEGORY(candidateID, userid, categoryIDList, categoryID, name) as (-- CATEGORY >>> FUNCTIONAL EXPERTISE
...
) , SPECIALTY(candidateID, userid, specialtyIDList, SpecialtyID, name) as (--SPECIALTY >>> SUB FUNCTIONAL EXPERTISE
...
) , skill (candidateID,userid, SkillName) as ( --SKILL
...
) , admissionRows (userId, CombinedText, dateadded) as (-- ADMISSION
...
) , EducationGroup as (-- NEWEST EDUCATION
        select userID, max(userEducationID) as userEducationID 
...
) , EducationSummary(userId, es) as (-- Education Summary
       SELECT userId
       , STRING_AGG(
       ...
       FROM bullhorn1.BH_UserEducation GROUP BY userId        
) , EmploymentHistory(userId, eh) as (--EMPLOYMENT HISTORY
       SELECT userId
         , STRING_AGG( 
                     REPLACE( REPLACE( REPLACE( REPLACE( REPLACE(REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( 
                     REPLACE( REPLACE( REPLACE( REPLACE( REPLACE(REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( 
...
--       GROUP BY a.userId 
       ...
       FROM bullhorn1.BH_userWorkHistory GROUP BY userId
        ...
        SELECT userID, STRING_AGG( concat('Title: ',title,' - Status: ',status) ,char(10)) WITHIN GROUP (ORDER BY title) name 
        from wr GROUP BY userID 
) , lc (userid,comments,dateAdded,rn) as ( --LATEST COMMENT
...
) , owner as ( --CANDIDATE CERTIFICATE (no value)--OWNER
        select ca.candidateid, ca.recruiterUserID, ca.owneruseridlist, concat(ca.recruiterUserID,',', ca.ownerUserIDList) as owners 
        ...
        SELECT candidateid, Split.a.value('.', 'VARCHAR(100)') AS String
	...
	where owners is not null and owners <> '') AS A CROSS APPLY Data.nodes ('/M') AS Split(a)
        ...
        select distinct owner2a.candidateid, UC.email, UC.name 
        from owner2a 
        left join (select ca.candidateid, uc.email, uc.name from bullhorn1.Candidate CA left join bullhorn1.BH_UserContact UC on CA.UserID = UC.userID ) UC on UC.candidateid = owner2a.String
...
) , files(candidateUserID, files) as (-- DOCUMENT
        SELECT candidateUserID, STRING_AGG( concat(candidateFileID, fileExtension),',' ) WITHIN GROUP (ORDER BY candidateFileID) files
	from bullhorn1.View_CandidateFile GROUP BY candidateUserID
) , placementfiles(userID, files) as (
        SELECT userID, STRING_AGG( concat(placementFileID, fileExtension),',' ) WITHIN GROUP (ORDER BY placementFileID) files
	from bullhorn1.View_PlacementFile GROUP BY userID
) , doc(Userid, files) as ( 
        select f.candidateUserID, STRING_AGG( f.files,',' ) WITHIN GROUP (ORDER BY f.files) files
	...
	SELECT CA.candidateid
		 , stuff( 
                        coalesce('BH Candidate ID: ' + nullif(cast(CA.userID as varchar(max)), '') + char(10), '')
                        + coalesce('General Comments: ' + nullif(convert(varchar(max),CA.comments), '') + char(10), '')
--                        + coalesce('Any Holiday''s Booked?: ' + nullif(cast(CA.customText1 as varchar(max)), '') + char(10), '')
--                        + coalesce('References Requested: ' + nullif(cast(CA.customText5 as varchar(max)), '') + char(10), '')
                        + coalesce('Date available: ' + nullif(convert(nvarchar(10),ca.dateAvailable,120), '') + char(10), '')
--                        + coalesce('Available Until: ' + nullif(convert(nvarchar(10),ca.dateAvailableEnd,120), '') + char(10), '')
...
--                        + coalesce('Current Hourly Rate: ' + nullif(cast(ca.l as varchar(max)), '') + char(10), '')
...
--MAIN SCRIPT
select
         concat('TRCD', c.candidateID) as 'candidate-externalId'
	, case 
	...
	, iif(ed.rn > 1,concat(ed.rn, '_', ed.email), iif(ed.email = '' or ed.email is null, concat('candidate_',cast(c.candidateID as varchar(max)),'@noemailaddress.co'),ed.email) ) as 'candidate-email'
...
--left join bullhorn1.BH_ClientCorporation CC ON CC.clientCorporationID = C.clientCorporation
