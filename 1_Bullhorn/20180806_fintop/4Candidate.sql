...
--select top 10 * from placementfiles
	...
	select --top 5
		...
		, c.businessSectorIDList as 'INDUSTRY'
              , c.categoryID as 'FUNCTIONAL EXPERTISE'
              , c.skillIDList as 'SUB FUNCTIONAL EXPERTSE'
              , c.source as 'CANDIDATE SOURCE'
              , c.dayRate as 'Desired contract rate'
              , c.dayRateLow as 'Contract Rate'
...
from bullhorn1.Candidate CA --where CA.isPrimaryOwner = 1 --where convert(varchar(max),CA.comments) <> ''
left join CName on CA.userID = CName.Userid
where CA.isPrimaryOwner = 1 and CName.name is not null
