...
with note as (
	...
	, trim(@LineDouble from
		...
		, concat(@LineDouble, 'Company Description: ', CC.companyDescription)
		...
		--+ Coalesce('General Comments: ' + NULLIF(cast(CC.notes as varchar(max)), '') + char(10), '')
		...
		--+ Coalesce('Contact: ' + NULLIF(customText1, '') + char(10), '')
		...
		--+ Coalesce('Brand: ' + NULLIF(customText3, '') + char(10), '')
...
select
	...
	, trim(isnull(CC.city, '')) as 'company-locationCity'
	
	, trim(isnull(CC.state, '')) as 'company-locationState'
	
	, trim(isnull(CC.zip, '')) as 'company-locationZipCode'
	
	, tc.abbreviation as 'company-locationCountry'
	
	, trim(isnull(CC.phone, '')) as 'company-phone'
	--, CC.phone as 'company-switchboard'
	--, CC.fax as 'company-fax'
	, trim(isnull(CC.companyURL, '')) as 'company-website' --limitted by 100 characters
	--, CC.ownership as 'company-owners'
	, trim(' ,' from isnull(doc.ResumeId, '')) as 'company-document'
