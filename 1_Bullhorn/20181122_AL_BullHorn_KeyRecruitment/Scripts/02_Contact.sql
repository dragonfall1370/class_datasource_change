...
select
	...
	, trim(isnull(
		case
			when (ltrim(replace(UC.firstName,'?','')) = '' or  UC.firstName is null) then 'Firstname'
			else ltrim(replace(UC.firstName,'?',''))
		end
	, '')) as 'contact-firstName'
	, trim(isnull(
		case
			when (ltrim(replace(UC.lastName,'?','')) = '' or  UC.lastName is null) then concat('Lastname-',Cl.clientID)
			else ltrim(replace(UC.lastName,'?',''))
		end
	, '')) as 'contact-Lastname'
	, trim(isnull(UC.middleName, '')) as 'contact-middleName'
	, trim(isnull(UC2.email, '')) as 'contact-owners' --, UC2.name as '#Owners Name'
	, trim(isnull(UC.Phone, '')) as 'contact-phone'
	, trim(isnull(UC.Phone2, '')) as 'contact-homePhone' --<<
	--, ltrim(Stuff( Coalesce(' ' + NULLIF(UC.Phone2, ''), '') , 1, 0, '') ) as 'contact-phone'
	, trim(isnull(UC.Mobile, '')) as 'Contact-MobilePhone'
/*       , ltrim(Stuff( Coalesce(' ' + NULLIF(UC.Phone, ''), '')
    ...
    , ces.emails as 'contact-email'
	, trim(isnull(UC.occupation, '')) as 'contact-jobTitle'
	, trim(isnull(doc.files, '')) as 'contact-document'
	, trim(isnull(note.note, '')) as 'contact-note'
    ...
    --, trim(isnull(UC.customText12, '')) as 'contact-LinkedIn'
	, UC.address1 --<<
    , UC.address2 --<<
    , UC.city --<<
    , UC.state --<<
	...
	, NULLIF(cast(UC.skillSet as varchar(max)), '') as Skills
...
update VCCons
set [contact-email] = 'fieldfamilycupboard@telkomsa.net'
where [contact-externalId] = 215
go

update VCCons
set [contact-jobTitle] = ''
where [contact-jobTitle] is null or [contact-jobTitle] = 'xxx' or [contact-jobTitle] = '?'
go
