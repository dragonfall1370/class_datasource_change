...
--select count(*) from EmailBody --195896
--select top 100 * from EmailBody --where candidateid = 33
--select SMTPSendDate from EmailBody where SMTPSendDate is null or SMTPSendDate = ''
...
select --top 200
        ...
        --, coalesce(NULLIF(convert(varchar(19), SMTPSendDate, 120),''), convert(varchar(19), CURRENT_TIMESTAMP, 120)) as 'feedback_timestamp_insert_timestamp'
        , coalesce(NULLIF(SMTPSendDate,''), '') as 'feedback_timestamp_insert_timestamp'
