...
select --top 100 
	...
	, convert(varchar(10),iif(a.status in ('Archive','Filled by Client','Lost to Competitor','NA','Placed'),getdate()-2,dateClosed),120) as 'position-endDate'
	, doc.files as 'position-document'
	, concat(note.note,placementnote.note) as 'position-note' --left(,32000)
-- select distinct --employmentType -- select distinct status -- select count(*) --1380 -- select top 10 *
from bullhorn1.BH_JobPosting a
left join bullhorn1.BH_Client b on a.clientUserID = b.userID
