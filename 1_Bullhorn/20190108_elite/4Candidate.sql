...
, INDUSTRY0(userid, businessSectorID) as (SELECT userid, Split.a.value('.', 'VARCHAR(2000)') AS businessSectorID FROM (SELECT userid, CAST('<M>' + REPLACE(cast( ltrim(rtrim(businessSectorIDList)) as nvarchar(max)),',','</M><M>') + '</M>' AS XML) AS x FROM bullhorn1.Candidate where isdeleted <> 1 and status <> 'Archive') t CROSS APPLY x.nodes('/M') AS Split(a) )
, INDUSTRY(userId, BusinessSector) as (SELECT userid, STRING_AGG( ltrim(rtrim(BSL.name)),', ' ) WITHIN GROUP (ORDER BY BSL.name) name from INDUSTRY0 left join bullhorn1.BH_BusinessSectorList BSL ON INDUSTRY0.businessSectorID = BSL.businessSectorID WHERE INDUSTRY0.businessSectorID <> '' GROUP BY userid )
...
select --top 10
	...
	, owner.email as 'candidate-owners' --, C.recruiterUserID
...
left join owner on owner.recruiterUserID = C.recruiterUserID
