...
note as (
	...
	Stuff( coalesce('BH Company ID: ' + nullif(cast(CC.clientCorporationID as nvarchar(max)), '') + char(10), '')
              --+ coalesce('Company Legal Name: ' + nullif(cast(CC.customText2 as varchar(max)), '') + char(10), '')
              --+ coalesce('Company Number: ' + nullif(cast(CC.customText3 as varchar(max)), '') + char(10), '')
...
--       , CC.numEmployees as 'No. of Employees'
       ...
       , CC.customText2 as 'Billing Details > Trading Name'
       , CC.customText3 as 'Company Number'
