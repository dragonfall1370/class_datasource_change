...
, jobapp as (
        select
                ...
                , Coalesce(NULLIF(case
                                ...
                                when JR.status = 'Submitted to Job' then 'SENT'
                                
                                when JR.status = 'Candidate Evaluating JD' then 'SHORTLISTED'
                                when JR.status = 'Candidate Interested' then 'SHORTLISTED'
                                when JR.status = 'Candidate Not Interested' then 'SHORTLISTED'
                                when JR.status = 'Consultant Approved' then 'SHORTLISTED'
                                when JR.status = 'Consultant Rejected' then 'SHORTLISTED'
...
select * 
from jobapp 
where [application-stage] <> '' --and [#Candidate Name] like '%Freeman%'
