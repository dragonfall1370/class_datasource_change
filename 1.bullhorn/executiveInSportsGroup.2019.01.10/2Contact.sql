...
, note as (
	select UC.userID
	, Stuff(  
	    Coalesce('ID: ' + NULLIF(cast(UC.userID as varchar(max)), '') + char(10), '')
...
select   
       ...
       , iif(UC2.email = '0', '', uc2.email) as 'contact-owners' --, UC2.name as '#Owners Name'
       ...
       , UC.Phone2 as 'home_phone' --, ltrim(Stuff( Coalesce(' ' + NULLIF(UC.Phone2, ''), '') , 1, 0, '') ) as 'contact-home_phone'
