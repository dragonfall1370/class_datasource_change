...
, placement as (
       ...
       WHERE pl.userid = a.userid order by pl.dateAdded desc
       FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '') AS note 
       FROM bullhorn1.BH_Placement as a
       GROUP BY a.userId         
)        
...
+ Coalesce(NULLIF(convert(varchar(max),p.note), '') + char(10), '')
...
-- Reference (EMPTY)
	...
	left join ( select userid, firstname, lastname from bullhorn1.BH_UserContact )UC ON UC.userID = CA.referredByUserID
       ...
       left join placement p on p.userid = ca.userid
...
select --top 200
	...
	, case 
	     ...
	     when C.namePrefix in ('Miss') then 'MISS'	
	     when C.namePrefix in ('Mrs') then 'MRS'
	     when C.namePrefix in ('Ms','Ms.') then 'MS'
	     else '' end as 'candidate-title'
	, case
	     when C.namePrefix in ('Mr','Mr.') then 'MALE' 
	     when C.namePrefix in ('Miss','Mrs','Ms','Ms.') then 'FEMALE' 
	...
	, C.state as 'candidate-state'
       , tc.abbreviation as 'candidate-Country' --, CASE WHEN (tc.abbreviation = 'NONE' OR tc.abbreviation in ('NULL','ZR') ) THEN '' ELSE tc.abbreviation END 
	, cast(C.salaryLow as int) as 'candidate-currentSalary' , replace(C.customText11,'Euro','EUR') as 'currency'
...
-- select count (*) -- select distinct employmentPreference -- select skillset, skillIDlist, customTextBlock1 --select top 10 * -- select distinct namePrefix --customText11
...
left join note on note.candidateID = C.candidateID
