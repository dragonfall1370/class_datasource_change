...
+ Coalesce('Priority: ' + NULLIF(cast(JP.type as varchar(max)), '') + char(10), '')*/
  ...
  */
	        , 1, 0, '') as note
        -- select count(*) -- select top 50 * -- select feeArrangement
...
-- select distinct employmentType -- select distinct Type -- select distinct status -- select distinct customtext1 -- select count(*) --2574 -- select top 100 startDate  -- select customtext1
