...
with note as (
	...
	, Stuff( Coalesce('Client Corporation ID: ' + NULLIF(cast(CC.clientCorporationID as varchar(max)), '') + char(10), '')
        ...
        left join tmp_country tc ON CC.customText11 = tc.code ) --where CC.customText11 is not null )
...
-- select * --count (*) --4763 -- select distinct CC.ownership
from bullhorn1.BH_ClientCorporation CC
