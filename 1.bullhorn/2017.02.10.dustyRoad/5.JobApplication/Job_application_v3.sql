...
with
 tmp1 as (
	...
	left join bullhorn1.Candidate CA on VA.candidateUserID = CA.userID ) -- where CA.status not like '%Archived%' ) --2279 lines
...
UNION
	...
	where b.isPrimaryOwner = 1 ) --and b.status not like '%Archived%'
	--select * from temp
