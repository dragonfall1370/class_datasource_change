...
, note as (
	select Cl.clientID
	, Stuff(coalesce('BH Contact ID: ' + NULLIF(cast(UC.userID as varchar(max)), '') + char(10), '')
	        + coalesce('Next Steps: ' + nullif(convert(varchar(max),Cl.comments), '') + char(10), '')
                + coalesce('Date Next Step: ' + nullif(convert(varchar(max),UC.customDate1), '') + char(10), '')
                + coalesce('Last Visit: ' + nullif(convert(varchar(max),Cl.dateLastVisit), '') + char(10), '')
                + coalesce('Referred By: ' + nullif(convert(varchar(max),ref.name), '') + char(10), '') --UC.referredByUserID
                + coalesce('Revenue: ' + nullif(convert(varchar(max),CC.revenue), '') + char(10), '') --, CC.annualRevenue
                + coalesce('Secondary Owners: ' + nullif(convert(varchar(max),owner2c.name), '') + char(10), '')
                + coalesce('Status: ' + nullif(convert(varchar(max),Cl.status), '') + char(10), '')
                , 1, 0, '') as note
-- select count(*) --7487 -- select distinct Cl.status --UC.namePrefix --  select userid, ownerUserIDList
        from bullhorn1.BH_UserContact UC --where ownerUserIDList like '%,%'
        left join tmp_country on UC.countryID = tmp_country.CODE
        left join bullhorn1.BH_Client Cl on Cl.Userid = UC.UserID
        left join (select clientCorporationID, revenue from bullhorn1.BH_ClientCorporation) CC on CC.clientCorporationID = UC.clientCorporationID
        ...
        --and owner2c.userid is not null 
        --and UC.userid in (8281,12389,6467,10883,4281) 
        --and Cl.clientID= 3182
)
--select count(*) from note --10011
--select * from note where clientID in (1667,1901,2762,2822,3082,2008,2082,2918,2942,3052)
...
-- select count(*) --8366 -- select top 10 *
from bullhorn1.BH_Client Cl --where isPrimaryOwner = 1
left join bullhorn1.BH_UserContact UC ON Cl.userID = UC.userID
left join (select userID, email, name from bullhorn1.BH_UserContact) UC2 on Cl.recruiterUserID = UC2.userID
