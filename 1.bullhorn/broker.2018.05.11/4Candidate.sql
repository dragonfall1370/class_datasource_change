...
, note as (
	SELECT CA.userID
		 , Stuff( Coalesce('BullHorn Candidate ID: ' + NULLIF(cast(CA.userID as varchar(max)), '') + char(10), '')  
                        ...
                        --+ Coalesce('Placements: ' + NULLIF(cast(CA.activePlacements as varchar(max)), '') + char(10), '')
                        ...
                        , 1, 0, '') as note
	-- select top 10 * -- select referredBy, referredByUserID -- select count(*)
...
--select * from EmploymentHistory where userid in (164043);
	...
	select --top 200
	      ...
	      , case c.namePrefix
                     ...
                     when 'Mrs' then 'MRS'
                     when 'Mrs.' then 'MRS'
                     when 'Ms' then 'MS' end as 'candidate-title'
		, case c.namePrefix
                     ...
                     when 'Mrs.' then 'FEMALE'
                     when 'Ms' then 'FEMALE'
                     when 'Ms.' then 'FEMALE' end as 'candidate-gender'
	...
	-- select count (*) -- select distinct gender -- select skillset, skillIDlist, customTextBlock1 --select top 10 *
	...
	left join EducationSummary on EducationSummary.userID = C.userID
