...
, note as (
	...
	, Stuff(  Coalesce('BH Job ID: ' + NULLIF(cast(JP.jobPostingID as varchar(max)), '') + char(10), '')
	        ...
	        , 1, 0, '') as note
        -- select JP.jobPostingID,title, JP.numOpenings
...
select --top 100 
	...
	, case when a.employmentType is null then 'PERMANENT'
	       ...
	       when a.employmentType = 'Permanent - RPO' then 'PERMANENT'
...
-- select distinct employmentType --select distinct Type -- select distinct status -- select distinct customtext1 -- select count(*) --1380 -- select customInt1 -- select *
from bullhorn1.BH_JobPosting a
