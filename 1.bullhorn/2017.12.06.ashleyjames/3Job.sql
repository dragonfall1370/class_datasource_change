...
, int as (
        ...
        , Stuff(   Coalesce('Required Skills: ' + NULLIF(cast(JP.skills as varchar(max)), '') + char(10), '')
                 + Coalesce('Internal Job Description: ' + NULLIF(cast(JP.description as varchar(max)), '') + char(10), '')
...
, note as (
	...
	, Stuff(  Coalesce('BH Job ID: ' + NULLIF(cast(JP.jobPostingID as varchar(max)), '') + char(10), '')
	        ...
	        + Coalesce('Job Category: ' + NULLIF(cast(JP.publishedCategoryID as varchar(max)), '') + char(10), '')
	        + Coalesce('Industries: ' + NULLIF(cast(cc.businesssectorlist as varchar(max)), '') + char(10), '')
	        ...
	        + Coalesce('Country: ' + NULLIF(cast(JP.countryID as varchar(max)), '') + char(10), '')	        
	        + Coalesce('Published Contact Info: ' + NULLIF(cast(JP.responseUserID as varchar(max)), '') + char(10), '')	        
	        ...
	        --+ Coalesce('Custom 10: ' + NULLIF(cast(JP.customPayRate10 as varchar(max)), '') + char(10), '')
	        ...
	        , 1, 0, '') as note
        -- select top 30 CC.industryList,  cc.businesssectorlist
        from bullhorn1.BH_JobPosting JP
        left join bullhorn1.BH_CategoryList CL on JP.publishedCategoryID = CL.categoryID
        left join bullhorn1.BH_ClientCorporation CC on JP.clientCorporationID = CC.clientCorporationID --where convert(varchar(max),CC.industryList) <> '' or convert(varchar(max),cc.businesssectorlist) <> ''
        )
...
select --top 100 
	...
	, case when a.employmentType is null then 'PERMANENT'
	       ...
	       when a.employmentType = 'Fixed Term' then 'TEMPORARY'
	       else ''
	...
	, a.isOpen, convert(varchar(10),iif(a.isOpen in (null,0),getdate()-1,dateClosed),120) as 'position-endDate' --, convert(varchar(10),iif(a.status in ('Archive','Cancelled','Filled by Client','Lost to Competitor'),getdate()-2,dateClosed),120) as 'position-endDate'
...
-- select distinct employmentType --select distinct Type -- select distinct status -- select distinct customtext1 --select count(*) --1380 -- select top 10 *
from bullhorn1.BH_JobPosting a
left join bullhorn1.BH_Client b on a.clientUserID = b.userID
