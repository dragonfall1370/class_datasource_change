...
, jobapp as (
        select
                ...
                , Coalesce(NULLIF(case
                                ...
                                when JR.status = '2nd Interview' then '2ND_INTERVIEW'
                                when JR.status = '3rd Interview' then '2ND_INTERVIEW'
                                when JR.status = 'Final Interview' then '2ND_INTERVIEW'
                                when JR.status = '1st Interview' then '1ST_INTERVIEW'
                                when JR.status = 'CV Sent' then 'SENT'
                                when JR.status = 'Submitted' then 'SENT'
                                when JR.status = 'Candidate Rejected' then 'SHORTLISTED'
                                --when JR.status = 'Client Rejected' then 'CANDIDATES'
