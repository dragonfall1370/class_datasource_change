...
with EmailBody as (
        ...
        left join (SELECT userMessageID, STUFF((SELECT ', ' + name from bullhorn1.BH_UserMessageFile WHERE userMessageID = a.userMessageID order by name asc FOR XML PATH (''), TYPE).value('.', 'nvarchar(64)'), 1, 1, '') AS att FROM bullhorn1.BH_UserMessageFile AS a GROUP BY a.userMessageID ) umf on umf.userMessageID = um.userMessageID
...
--select count(*) from EmailBody --42213
...
select --top 200
        ...
        /*, concat(  'From: ' , convert(nvarchar(max), externalFrom, 120) , char(10) ,
                ...
                , ltrim(Stuff( Coalesce('From: ' + NULLIF(externalFrom, '') + char(10), '')
                        + Coalesce('To: ' + NULLIF(externalTo, '') + char(10), '')
                        + Coalesce('CC: ' + NULLIF(externalCC, '') + char(10), '')
                        + Coalesce('BCC: ' + NULLIF(externalBCC, '') + char(10), '')
                        + Coalesce('Subject: ' + NULLIF(subject, '') + char(10), '')
                        + Coalesce('Content: ' + NULLIF(comments, '') + char(10), '')
                        + Coalesce('Attachments: ' + NULLIF(att, ''), '')
                        , 1, 0, '') ) as 'comment_body'
