...
, job (jobPostingID, clientID, CompanyId, CompanyName, Companystatus, ContactUserID, Contact_firstname, Contact_lastname, Contact_isdeleted, Contact_status, title, starDate, rn) as (
	SELECT distinct a.jobPostingID as jobPostingID
...
, note as (
	...
	, Stuff(  
                ...
                + Coalesce('Broadbean: ' + NULLIF(cast(JP.customtext10 as varchar(max)), '') + char(10), '')
...
--	      , 1, 0, '') as 'position-internalDescription'
	...
	, note.note as 'position-note' --left(,32000) concat(note.note,placementnote.note) 
       , ltrim(Stuff( 
              ...
              , 1, 1, '') ) as 'key_words'
--select count(*) --14770 -- select distinct employmentType -- select distinct Type -- select distinct status -- select distinct customtext1 4 -- select top 100 startDate  -- select *
