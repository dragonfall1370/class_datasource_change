...
, SkillIDName(userId, SkillName) as (SELECT userId, STUFF((SELECT DISTINCT ', ' + name from skillid where name <> '' and userId = a.userId FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 2, '') AS URLList FROM skillid as a where a.skillID <> '' GROUP BY a.userId)
--select top 1000 * from SkillIDName
--select * from t
--select count(*) from t --161831
...
, note as (
	SELECT CA.userID
		 , Stuff( 
                        ...
                        + Coalesce('Referred by: ' + NULLIF( concat(cast(CA.referredByUserID as varchar(max)),' - ',UC.firstname,' ', UC.lastname ), '') + char(10), '') --CA.referredByUserID
                        + Coalesce('Current Daily Rate: ' + NULLIF(cast(CA.dayRateLow as varchar(max)), '') + char(10), '')
                        + Coalesce('Minimum Daily Rate Req: ' + NULLIF(cast(CA.dayRate as varchar(max)), '') + char(10), '')
                        + Coalesce('Bonus & Benefits: ' + NULLIF(cast(CA.customText14 as varchar(max)), '') + char(10), '')
                        + Coalesce('Level: ' + NULLIF(cast(SpeName.name as varchar(max)), '') + char(10), '') 
                        
                        + Coalesce('Additional Information: ' + NULLIF(
                                    Stuff(
                                    Coalesce(' ' + NULLIF(cast(CA.secondaryAddress1 as varchar(max)), '') , '')
                                + Coalesce(', ' + NULLIF(cast(CA.secondaryAddress2 as varchar(max)), '') , '')
                                + Coalesce(', ' + NULLIF(cast(CA.secondaryCity as varchar(max)), '') , '')
                                + Coalesce(', ' + NULLIF(cast(CA.secondaryState as varchar(max)), '') , '')
                                + Coalesce(', ' + NULLIF(cast(CA.secondaryZip as varchar(max)), '') , '')
                                + Coalesce(', ' + NULLIF(cast(addr.country as varchar(max)), '') , '')
                                , 1, 1, '') 
                                , '') + char(10), '')
                        + Coalesce('Category: ' + NULLIF(cast(CName.Name as varchar(max)), '') + char(10), '')
                        --+ Coalesce('Skills: ' + NULLIF(cast(SN.SkillName as varchar(max)), '') + char(10), '') --+ Coalesce('Level: ' + NULLIF(cast(CA.specialtyCategoryID as varchar(max)), '') + char(10), '')
                        + Coalesce('Sub-Categories: ' + NULLIF(cast(SN.SkillName as varchar(max)), '') + char(10), '')
                        ...
                        + Coalesce('Notice Period: ' + NULLIF(cast(CA.customText18 as varchar(max)), '') + char(10), '')
                        + Coalesce('Visa Status: ' + NULLIF(cast(CA.customText2 as varchar(max)), '') + char(10), '')
                        + Coalesce('Reason for Wanting to Leave: ' + NULLIF(cast(CA.customTextBlock2 as varchar(max)), '') + char(10), '')
                        + Coalesce('Languages: ' + NULLIF(cast(CA.phone2 as varchar(max)), '') + char(10), '')
                        + Coalesce('Nationality: ' + NULLIF(cast(CA.customText4 as varchar(max)), '') + char(10), '')
                        + Coalesce('CV: ' + NULLIF(UW.description, '') , '') --cast(UW.description as varchar(max))
                        ...
                        , 1, 0, '') as note
	-- select top 10 * -- select * -- select userid, secondaryAddress1,secondaryAddress2 --companyDescription -- select count(*) -- select skillSet, skillIDList
	...
	left join SkillIDName SN on CA.userID = SN.userId
	...
	left join ( select * from tmp_country ) addr on cast(CA.secondaryCountryID as varchar(20)) = addr.code
        ...
        --left join bullhorn1.BH_UserWork uw on uw.userid = ca.userid
/*        left join (SELECT userid, STUFF((
                        ...
                        REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( 
                                                ...
                                                ,char(0x001C),'') ,char(0x001D),'') ,char(0x001E),'') ,char(0x001F),'') + char(10) + '--------------------------------------------------' + char(10)
                                                --[dbo].[fn_ConvertHTMLToText](cast(description as varchar(max))) + char(10) + '--------------------------------------------------' + char(10)
                        ...
                        FROM (   select userid,
                                        REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( 
                                                ...
                                                ,char(0x0012),'') ,char(0x0013),'') ,char(0x0014),'') ,char(0x0015),'') ,char(0x0016),'') 
                                                ,char(0x0017),'') ,char(0x0018),'') ,char(0x0019),'') ,char(0x001A),'') ,char(0x001B),'') 
                                                ,char(0x001C),'') ,char(0x001D),'') ,char(0x001E),'') ,char(0x001F),'') as description
                                                --[dbo].[fn_ConvertHTMLToText](cast(description as varchar(max))) as description
                                        from bullhorn1.BH_UserWork) AS a GROUP BY a.userid ) uw on uw.userid = ca.userid */
        left join (SELECT userid, STUFF((
	...
	where CA.isPrimaryOwner = 1 ) --and CA.referredByUserID is not null )
--select * from note
--select count(*) from note
--select top 3 * from note where userid in (40837,16214,83141) or note like '%Additional Information%'
...
, Education as (select EG.userID, UE.userEducationID, UE.school, convert(varchar(10),UE.graduationDate,110) as graduationDate, UE.degree, UE.major, UE.comments from EducationGroup EG left join bullhorn1.BH_UserEducation UE on EG.userEducationID = UE.userEducationID)
	...
	select --top 200
		...
		--, case C.namePrefix when 'M' then 'MR' when 'F' then 'MISS'	else '' end as 'candidate-title'
		...
		, C.Nickname as 'Preferred Name'  --VC nick_name
		, CONVERT(VARCHAR(10),C.dateOfBirth,120) as 'candidate-dob'
		, CASE WHEN (tc.abbreviation = 'NONE' OR tc.abbreviation = 'NULL' OR tc.abbreviation = 'ZR') THEN '' ELSE tc.abbreviation END as 'candidate-citizenship' --- NO IMPORT
		...
		, CASE WHEN (tc.abbreviation = 'NONE' OR tc.abbreviation in ('NULL','ZR','BU') ) THEN '' ELSE tc.abbreviation END as 'candidate-Country'
		, C.zip as 'candidate-zipCode'
		, C.state as 'candidate-state' --, C.County
		, substring(C.customText1,0,4) as 'candidate-currency' --, C.customText1 as 'candidate-currency'
		...
		, C.customTextBlock1 as 'candidate-skills' -- , SN.SkillName as 'candidate-skills'
		, c.customText3 as 'Education > Qualification'
		...
		--, C.occupation as 'candidate-jobTitle1'
                /*, ltrim(Stuff( Coalesce(',Job Title: ' + NULLIF(C.occupation, '') + char(10), '')
                                 + Coalesce('Employer: ' + NULLIF(C.companyName, ''), '')
		...
		,C.dateAdded as 'candidate-RegistrationDate'
		--, left(comment.comment,32760) as 'candidate-comments'
	-- select count (*) --8877-- select distinct employmentPreference --) -- select distinct gender --namePrefix -- select top 10 County -- select email, email2 -- select dateAdded
	from bullhorn1.Candidate C --where C.isPrimaryOwner = 1 --73622
