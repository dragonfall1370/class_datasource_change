...
, case when (addr2 = '' OR addr2 is NULL) THEN '' ELSE concat('Address 2: ', addr2, char(10)) END as addr2
, case when (created_date = '' OR created_date is NULL) THEN '' ELSE concat('Created Date: ', created_date, char(10)) END as created_date
, case when (cast(overview as varchar(max)) = '' OR overview is NULL) THEN '' ELSE concat('Overview: ', addr2, char(10)) END as overview
, case when (industry = '' OR industry is NULL) THEN '' ELSE concat('Industry: ', industry, char(10)) END as industry
, concat ('Location: ',A.addr1,' ', A.city,' ', A.state,' ', A.zip,' ', CC.Code, char(10)) as location
from company A
left join CountryCode CC on A.country = CC.Name)
--select * from tmp1

, tmp2 (id, company_note) as (select id, concat(addr2, created_date, overview, industry, location)
from tmp1)
...
, tmp4 as (select id, concat(createdate, char(10), 'Parent company: ', name, char(10), 'Parent CompanyID: ', char(10), addr2) as branch_name_note
from tmp3)
...
where CB.name <> 'Main' --and industry is not null
