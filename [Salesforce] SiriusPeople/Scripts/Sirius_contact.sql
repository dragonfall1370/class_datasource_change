...
FROM UnionEmail where EMAIL is not NULL)
...
--CONTACT OWNERS
, ContactOwners as (select ID
	, CONCAT_WS(';',OWNERID, OWNER__C, CONTACT_OWNER_SBS__C, CONTACT_OWNERSSM__C, CONTACT_OWNER_STSSTM__C, CONTACT_OWNER_IND__C
...
, concat('Contact External ID: ',c.ID,char(10)
	, 'Salutation: ',c.SALUTATION,char(10)
	...
	, coalesce('Nof of termps: ' + c.NO_TEMPS_IN_TEAM__C + char(10),'')
...
, concat(coalesce(cf.ContactFiles,''), coalesce(',' + ca.ConAttachments,'')) as 'contact-document'
from Contact c
...
, satverbal as Sirius_field_value
, now() as Sirius_insert_timestamp
from contact
