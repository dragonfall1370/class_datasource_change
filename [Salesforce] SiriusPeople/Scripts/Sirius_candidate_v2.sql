...
, CandUnionEmail as (select ID, PEOPLECLOUD1__HOME_EMAIL__C as EMAIL from Candidate where PEOPLECLOUD1__HOME_EMAIL__C is not NULL
UNION ALL
select ID, PEOPLECLOUD1__WORK_EMAIL__C from Candidate where PEOPLECLOUD1__WORK_EMAIL__C is not NULL)
...
select sh.SMAGICINTERACT__CONTACT__C as CandidateExtID
	...
	, case when sh.CREATEDDATE is not NULL then CONVERT(Datetime, sh.CREATEDDATE, 103)
...
select t.WHOID as CandidateExtID
	...
	where t.WHOID is not NULL and t.WHOID in (select ID from Candidate where PEOPLECLOUD1__STATUS__C not in ('Inactive'))
...
, CONVERT(Datetime, CREATEDDATE, 103) as Sirius_RegDate
from Candidate

--Mapping
select id, external_id, insert_timestamp from Candidate
...
, DO_YOU_HAVE_A_VALID_DRIVERS_LICENSE__C    --DO_YOU_HAVE_A_VALID_DRIVERS_LICENSE
