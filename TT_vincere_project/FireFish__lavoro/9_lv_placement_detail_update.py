...
note = placement_detail_info_perm[[
    ...
    'AdditionalRemuneration',
    'AdditionalAmount'
    ,'AgreedSalary','candidate_externalid','job_externalid','NetHours'
    ]]
note['TotalRemuneration'] = note['AdditionalAmount'].astype(float) + note['AgreedSalary'].astype(float)
note['TotalRemuneration'] = note['TotalRemuneration'].apply(lambda x: str(x) if x else x)
note['note'] = note[['Notes',
    'AdditionalRemuneration',
    'AdditionalAmount',
    'TotalRemuneration','NetHours']] \
   .apply(lambda x: '\n'.join([': '.join([i for i in e]) for e in zip(['Notes', 'Additional Benefits',
                                                                       'Benefits Value', 'Total Remuneration', 'Standard Hours'], x) if e[1]]), axis=1)
