...
select j.ID as job_externalid
     ...
     , j.AdditionalRemuneration
     , j.AdditionalAmount
     , j.PermProposedSalary, l2.Value as LocationArea, l1.Value as Location, DisplayID
from Job j
left join JobType jt on jt.JobTypeID = j.JobTypeID
left join Drop_Down___Locations l1 on l1.id = j.LocationWSIID
left join Drop_Down___Locations l2 on l2.id = j.LocationAreaWSIID
"""
note = pd.read_sql(sql, engine_sqlite)
note['job_externalid'] = note['job_externalid'].apply(lambda x: str(x) if x else x)
note['TotalRemuneration'] = note['AdditionalAmount'].astype(float) + note['PermProposedSalary'].astype(float)
note['TotalRemuneration'] = note['TotalRemuneration'].fillna('0.0')
note['TotalRemuneration'] = note['TotalRemuneration'].astype(str)
note['note'] = note[['DisplayID', 'AdditionalRemuneration', 'AdditionalAmount', 'TotalRemuneration','LocationArea','Location']]\
    .apply(lambda x: '\n'.join([': '.join(e) for e in zip(['Lavoro ID', 'Additional Remuneration', 'Additional Amount', 'Total Remuneration','LocationArea','Location'], x) if e[1]]), axis=1)
