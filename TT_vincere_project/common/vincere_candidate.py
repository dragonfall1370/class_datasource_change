...
class Candidate(vincere_common.Common):
    def __init__(self, ddbconn):
        ...
        if ddbconn:
            ...
            self.candidate = pd.read_sql("select id, external_id as candidate_externalid, current_location_id, experience_details_json, edu_details_json from candidate where deleted_timestamp is null", ddbconn)
    ...
    def update_preferred_name_v2(self, df, dbconn_param, logger):
        """
        :param ddbconn:
        :param logger:
        :return:
        ...
        # transform data
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'candidate', ['nickname', ], ['id', ], logger)
        return tem2
    ...
    def update_location_type(self, df, logger):
        """
        ...
        """
        # prepare position type values
        tem2 = df[['candidate_externalid', 'location_type']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2['id'] = tem2['current_location_id'].apply(lambda x: int(str(x).split('.')[0]))
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['location_type', ], ['id', ], 'common_location', logger)
        return tem2

    def update_address_line1(self, df, logger):
        """
        deal with company has multi location case
        :param ddbconn:
        :param logger:
        ...
        """
        # prepare position type values
        tem2 = df[['candidate_externalid', 'address_line1']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2['id'] = tem2['current_location_id'].apply(lambda x: int(str(x).split('.')[0]))
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['address_line1', ], ['id', ], 'common_location', logger)
        return tem2

    def update_address_line2(self, df, logger):
        """
        deal with company has multi location case
        :param ddbconn:
        :param logger:
        ...
        """
        # prepare position type values
        tem2 = df[['candidate_externalid', 'address_line2']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2['id'] = tem2['current_location_id'].apply(lambda x: int(str(x).split('.')[0]))
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['address_line2', ], ['id', ], 'common_location', logger)
        return tem2
    ...
    def update_desired_bonus(self, df, logger):
        """
        """
        tem2 = df[['candidate_externalid', 'desired_bonus']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['desired_bonus', ], ['id', ], 'candidate', logger)
        return tem2

    def update_other_benefits(self, df, logger):
        """
        """
        tem2 = df[['candidate_externalid', 'other_benefits']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['other_benefits', ], ['id', ], 'candidate', logger)
        return tem2

    def update_current_bonus(self, df, logger):
        """
        """
        tem2 = df[['candidate_externalid', 'current_bonus']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['current_bonus', ], ['id', ], 'candidate', logger)
        return tem2

    def update_total_gross_per_annum(self, df, logger):
        """
        """
        tem2 = df[['candidate_externalid', 'total_p_a']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['total_p_a', ], ['id', ], 'candidate', logger)
        return tem2
    ...
    def update_owner(self, df, logger):
        ...
        tem2.candidate_owner_json = tem2.candidate_owner_json.map(lambda x: x.replace('"primary":false', '"primary":"false"'))
        ...
        tem2.value = tem2.value.astype(int)
    ...
    def update_skills(self, df, logger):
        """
        """
        tem2 = df[['candidate_externalid', 'skills']].drop_duplicates().groupby('candidate_externalid')['skills'].apply(','.join).reset_index()
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['skills', ], ['id', ], 'candidate', logger)
        return tem2

    def update_skills2(self, df, dbconn, logger):
        """
        """
        tem2 = df[['candidate_externalid', 'skills']].drop_duplicates().groupby('candidate_externalid')['skills'].apply(','.join).reset_index()
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn, 'update', 'candidate', ['skills', ], ['id', ], logger)
        return tem2
    ...
    def update_exprerience_work_history2(self, df, dbconn_param, logger):
        """
        """
        tem2 = df[['candidate_externalid', 'experience']].drop_duplicates().groupby('candidate_externalid')['experience'].apply('\n\n'.join).reset_index()
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2.experience = tem2.experience.map(lambda x: x.replace('\n', '<br/>'))
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'candidate', ['experience'], ['id'], logger)
        return tem2
    ...
    def update_last_activity_date(self, df, logger):
        ...
        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'last_activity_date']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2 = tem2.merge(pd.read_sql("select candidate_id from candidate_extension", self.ddbconn), left_on='id', right_on='candidate_id')
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['last_activity_date', ], ['candidate_id', ],'candidate_extension', logger)
        return tem2
    ...
    def update_note2(self, df, connectparams, logger):
        ...
        tem2 = df[['candidate_externalid', 'note']].dropna()
        tem2.note = tem2.note.map(lambda x: x.replace('\n', '<br/>'))
        tem2.note = tem2.note.map(lambda x: x.replace(r'\n', '<br/>'))
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
    ...
    def update_marital_status(self, df, logger):
        """
        :param ddbconn:
        :param logger:
        :return:
        ...
        tem2 = df[['candidate_externalid', 'maritalstatus']]
        # transform data
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['maritalstatus'], ['id'], 'candidate', logger)
        return tem2

    def update_met_notmet(self, df, logger):
        """
        :param ddbconn:
        :param logger:
        :return:
        ...
        # 1:Met
        # 2:not met
        tem2 = df[['candidate_externalid', 'status']]
        # transform data
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['status'], ['id'], 'candidate', logger)
        return tem2

    def update_will_relocate(self, df, logger):
        """
        :param ddbconn:
        :param logger:
        :return:
        ...
        # 1:Yes
        # 2:No
        tem2 = df[['candidate_externalid', 'relocate']]
        # transform data
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['relocate'], ['id'], 'candidate', logger)
        return tem2

    def update_payment_type(self, df, logger):
        """
        :param ddbconn:
        :param logger:
        :return:
        ...
        # 1:Met
        # 2:not met
        tem2 = df[['candidate_externalid', 'paymenttype']]
        tem2.loc[tem2['paymenttype'] == 'PAYE', 'payment_type'] = 2
        tem2.loc[tem2['paymenttype'] == 'Ltd', 'payment_type'] = 3
        # transform data
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['payment_type'], ['id'], 'candidate', logger)
        return tem2

    def update_availability_start(self, df, logger):
        """
        :param ddbconn:
        :param logger:
        :return:
        ...
        tem2 = df[['candidate_externalid', 'availability_start']]
        # transform data
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['availability_start'], ['id'], 'candidate', logger)
        return tem2
    ...
    def update_reference(self, df, dbconn_param, logger):
        """
        :param ddbconn:
        :param logger:
        :return:
        ...
        tem2 = df[['candidate_externalid', 'reference']].drop_duplicates().groupby('candidate_externalid')['reference'].apply('\n\n'.join).reset_index()
        # transform data
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'candidate', ['reference'], ['id'], logger)
        return tem2
    ...
    def update_gender(self, df, logger):
        ...
        # 1000: other
        # prepare position type values
        tem2 = df[['candidate_externalid', 'male']]
        # transform data
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2[['id', 'male']].dropna().drop_duplicates(), self.ddbconn, ['male'], ['id'], 'candidate', logger)
        return tem2
    ...
    def update_candidate_current_employer_title(self, df, logger):
        ...
        tem2['experience_details_json'] = tem2.apply(lambda x: re.sub(r',\"jobTitle\":\".*?\",|,\"jobTitle\":null,', (',"jobTitle":"%s",' % ('%r'%x['current_job_title']).strip("'")), x['experience_details_json']), axis=1)
        tem2['experience_details_json'] = tem2.apply(lambda x: re.sub(r',\"currentEmployer\":null,|,\"currentEmployer\":\".*?\",', (',"currentEmployer":"%s",' % ('%r'%x['current_employer']).strip("'")), x['experience_details_json']), axis=1)
    ...
    def update_candidate_current_employer_v3(self, df,conn_param, logger):
        tem = df
        tem['currentEmployer'] = tem['current_employer']
        tem['jobTitle'] = tem['current_job_title']
        tem = tem.fillna('')
        df_cols = []
        if 'currentEmployer' in tem.columns:
            tem.currentEmployer = tem.currentEmployer.map(lambda x: '"currentEmployer":"%s"' % x)
            df_cols.append('currentEmployer')
        if 'company' in tem.columns:
            tem.company = tem.company.map(lambda x: '"company":"%s"' % x)
            df_cols.append('company')
        if 'jobTitle' in tem.columns:
            tem.jobTitle = tem.jobTitle.map(lambda x: '"jobTitle":"%s"' % x)
            df_cols.append('jobTitle')
        if 'dateRangeFrom' in tem.columns:
            tem.dateRangeFrom = tem.dateRangeFrom.map(lambda x: '"dateRangeFrom":"%s"' % x)
            df_cols.append('dateRangeFrom')
        if 'dateRangeTo' in tem.columns:
            tem.dateRangeTo = tem.dateRangeTo.map(lambda x: '"dateRangeTo":"%s"' % x)
            df_cols.append('dateRangeTo')
        if 'cbEmployer' in tem.columns:
            tem['cbEmployer'] = tem['cbEmployer'].map(lambda x: '"cbEmployer":"%s"' % x)
            df_cols.append('cbEmployer')
        if 'address' in tem.columns:
            tem['address'] = tem['address'].map(lambda x: '"address":"%s"' % x)
            df_cols.append('address')
        tem['cbEmployer'] = tem['cbEmployer'].apply(lambda x: x.replace('1.0', '1'))
        tem['experience_details_json'] = tem[df_cols].apply(lambda x: '{%s}' % (','.join(x)), axis=1)
        tem2 = tem.groupby(['candidate_externalid'])['experience_details_json'].apply(','.join).reset_index()
        tem2.experience_details_json = tem2.experience_details_json.map(lambda x: '[%s]' % x)
        tem2 = tem2.merge(pd.read_sql("select id, external_id as candidate_externalid from candidate", self.ddbconn),
                          on=['candidate_externalid'])
        tem2['candidate_id'] = tem2['id']
        # vincere_custom_migration.load_data_to_vincere(tem2, conn_param, 'update', 'candidate_extension',['current_employer', 'current_job_title', ], ['candidate_id', ],logger)
        vincere_custom_migration.load_data_to_vincere(tem2, conn_param, 'update', 'candidate',
                                                      ['experience_details_json', ], ['id', ], logger)
    ...
    def update_candidate_current_employer_v2(self, df, conn_param, logger):
        tem2 = df[['candidate_externalid', 'current_employer']].dropna().drop_duplicates()
        tem2 = tem2.merge(self.candidate, left_on='candidate_externalid', right_on='candidate_externalid')
        ...
        vincere_custom_migration.load_data_to_vincere(tem2, conn_param, 'update', 'candidate_extension',['current_employer', ], ['candidate_id', ],logger)
        vincere_custom_migration.load_data_to_vincere(tem2, conn_param, 'update', 'candidate_work_history',['current_employer',], ['candidate_id', ], logger)
        vincere_custom_migration.load_data_to_vincere(tem2, conn_param, 'update', 'candidate', ['experience_details_json', ], ['id', ], logger)
        return tem2
    ...
    def emergency_name(self, df, logger):
        """

        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'emergency_name']].dropna()
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['emergency_name', ], ['id', ], 'candidate', logger)
        return tem2

    def emergency_phone(self, df, logger):
        """

        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'emergency_phone']].dropna()
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['emergency_phone', ], ['id', ], 'candidate', logger)
        return tem2

    def emergency_email(self, df, logger):
        """

        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'emergency_email']].dropna()
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['emergency_email', ], ['id', ], 'candidate', logger)
        return tem2

    def emergency_relationship(self, df, logger):
        """

        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'emergency_relationship']].dropna()
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['emergency_relationship', ], ['id', ], 'candidate', logger)
        return tem2
    ...
    def update_payslip_email(self, df, logger):
        """

        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'payslip_email']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['payslip_email', ], ['id', ], 'candidate', logger)
        return tem2
    ...
    def update_personal_email(self, df, logger):
        ...
        tem2['email2'] = tem2['personal_email']
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['email2', ], ['id', ], 'candidate', logger)
        return tem2
    ...
    def update_home_phone2(self, df, dbconn_param, logger):
        """
        ...
        tem2 = tem2.groupby('candidate_externalid').apply(lambda sufdf: ', '.join(sufdf.home_phone)).reset_index().rename(columns={0: 'home_phone'})
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'candidate', ['home_phone', ], ['id', ], logger)
        return tem2
    ...
    def update_work_phone_2(self, df, dbconn_param, logger):
        """
        ...
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        # vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['work_phone', ], ['id', ], 'candidate', logger)
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'candidate', ['work_phone', ],['id', ], logger)
        return tem2
    ...
    def update_candidate_status(self, df, dbcon_param, logger):
        """

        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'active']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['active', ], ['id', ], 'candidate', logger)
        return tem2
    ...
    def create_status_list(self, df, logger):
        ...
        if 'insert_timestamp' not in tem2.columns:
            tem2['insert_timestamp'] = datetime.datetime.now()
        status = pd.read_sql("select id , name from candidate_status", self.ddbconn)
        ...
        tem2['type'] = 1
        vincere_custom_migration.psycopg2_bulk_insert_tracking(tem2, self.ddbconn, ['creator_id', 'name', 'type', 'insert_timestamp'], 'candidate_status', logger)

    def add_candidate_status(self, df, logger):
        tem2 = df[['candidate_externalid', 'name']]
        status = pd.read_sql("select id as active, name from candidate_status", self.ddbconn)
    ...
    def update_xing(self, df, logger):
        ...
        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'xing']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['xing', ], ['id', ], 'candidate', logger)
        return tem2
    ...
    def update_skype(self, df, logger):
        ...
        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'skype']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['skype', ], ['id', ], 'candidate', logger)
        return tem2
    ...
    def update_education(self, df, dbconn_param, logger):
        tem = df
        tem = tem.fillna('')
        df_cols = []
        if 'schoolName' in tem.columns:
            tem.schoolName = tem.schoolName.map(lambda x: '"schoolName":"%s"' % x)
            df_cols.append('schoolName')
        if 'schoolAddress' in tem.columns:
            tem.schoolAddress = tem.schoolAddress.map(lambda x: '"schoolAddress":"%s"' % x)
            df_cols.append('schoolAddress')
        if 'institution' in tem.columns:
            tem.institution = tem.institution.map(lambda x: '"institution":"%s"' % x)
            df_cols.append('institution')
        if 'institutionAddress' in tem.columns:
            tem.institutionAddress = tem.institutionAddress.map(lambda x: '"institutionAddress":"%s"' % x)
            df_cols.append('institutionAddress')
        if 'institutionName' in tem.columns:
            tem.institutionName = tem.institutionName.map(lambda x: '"institutionName":"%s"' % x)
            df_cols.append('institutionName')
        if 'course' in tem.columns:
            tem.course = tem.course.map(lambda x: '"course":"%s"' % x)
            df_cols.append('course')
        if 'startDate' in tem.columns:
            tem.startDate = tem.startDate.map(lambda x: '"startDate":"%s"' % x)
            df_cols.append('startDate')
        if 'training' in tem.columns:
            tem.training = tem.training.map(lambda x: '"training":"%s"' % x)
            df_cols.append('training')
        if 'graduationDate' in tem.columns:
            tem.graduationDate = tem.graduationDate.map(lambda x: '"graduationDate":"%s"' % x)
            df_cols.append('graduationDate')
        if 'degreeName' in tem.columns:
            tem.degreeName = tem.degreeName.map(lambda x: '"degreeName":"%s"' % x)
            df_cols.append('degreeName')
        if 'qualification' in tem.columns:
            tem.qualification = tem.qualification.map(lambda x: '"qualification":"%s"' % x)
            df_cols.append('qualification')
        if 'department' in tem.columns:
            tem.department = tem.department.map(lambda x: '"department":"%s"' % x)
            df_cols.append('department')
        if 'thesis' in tem.columns:
            tem.thesis = tem.thesis.map(lambda x: '"thesis":"%s"' % x)
            df_cols.append('thesis')
        if 'description' in tem.columns:
            tem.description = tem.description.map(lambda x: '"description":"%s"' % x)
            df_cols.append('description')
        if 'grade' in tem.columns:
            tem.grade = tem.grade.map(lambda x: '"grade":"%s"' % x)
            df_cols.append('grade')
        if 'gpa' in tem.columns:
            tem.gpa = tem.gpa.map(lambda x: '"gpa":"%s"' % x)
            df_cols.append('gpa')
        if 'honorific' in tem.columns:
            tem.honorific = tem.honorific.map(lambda x: '"honorific":"%s"' % x)
            df_cols.append('honorific')
        if 'hornors' in tem.columns:
            tem.hornors = tem.hornors.map(lambda x: '"hornors":"%s"' % x)
            df_cols.append('hornors')
        if 'major' in tem.columns:
            tem.major = tem.major.map(lambda x: '"major":"%s"' % x)
            df_cols.append('major')
        if 'minor' in tem.columns:
            tem.minor = tem.minor.map(lambda x: '"minor":"%s"' % x)
            df_cols.append('minor')
        if 'educationId' in tem.columns:
            tem.educationId = tem.educationId.map(lambda x: '"educationId":"%s"' % x)
            df_cols.append('educationId')
        tem['edu_details_json'] = tem[df_cols].apply(lambda x: '{%s}' % (','.join(x)), axis=1)
        tem2 = tem.groupby('candidate_externalid')['edu_details_json'].apply(','.join).reset_index()
        tem2.edu_details_json = tem2.edu_details_json.map(lambda x: '[%s]' % x)
        # [{"languageCode":"km","level":""},{"languageCode":"my","level":""}]
        tem2 = tem2.merge(pd.read_sql("select id, external_id as candidate_externalid from candidate", self.ddbconn),
                          on=['candidate_externalid'])
        # vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['edu_details_json', ], ['id', ],'candidate', logger)
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'candidate', ['edu_details_json', ],['id', ], logger)
    ...
    def insert_common_pref_location_v2(self, df, dbcon_param, logger):
        """
        can be run many times, the second will fire an exception:
            psycopg2.IntegrityError: duplicate key value violates unique constraint "current_location_candidate_uni_idx"
            DETAIL:  Key (current_location_candidate_id)=(84120) already exists.
    ...
    def update_common_location_location_name_2(self, df, conn_param, logger):
        """
        can be run many times, the second will fire an exception:
            psycopg2.IntegrityError: duplicate key value violates unique constraint "current_location_candidate_uni_idx"
            DETAIL:  Key (current_location_candidate_id)=(84120) already exists.
        ...
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2['id'] = tem2['current_location_id']
        vincere_custom_migration.load_data_to_vincere(tem2, conn_param, 'update', 'common_location', ['location_name', ], ['id'], logger)
        return tem2
    ...
    def update_location_state2(self, df, conn_param, logger):
        """
        ...
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2['id'] = tem2['current_location_id'].apply(lambda x: int(str(x).split('.')[0]))
        vincere_custom_migration.load_data_to_vincere(tem2, conn_param, 'update', 'common_location', ['state', ], ['id'], logger)
        return tem2
    ...
    def update_location_city2(self, df, dbconn_param, logger):
        """
        ...
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2['id'] = tem2['current_location_id'].apply(lambda x: int(str(x).split('.')[0]))
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'common_location', ['city', ], ['id', ], logger)
        return tem2

    def update_address_line1_2(self, df, dbconn_param, logger):
        """
        ...
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2['id'] = tem2['current_location_id'].apply(lambda x: int(str(x).split('.')[0]))
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'common_location', ['address_line1', ], ['id', ], logger)
        return tem2

    def update_address_line2_2(self, df, dbconn_param, logger):
        """
        ...
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2['id'] = tem2['current_location_id'].apply(lambda x: int(str(x).split('.')[0]))
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'common_location', ['address_line2', ], ['id', ], logger)
        return tem2
    ...
    def update_personal_location_city2(self, df, dbcon_param, logger):
        """
        ...
        tem2 = tem2.merge(pd.read_sql("select id as candidate_id, external_id as candidate_externalid, personal_location_id from candidate", self.ddbconn), on=['candidate_externalid'])
        tem2['id'] = tem2['personal_location_id']
        vincere_custom_migration.load_data_to_vincere(tem2, dbcon_param, 'update', 'common_location', ['city', ], ['id', ], logger)
        return tem2
    ...
    def update_location_post_code2(self, df, dbcon_param, logger):
        """
        ...
        tem2 = tem2.merge(pd.read_sql("select id, external_id as candidate_externalid, current_location_id from candidate", self.ddbconn), on=['candidate_externalid'])
        tem2['id'] = tem2['current_location_id'].apply(lambda x: int(str(x).split('.')[0]))
        vincere_custom_migration.load_data_to_vincere(tem2, dbcon_param, 'update', 'common_location', ['post_code', ],['id', ], logger)
        return tem2
    ...
    def update_location_country_code(self, df, logger):
        ...
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2['id'] = tem2['current_location_id'].apply(lambda x: int(str(x).split('.')[0]) if x else x)
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['country_code', ], ['id', ], 'common_location', logger)
        return tem2

    def update_location_country_code2(self, df, dbcon_param, logger):
        """
        ...
        tem2['id'] = tem2['current_location_id'].apply(lambda x: int(str(x).split('.')[0]))
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['country_code', ], ['id', ], 'common_location', logger)
        # vincere_custom_migration.load_data_to_vincere(tem2, dbcon_param, 'update', 'common_location', ['country_code', ], ['id', ], logger)
        return tem2

    def update_visa_type(self, df, dbconn_param, logger):
        """
        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'visa_type']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'candidate', ['visa_type', ], ['id', ], logger)
        return tem2

    def update_visa_status(self, df, dbconn_param, logger):
        """
        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'visa_status']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'candidate', ['visa_status', ], ['id', ], logger)
        return tem2

    def update_visa_note(self, df, dbconn_param, logger):
        """
        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'visa_note']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'candidate', ['visa_note', ], ['id', ], logger)
        return tem2

    def update_visa_renewal_date(self, df, dbconn_param, logger):
        """
        :rtype: object
        """
        tem2 = df[['candidate_externalid', 'visa_renewal_date']]
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.load_data_to_vincere(tem2, dbconn_param, 'update', 'candidate', ['visa_renewal_date', ], ['id', ], logger)
        return tem2
    ...
    def insert_candidate_branch(self, df, logger):
        ...
        tem2 = tem2.merge(pd.read_sql("select record_id, branch_id, 'existed' as note from branch_record where record_type = 'candidate'", self.ddbconn), on=['record_id', 'branch_id'], how='left')
        tem2 = tem2.loc[tem2['note'].isnull()]
        tem2['insert_timestamp'] = datetime.datetime.now()
        tem2['record_type'] = 'candidate'
        vincere_custom_migration.psycopg2_bulk_insert_tracking(tem2, self.ddbconn, cols, 'branch_record', logger)
        return tem2

    def insert_candidate_brand(self, df, logger):
        tem2 = df[['candidate_externalid', 'name']].dropna().drop_duplicates()
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2.rename(columns={'id':'candidate_id', }, inplace=True)
        cols = ['candidate_id', 'team_group_id', 'insert_timestamp']
        ...
        tem2 = tem2.merge(pd.read_sql("select candidate_id, team_group_id, 'existed' as note from team_group_candidate", self.ddbconn), on=['candidate_id', 'team_group_id'], how='left')
        tem2 = tem2.loc[tem2['note'].isnull()]
        tem2['insert_timestamp'] = datetime.datetime.now()
        vincere_custom_migration.psycopg2_bulk_insert_tracking(tem2, self.ddbconn, cols, 'team_group_candidate', logger)
        return tem2

    def insert_industry_2(self, ind, colname_ind, sind, colname_sind, mylog):
        time = datetime.datetime.now()
        df_ind = ind[[colname_ind]].drop_duplicates()
        df_sind = None
        if sind is not None:
            df_sind = sind[[colname_ind, colname_sind]].drop_duplicates().dropna()
        cols = ['name', 'insert_timestamp']
        ...
        mylog.info("inserting vertical")
        df_vertical = df_ind
        df_vertical['name'] = df_vertical.colname_ind
        df_vertical['insert_timestamp'] = time
        vincere_custom_migration.psycopg2_bulk_insert(df_vertical, self.ddbconn, cols, 'vertical')
        ...
        vincere_custom_migration.psycopg2_bulk_insert(df_vertical, self.ddbconn, cols, 'vertical_detail_language')
        vincere_custom_migration.mapping_industries_to_team_brand(self.ddbconn)
        if df_sind is not None:
            df_sind = df_sind.merge(pd.read_sql("select id, name from vertical", self.ddbconn),left_on=colname_ind, right_on='name')
            df_sind['parent_id'] = df_sind['id']
            df_sind['name'] = df_sind.colname_sind
            df_sind['time'] = time
            cols = ['parent_id', 'name', 'insert_timestamp']
            vincere_custom_migration.psycopg2_bulk_insert(df_sind, self.ddbconn, cols, 'vertical')
        return df_vertical
    ...
    def insert_candidate_industry_subindustry(self, df, logger):
        tem2 = df[['candidate_externalid', 'name']].dropna().drop_duplicates()
    ...
    def insert_internal_note(self, df, logger):
        tem2 = df[['candidate_externalid', 'note', 'title', 'insert_timestamp']].dropna().drop_duplicates()
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        tem2.rename(columns={'id':'candidate_id', }, inplace=True)
        cols = ['title', 'candidate_id', 'insert_timestamp', 'note']
        tem2.note = tem2.note.map(lambda x: x.replace('\n', '<br/>'))
        tem2.note = tem2.note.map(lambda x: x.replace(r'\n', '<br/>'))
        vincere_custom_migration.psycopg2_bulk_insert_tracking(tem2, self.ddbconn, cols, 'candidate_note', logger)
        return tem2
    ...
    def insert_custom_field_table_value(self, df, parent_id, child_id, logger):
        tem2 = df[['candidate_externalid', 'value']].dropna()
        tem2['index'] = tem2.groupby('candidate_externalid').cumcount() + 1
        pare_cff = pd.read_sql("select * from configurable_form_field where field_key='%s';" % parent_id, self.ddbconn)
        chil_cff = pd.read_sql("select * from configurable_form_field where field_key='%s';" % child_id, self.ddbconn)
        parent_id = pare_cff.loc[0].get('id')
        children_id = chil_cff.loc[0].get('id')
        children_constraint_id = chil_cff.loc[0].get('constraint_id')
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid']).rename(columns={'id': 'candidate_id'})
    ...
    def insert_custom_field_table_value_v2(self, df, conn_param, parent_id, child_id, logger):
        tem2 = df[['candidate_externalid', 'value']].dropna()
        tem2['index'] = tem2.groupby('candidate_externalid').cumcount() + 1
        pare_cff = pd.read_sql("select * from configurable_form_field where field_key='%s';" % parent_id, self.ddbconn)
        chil_cff = pd.read_sql("select * from configurable_form_field where field_key='%s';" % child_id, self.ddbconn)
        ...
        # check existed
        ext = pd.read_sql(
            "select id, candidate_id, parent_id, children_id, constraint_id, 'existed' as exsited from configurable_form_group_value",
            self.ddbconn)
        tem2 = tem2.merge(ext, on=['candidate_id', 'parent_id', 'children_id', 'constraint_id'], how='left')
        update_tem2 = tem2.loc[tem2.exsited.notnull()]
        insert_tem2 = tem2.loc[tem2.exsited.isnull()]

        vincere_custom_migration.load_data_to_vincere(tem2, conn_param, 'insert', 'configurable_form_group_value',
                                                      ['candidate_id', 'parent_id', 'children_id', 'text_data',
                                                                "index", 'insert_timestamp', 'constraint_id' ], [], logger)
        vincere_custom_migration.load_data_to_vincere(update_tem2, conn_param, 'update', 'configurable_form_group_value',
                                                      ['text_data', ], ['id', ], logger)

        # vincere_custom_migration.psycopg2_bulk_insert_tracking(insert_tem2, self.ddbconn,['candidate_id', 'parent_id', 'children_id', 'text_data',
        #                                                         "index", 'insert_timestamp', 'constraint_id'],
        #                                                        'configurable_form_group_value', logger)
        # vincere_custom_migration.psycopg2_bulk_update_tracking(update_tem2, self.ddbconn, ['text_data'], ['id'],
        #                                                        'configurable_form_group_value', logger)
        return tem2
    ...
    def insert_onboarding_value(self, df, logger):
        tem2 = df[['candidate_externalid', 'value', 'type']].dropna()
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid']).rename(columns={'id': 'entity_id'})

        tem2.loc[tem2['type'] == 'national_insurance_number', 'field_id'] = 5
        tem2.loc[tem2['type'] == 'payment_method', 'field_id'] = 12
        tem2.loc[tem2['type'] == 'company_name', 'field_id'] = 26
        tem2.loc[tem2['type'] == 'company_number', 'field_id'] = 30
        tem2.loc[tem2['type'] == 'sort_code', 'field_id'] = 19
        tem2.loc[tem2['type'] == 'bank_name', 'field_id'] = 14
        tem2.loc[tem2['type'] == 'bank_branch', 'field_id'] = 15
        tem2.loc[tem2['type'] == 'account_name', 'field_id'] = 17
        tem2.loc[tem2['type'] == 'account_number', 'field_id'] = 16
        tem2.loc[tem2['type'] == 'employee_id', 'field_id'] = 3
        tem2.loc[tem2['type'] == 'payroll_id', 'field_id'] = 1
        tem2.loc[tem2['type'] == 'tax_code', 'field_id'] = 9
        tem2.loc[tem2['type'] == 'bsb_number', 'field_id'] = 20
        tem2.loc[tem2['type'] == 'national_insurance_letter', 'field_id'] = 6
        # tem2['candidate_id'] = tem2['id']
        tem2['entity'] = 'candidate'
        tem2['dataset'] = 0
        tem2['insert_timestamp'] = datetime.datetime.now()
        tem2['field_value'] = tem2['value']
        col = ['entity','entity_id','field_id','dataset','field_value','insert_timestamp']

        # check existed
        ext = pd.read_sql("select id, entity, entity_id, field_id, 'existed' as exsited from configurable_form_core_field_value", self.ddbconn)
        tem2 = tem2.merge(ext, on=['entity', 'entity_id', 'field_id'], how='left')
        update_tem2 = tem2.loc[tem2.exsited.notnull()]
        insert_tem2 = tem2.loc[tem2.exsited.isnull()]
        vincere_custom_migration.psycopg2_bulk_insert_tracking(insert_tem2, self.ddbconn, col,'configurable_form_core_field_value', logger)
        vincere_custom_migration.psycopg2_bulk_update_tracking(update_tem2, self.ddbconn, ['field_value'], ['id'], 'configurable_form_core_field_value', logger)
        return tem2
    ...
    def update_candidate_current_employer2(self, df, conn_param, logger):
        tem2 = df[['candidate_externalid', 'current_employer', 'current_job_title']].fillna('')
        tem2 = tem2.merge(self.candidate, left_on='candidate_externalid', right_on='candidate_externalid')
        # replace 2nd job title
        tem2.loc[tem2['experience_details_json'].isnull(), 'experience_details_json'] = '[{"company":null,"jobTitle":null,"currentEmployer":null,"yearOfExperience":null,"industry":null,"functionalExpertiseId":null,"subFunctionId":null,"cbEmployer":null,"currentEmployerId":null,"dateRangeFrom":null,"dateRangeTo":null}]'
        tem2['experience_details_json_2'] = ',{"company":"","jobTitle":"","currentEmployer":"","yearOfExperience":"","industry":"","functionalExpertiseId":"","subFunctionId":"","cbEmployer":"","currentEmployerId":"","dateRangeFrom":"","dateRangeTo":""}]'
        tem2['experience_details_json_2'] = tem2.apply(lambda x: re.sub(r',\"jobTitle\":\".*?\",|,\"jobTitle\":null,', (',"jobTitle":"%s",' % ('%s' % x['current_job_title']).strip("'")), x['experience_details_json_2']),axis=1)
        tem2['experience_details_json_2'] = tem2.apply(lambda x: re.sub(r',\"currentEmployer\":null,|,\"currentEmployer\":\".*?\",', (',"currentEmployer":"%s",' % x['current_employer']), x['experience_details_json_2']), axis=1)
        tem2['experience_details_json'] = tem2[['experience_details_json','experience_details_json_2']].apply(lambda x: "{}{}".format(x[0][:-1],x[1]), axis=1)
        del tem2['experience_details_json_2']
        tem2['candidate_id'] = tem2['id']
    ...
    def create_talent_pool(self, df, logger):
        ...
        vincere_custom_migration.psycopg2_bulk_insert_tracking(tem2, self.ddbconn, ['owner_id', 'name', 'share_permission', 'insert_timestamp'], 'candidate_group', logger)

    def add_candidate_talent_pool(self, df, logger):
        tem2 = df[['candidate_externalid', 'group_name']].dropna().drop_duplicates()
        if 'insert_timestamp' not in tem2.columns:
            tem2['insert_timestamp'] = datetime.datetime.now()
        tem2 = tem2.merge(self.candidate, left_on='candidate_externalid', right_on='candidate_externalid')
        df_group = pd.read_sql("select id as candidate_group_id, name from candidate_group", self.ddbconn)
        ...
        vincere_custom_migration.psycopg2_bulk_insert_tracking(tem2, self.ddbconn,
                                                               ['candidate_group_id', 'candidate_id', 'insert_timestamp'], 'candidate_group_candidate', logger)
    def update_make_hot(self, df, logger):
        ...
        """
        # prepare position type values
        tem2 = df[['candidate_externalid', 'hot_end_date']]
        # transform data
        tem2 = tem2.merge(self.candidate, on=['candidate_externalid'])
        vincere_custom_migration.psycopg2_bulk_update_tracking(tem2, self.ddbconn, ['hot_end_date'], ['id'],'candidate', logger)
        return tem2