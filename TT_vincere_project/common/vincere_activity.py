...
def transform_activities_temp(df_act, connection_str, logger):
    if len(df_act):
        # add cols if not existed
        df_act['category'] = 'comment'
        if 'company_external_id' not in df_act.columns:
...
def transform_tasks_temp(df_act, connection_str, logger):
    if len(df_act):
        # add cols if not existed
        df_act['category'] = 'task'
        # df_act['type'] = 'candidate'
        if 'company_external_id' not in df_act.columns:
            df_act['company_external_id'] = None
        if 'contact_external_id' not in df_act.columns:
            df_act['contact_external_id'] = None
        ...
        if 'position_external_id' not in df_act.columns:
            df_act['position_external_id'] = None
        # remove trailing and leading spaces from external ids
        df_act['company_external_id'] = df_act.apply(
            lambda x: str(x['company_external_id']).strip() if (x['company_external_id'] != None) and (
                        str(x['company_external_id']) != 'nan') else None, axis=1)
        df_act['contact_external_id'] = df_act.apply(
            lambda x: str(x['contact_external_id']).strip() if (x['contact_external_id'] != None) and (
                        str(x['contact_external_id']) != 'nan') else None, axis=1)
        df_act['candidate_external_id'] = df_act.apply(
            lambda x: str(x['candidate_external_id']).strip() if (x['candidate_external_id'] != None) and (
                        str(x['candidate_external_id']) != 'nan') else None, axis=1)
        df_act['position_external_id'] = df_act.apply(
            lambda x: str(x['position_external_id']).strip() if (x['position_external_id'] != None) and (
                        str(x['position_external_id']) != 'nan') else None, axis=1)

        # sdbconn_engine = sqlalchemy.create_engine(connection_str)
        sdbconn_engine = sqlalchemy.create_engine(connection_str, pool_size=100, max_overflow=200,
                                                  client_encoding='utf8', use_batch_mode=True)

        connection = sdbconn_engine.raw_connection()

        df_vin_cont = pd.read_sql(
            "select id as contact_id, external_id from contact where deleted_timestamp is null", connection)
        df_vin_cand = pd.read_sql(
            "select id as candidate_id, external_id from candidate where deleted_timestamp is null", connection)
        df_vin_comp = pd.read_sql(
            "select id as company_id, external_id from company where deleted_timestamp is null", connection)
        df_vin_posi = pd.read_sql(
            "select id as position_id, external_id from position_description where deleted_timestamp is null",
            connection)
        df_owner = pd.read_sql("select id as user_account_id, email from user_account", connection)
        ...
        df_act['user_account_id'] = df_act['user_account_id'].map(lambda x: x if x else -10)
        df_act = df_act[
            df_act.contact_id.notnull() | df_act.candidate_id.notnull() | df_act.company_id.notnull() | df_act.position_id.notnull()]
        connection.close()
        test = df_act[[
            'company_id',
            'contact_id',
