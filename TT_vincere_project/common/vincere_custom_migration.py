...
def insert_branch(df, ddbconn):
    cols = ['name', 'group_type','insert_timestamp']
    df['group_type'] = 'BRANCH'
    df['insert_timestamp'] = datetime.datetime.now()
    psycopg2_bulk_insert(df, ddbconn, cols, 'team_group')

def insert_brand(df, ddbconn):
    cols = ['name', 'group_type','insert_timestamp']
    df['group_type'] = 'BRAND'
    df['insert_timestamp'] = datetime.datetime.now()
    psycopg2_bulk_insert(df, ddbconn, cols, 'team_group')
...
def insert_bulk_upload_photo_mapping_4_candidate(df, db_conn):
    df_candidate = pd.read_sql("""select id as entity_id, external_id from candidate""", db_conn)
    df['external_id'] = df.apply(lambda x: str(x['external_id']).strip(), axis=1)
    df = df.merge(df_candidate, left_on='external_id', right_on='external_id')
    list_values = []
...
def insert_candidate_documents_company(df, db_conn, conn_param, logger):
    df_company = pd.read_sql("""select id as company_id, external_id from company""", db_conn)
    df['external_id'] = df.apply(lambda x: str(x['external_id']).strip(), axis=1)
    df_temp = df.merge(df_company, left_on='external_id', right_on='external_id')
    if not 'document_types_id' in df_temp:
        df_temp['document_types_id'] = 21
    ...
    df_temp['created'] = pd.to_datetime(df_temp['created'])
    col = ['company_id'
        , 'document_types_id'
        , 'uploaded_filename'
        ,'saved_filename'
        ...
        ,'entity_type']
    tem = df_temp[col]
    load_data_to_vincere(tem, conn_param, 'insert', 'candidate_document', col, '', logger)
    return df_temp


def insert_candidate_documents_company_logo(df, db_conn, conn_param, logger):
    df_company = pd.read_sql("""select id as company_id, external_id from company""", db_conn)
    df['external_id'] = df.apply(lambda x: str(x['external_id']).strip(), axis=1)
    df_temp = df.merge(df_company, left_on='external_id', right_on='external_id')
    df_temp['saved_filename'] = df_temp['file_name']
    ...
    df_temp['insert_timestamp'] = datetime.datetime.now()
    df_temp['trigger_index_update_timestamp'] = df_temp['insert_timestamp']
    df_temp['document_type'] = 'company_logo'
    if not 'created' in df_temp:
        df_temp['created'] = df_temp['insert_timestamp']
    df_temp.loc[(df_temp['created'].isnull()), 'created'] = df_temp['insert_timestamp']
    df_temp['created'] = pd.to_datetime(df_temp['created'])
    ...
    tem = df_temp[col]
    load_data_to_vincere(tem, conn_param, 'insert', 'candidate_document', col, '', logger)
    tem2 = tem[['company_id','saved_filename']]
    tem2['id'] = tem2['company_id']
    tem2['logo_filename'] = tem2['saved_filename']
    psycopg2_bulk_update_tracking(tem2, db_conn, ['logo_filename', ], ['id', ],'company', logger)
    return df_temp

def insert_candidate_documents_contact(df, db_conn, conn_param, logger):
...
def insert_candidate_documents_contact_photo(df, db_conn, conn_param, logger):
    df_contact = pd.read_sql("""select id as contact_id, external_id from contact""", db_conn)
    df['external_id'] = df.apply(lambda x: str(x['external_id']).strip(), axis=1)
    df_temp = df.merge(df_contact, left_on='external_id', right_on='external_id')
    df_temp['saved_filename'] = df_temp['file_name']
    ...
    df_temp['insert_timestamp'] = datetime.datetime.now()
    df_temp['trigger_index_update_timestamp'] = df_temp['insert_timestamp']
    df_temp['document_type'] = 'contact_photo'
    if not 'created' in df_temp:
        df_temp['created'] = df_temp['insert_timestamp']
    df_temp.loc[(df_temp['created'].isnull()), 'created'] = df_temp['insert_timestamp']
    df_temp['created'] = pd.to_datetime(df_temp['created'])
    ...
    tem = df_temp[col]
    load_data_to_vincere(tem, conn_param, 'insert', 'candidate_document', col, '', logger)
    tem2 = tem[['contact_id', 'saved_filename']]
    tem2['id'] = tem2['contact_id']
    tem2['profile_picture_filename'] = tem2['saved_filename']
    psycopg2_bulk_update_tracking(tem2, db_conn, ['profile_picture_filename', ], ['id', ], 'contact', logger)
    return df_temp

def insert_candidate_documents_job(df, db_conn, conn_param, logger):
    df_posdes = pd.read_sql("""select id as position_description_id, external_id from position_description""", db_conn)
    df['external_id'] = df.apply(lambda x: str(x['external_id']).strip(), axis=1)
    df_temp = df.merge(df_posdes, left_on='external_id', right_on='external_id')
    if not 'document_types_id' in df_temp:
        df_temp['document_types_id'] = 19
    df_temp['saved_filename'] = df_temp['file_name']
    df_temp['version_no'] = 1
    ...
    df_temp['customer_portal'] = 0
    df_temp['visible'] = 1
    df_temp['entity_type'] = 'JOB'
    df_temp['insert_timestamp'] = datetime.datetime.now()
    df_temp['trigger_index_update_timestamp'] = df_temp['insert_timestamp']
    df_temp['document_type'] = 'legacy_job_document'
    if not 'created' in df_temp:
        df_temp['created'] = df_temp['insert_timestamp']
    df_temp.loc[(df_temp['created'].isnull()), 'created'] = df_temp['insert_timestamp']
    df_temp['created'] = pd.to_datetime(df_temp['created'])
    col = ['position_description_id'
        , 'document_types_id'
        , 'uploaded_filename'
        ,'saved_filename'
        ,'insert_timestamp'
...
def insert_candidate_photos_candidate(df, db_conn, conn_param, logger):
    ...
    df_temp['insert_timestamp'] = datetime.datetime.now()
    df_temp['trigger_index_update_timestamp'] = df_temp['insert_timestamp']
    df_temp['document_type'] = 'candidate_photo'
    if not 'created' in df_temp:
        df_temp['created'] = df_temp['insert_timestamp']
    df_temp.loc[(df_temp['created'].isnull()), 'created'] = df_temp['insert_timestamp']
    df_temp['created'] = pd.to_datetime(df_temp['created'])
...
def insert_candidate_documents_candidate(df, db_conn, conn_param, logger):
    df_candidate = pd.read_sql("""select id as candidate_id, external_id from candidate""", db_conn)
    df['external_id'] = df.apply(lambda x: str(x['external_id']).strip(), axis=1)
    df_temp = df.merge(df_candidate, left_on='external_id', right_on='external_id')
    if not 'document_types_id' in df_temp:
...
def load_data_to_vincere(df, conn_param, type, tblname, up_cols, wh_cols, logger):
    ...
    :return:
    """
    dfs = vincere_common.df_split_to_listofdfs(df, 500)
    # load to vincere
...
def insert_candidate_gdpr_compliance(df, db_conn, cols):
...
def insert_contact_date_field_values(df, candidate_extid, values_colname, field_key, ddbconn):
    """
    insert value for custom field: text box, multi value text box
    :param df: data frame contain atleast 2 columns: candidate external id col and drop down list values col
    :param candidate_extid: column name of the candidate external id field
...
, position_candidate_id, id
    ...
    cur = destination_db_connection.cursor()
    cur.execute("update position_description set position_sub_type = 1 where position_type = 1 and position_sub_type is null;")
    destination_db_connection.commit()
    cur.execute("update offer set position_sub_type = 1 where position_type = 1 and position_sub_type is null;")
    destination_db_connection.commit()
    cur.close()
...
def inject_contact_source(df, entidy_extid, colname_source, ddbconn):
    """ this function can be run many times without changing result"""
    df.drop_duplicates(inplace=True)
    df['lname'] = df[colname_source].map(lambda x: str(x).strip().lower() if (str(x) != 'nan') and (x != None) else x)
    df[colname_source] = df[colname_source].map(lambda x: str(x).strip() if (str(x) != 'nan') and (x != None) else x)
    ...
    df_cand_source['external_id'] = df_cand_source['external_id'].astype(str)
    # %% modify at 2019-02-15
    df_cand_source = df_cand_source.merge(pd.read_sql("select id, external_id from contact", ddbconn), on='external_id')
    # psycopg2_bulk_update(df_cand_source, ddbconn, ['candidate_source_id', ], ['external_id', ], 'candidate')
    psycopg2_bulk_update(df_cand_source, ddbconn, ['candidate_source_id', ], ['id', ], 'contact')
    # to here

    with ddbconn.cursor() as cur:
        cur.execute("update contact set candidate_source_id = (select id from candidate_source where name='Data Import') where candidate_source_id is null;")
        ddbconn.commit()
...
def insert_industry(ind, colname_ind, sind, colname_sind, ddbconn):
    ...
    clean_industry(ddbconn)  # IF CLEAN INDUSTRIES, GO TO SETTING => GROUPS, TAGS & LOCATIONS => GROUPS TAB => YOUR BRAND => INDUSTRIES => ADD ALL

    # insert new industries_cand (vertical)
    df_vertical = df_ind
    df_vertical['name'] = df_ind[colname_ind]
    df_vertical['insert_timestamp'] = time
    psycopg2_bulk_insert(df_vertical, ddbconn, cols, 'vertical')

    # insert vertical_detail_language: TAKE TIME TO BE EFFECTED
    df_vertical = df_vertical.merge(pd.read_sql('select * from vertical', ddbconn), left_on=['name', 'insert_timestamp'], right_on=['name', 'insert_timestamp'])
    df_vertical['vertical_id'] = df_vertical['id']
    df_vertical['language'] = 'en'
    cols = ['vertical_id', 'language', 'name', 'insert_timestamp']
    psycopg2_bulk_insert(df_vertical, ddbconn, cols, 'vertical_detail_language')
    mapping_industries_to_team_brand(ddbconn)
    if df_sind is not None:
        df_sind = df_sind.merge(pd.read_sql("select id, name from vertical", ddbconn), left_on=colname_ind, right_on='name')
        df_sind['parent_id'] = df_sind['id']
        df_sind['name'] = df_sind[colname_sind]
        df_sind['insert_timestamp'] = time
        cols = ['parent_id', 'name', 'insert_timestamp']
        psycopg2_bulk_insert(df_sind, ddbconn, cols, 'vertical')
    return df_vertical
