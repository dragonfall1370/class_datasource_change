...
def upload_files_to_s3(upload_folder,
                       ...
                       access_key = 'AKIAJUNYY2WD7YF6A53A',
                       secret_key = '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', log=None):
...
class Common():
    ...
    def process_vincere_email(self, df, id_col, email_col):
        ...
        df[email_col] = df.apply(lambda x: ','.join(set(re.findall(regex_pattern_email, x[email_col]))  # set does not allow duplicates
                                                    ), axis=1)  # extract valid email, remove dup email of the same contact, join them by ','
        df[email_col] = df.apply(lambda x: ('No_email_' + str(x[id_col]) + '@email.com') if len(x[email_col]) == 0 else x[email_col], axis=1)  # set fake email for missing values
        ...
        #         if type(e) == 'float':
        #             print('%i_%s' % (i, e))
        df_email['email_cumcount'] = df_email.groupby(df_email[email_col].str.lower()).cumcount() + 1  # group by email case insensitive

        df_email[email_col] = df_email.apply(lambda x: x[email_col] if x['email_cumcount'] == 1 else "%s_%s" % (x['email_cumcount'], x[email_col]), axis=1)  # add prefix for dup contact email
