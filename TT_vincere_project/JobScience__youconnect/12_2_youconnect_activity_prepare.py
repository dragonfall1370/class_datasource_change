...
, i.ts2__Job__c as position_external_id
, i.ts2__Candidate__c as candidate_external_id
, i.ts2__Account__c as company_externalid
, u.Email as owner
, u.FirstName|| ' ' ||u.LastName as  created_by
from ts2__Interview__c i
left join User u on i.ts2__PrimaryRecruiter__c  = u.Id
where ts2__Candidate__c is not null
""", engine_sqlite)
now = datetime.datetime.now()
candidate_interview.fillna({'CreatedDate':now}, inplace=True)
candidate_interview['insert_timestamp'] = pd.to_datetime(candidate_interview['CreatedDate'])
candidate_interview['content'] = '#Migration jobscience interview - ' + candidate_interview['created_by']
candidate_interview[candidate_interview['CreatedDate'].isnull()]
assert False
# %%
...
re1.to_sql(con=engine_sqlite, name='vincere_activity_candidate_2', if_exists='replace', dtype=dtype, index=False)
