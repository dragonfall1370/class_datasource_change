...
# %% primary email
tem = candidate[['candidate_externalid', 'Primary Email']].dropna().drop_duplicates()
tem['primary_email'] = tem['Primary Email']
tem['payslip_email'] = tem['Primary Email']
vcand.update_primary_email(tem, mylog)
vcand.update_payslip_email(tem, mylog)

# %% primary phone
tem = candidate[['candidate_externalid', 'primary_phone']].dropna().drop_duplicates()
tem.loc[tem['candidate_externalid']=='CAND3473']
vcand.update_primary_phone(tem, mylog)

# %% mobile phone
tem = candidate[['candidate_externalid', 'mobile_phone']].dropna().drop_duplicates()
# tem['mobile_phone'] = tem['Mobile']
vcand.update_mobile_phone(tem, mylog)

# %% home phone
tem = candidate[['candidate_externalid', 'home_phone']].dropna().drop_duplicates()
# tem['home_phone'] = tem['Phone']
...
# %% email sub
tem = candidate[['Primary Email']].dropna().drop_duplicates()
tem['email'] = tem['Primary Email']
tem['subscribed'] = 1
vcand.email_subscribe(tem,mylog)

# %% education
tem = candidate[['candidate_externalid','School Name','Degree Name','Qualification']].drop_duplicates()
tem['schoolName'] = tem['School Name']
tem['degreeName'] = tem['Degree Name']
tem['qualification'] = tem['Qualification']
cp9 = vcand.update_education(tem,dest_db, mylog)