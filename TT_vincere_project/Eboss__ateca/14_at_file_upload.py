...
document_path = r'D:\Tony\File\Ateca\sources_adminateca\sources_adminateca'
temp_msg_metadata = vincere_common.get_folder_structure(document_path)

assert False
# assert False
temp_msg_metadata['external_id'] = temp_msg_metadata['root'].apply(lambda x: x.split('\\')[-1])
temp_msg_metadata['external_id'] = temp_msg_metadata['external_id'].apply(lambda x: 'AT'+str(x))
temp_msg_metadata['file_name'] = temp_msg_metadata['alter_file2']
temp_msg_metadata['uploaded_filename'] = temp_msg_metadata['file']

assert False
# %% company
# company_file = vincere_custom_migration.insert_bulk_upload_document_mapping_4_company(temp_msg_metadata_client, ddbconn)
...
# candidate_file = pd.read_csv('candidate_file.csv')
# assert False
# %% upload files to s3
