...
standard_file_upload = '/Users/tungnguyen/MEGAsync/vincere/itrist__exede_recruitment/output/exede/standard_file_upload'
...
jobapp1 = pd.read_csv(os.path.join(standard_file_upload, '7_jobapplication_other.csv'))
jobapp2 = pd.read_csv(os.path.join(standard_file_upload, '7_jobapplication_placement.csv'))
...
, position_candidate_id, id
     from offer where position_candidate_id in (select id from znewtemp_jobapp);
...
position_candidate = pd.read_sql("""
    ...
    from position_candidate where id in (select position_candidate_id from offer where position_candidate_id in (select id from znewtemp_jobapp));
    """, destination_db_connection)
...
position_description = pd.read_sql("""
    ...
    from position_description where id in (select position_description_id from position_candidate where id in ((select id from znewtemp_jobapp)))
    """, destination_db_connection)
...
candidate = pd.read_sql("""
    ...
    from candidate where id in (select candidate_id from position_candidate where id in (select position_candidate_id from offer where position_candidate_id in (select id from znewtemp_jobapp)))
    """, destination_db_connection)
...
company = pd.read_sql("""
    ...
    select company_id from position_description where id in (select position_description_id from position_candidate where id in (select position_candidate_id from offer where position_candidate_id in (select id from znewtemp_jobapp)))
    )
    """, destination_db_connection)
...
contact = pd.read_sql("""
        ...
        select contact_id from position_description where id in (select position_description_id from position_candidate where id in (select position_candidate_id from offer where position_candidate_id in (select id from znewtemp_jobapp)))
        )
        """, destination_db_connection)
...
cur.execute(r"""
    ...
    where o.position_candidate_id in (select id from znewtemp_jobapp)
    ) as data (id, val)
