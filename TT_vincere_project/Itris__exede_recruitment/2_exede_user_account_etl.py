...
""", ddbconn)
duser_account.email = duser_account.email.map(lambda x: x.lower())
...
;
""", engine_mssql)
suser_account.Email = suser_account.Email.fillna('')
suser_account.loc[suser_account.Email=='', 'Email'] = suser_account.loc[suser_account.Email == ''].apply(lambda x: '%s.%s@christy_media.com' % (x.FirstName, x.LastName), axis=1)
suser_account['email'] = suser_account['Email']
suser_account.email = suser_account.email.map(lambda x: x.lower())
suser_account['rn'] = suser_account.groupby('email').cumcount()
suser_account.loc[suser_account.rn>0, 'email'] = suser_account.loc[suser_account.rn>0][['rn', 'email']].apply(lambda x: '_'.join(str(e) for e in x), axis=1)
