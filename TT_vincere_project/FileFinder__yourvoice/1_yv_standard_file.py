...
contact = pd.read_sql("""
select p.idperson, p.firstname, p.lastname, p.EmailWork, cont.idcompany, u.useremail, ps.Value as status
from personx p
join
...
where cont.rn = 1
and p.isdeleted = '0'
and status in ('CLIENT - CLIENT','Prospect - commercial')
""", engine_sqlite)
...
join (select p.idperson, p.firstname, p.lastname, p.EmailWork, cont.idcompany, u.useremail, ps.Value as status
from personx p
join
(select cp.idperson, cp.idcompany,
       ROW_NUMBER() OVER(PARTITION BY cp.idperson
...
where cont.rn = 1
and p.isdeleted = '0'
and status in ('CLIENT - CLIENT','Prospect - commercial')) comp_per on ac.idperson = comp_per.idperson) cont
...
# %% check
# place = pd.read_csv(os.path.join(standard_file_upload, '7_jobapplication_placement.csv'))
# tem = place.merge(job, left_on='application-positionExternalId', right_on='idAssignment')
# tem = tem.merge(candidate, left_on='application-candidateExternalId', right_on='idperson')
# tem2 = tem[['AssignmentTitle','firstname','lastname']]
# tem2.to_csv('placement1.csv',index=False)