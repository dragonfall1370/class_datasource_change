...
jobapp2 = pd.read_sql("""
select cand.idperson as candidate_externalid, ContextEntityId as job_externalid, ale.CreatedOn
 from activitylogentity ale
 join (select px.idperson
      , ROW_NUMBER() OVER(PARTITION BY c.idperson ORDER BY px.createdon DESC) rn
from candidate c
join (select personx.idperson, personx.createdon from personx where isdeleted = '0') px on c.idperson = px.idperson) cand on cand.idperson = ale.idPerson
...
where alct.value = 'Assignment Candidate' and cp.value = 'Placed'
""", engine_sqlite)
jobapp2['application_stage'] = 'Placed'
jobapp = pd.concat([jobapp2,jobapp1])
...
tem2 = pd.read_csv('jobapp_stage_mapping.csv',encoding='latin1')
tem2['matcher'] = tem2['FileFinder'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', x)).lower())
...
tem4['timestamp'] = tem4['CreatedOn'].astype(str)
tem4['vincere Stage'] = tem4['Vincere Stage'].astype(str)

# %%
tem5 = tem4[['candidate_externalid', 'job_externalid', 'timestamp', 'vincere Stage', 'Vincere Sub Status']]
...
jobapp_result = ja.process_jobapp_v3(tem5, 'Vincere Sub Status')
