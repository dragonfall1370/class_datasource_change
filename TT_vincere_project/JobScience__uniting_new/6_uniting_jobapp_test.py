...
from Applicaion_new c
""", engine_sqlite)
# job_app = job_app.drop_duplicates()
# job_app['application_stage'].unique()
assert False
# %% mapping stage
...
df_jobapplication_placement.to_csv(os.path.join(standard_file_upload, 'jobapplication_placement_new_2.csv'), index=False)
df_jobapplication_other.to_csv(os.path.join(standard_file_upload, 'jobapplication_other_new_2.csv'), index=False)
