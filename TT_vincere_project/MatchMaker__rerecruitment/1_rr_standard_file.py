...
# %% production
company = pd.read_sql("""
with client_owner as (
        select clo.cli_no,STRING_AGG(lower(c.con_email),',') as con_email
        from dbo.cli_owner clo
        left join dbo.consultant c on c.con_initials=clo.cli_con
        group by cli_no
)
select cl.cli_no, cli_name, con_email
from dbo.client cl
left join client_owner clo on clo.cli_no=cl.cli_no
""", engine_mssql)
