...
document_path = r'D:\Tony\File\MatchMaker\Review\physical_files'

# %% extract data
cand = pd.read_sql("""
select CONCAT('',peo_no) as candidate_externalid
           ,att_no
           , att_filename, att_folder
from peo_attach
where att_folder in ('Pack','CVs','General')
""", engine_mssql)
cand['matcher'] = cand['att_no'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', str(x))).lower())
temp_msg_metadata = vincere_common.get_folder_structure(document_path)
temp_msg_metadata['matcher'] = temp_msg_metadata['file'].apply(lambda x: x.split('.')[0])
temp_msg_metadata['matcher'] = temp_msg_metadata['matcher'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', str(x))).lower())
assert False
...
temp_msg_metadata_cand['uploaded_filename'] = temp_msg_metadata_cand['att_filename']
temp_msg_metadata_cand.loc[temp_msg_metadata_cand['att_folder']=='CVs', 'primary_document'] =1
