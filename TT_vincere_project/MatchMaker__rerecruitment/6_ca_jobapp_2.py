...
from Placement
where Placement_Code in ('PL41909'
,'PL41910'
,'PL44078'
,'PL43810'
,'PL44075'
...
""", engine_mssql)
jobapp['job_externalid'] = 'BK'+jobapp['job_externalid']
jobapp['application_stage'] = 'Placed'
jobapp['Created_DTTM_1'] = jobapp['Created_DTTM'].apply(lambda x: x[0:10] if x else x)
jobapp['Created_DTTM_2'] = jobapp['Created_DTTM'].apply(lambda x: x[11:19] if x else x)
jobapp['Created_DTTM'] = jobapp['Created_DTTM_1'] +' '+jobapp['Created_DTTM_2']
jobapp['Created_DTTM'] = jobapp['Created_DTTM'].apply(lambda x: x.replace('.',':') if x else x)
jobapp = jobapp.drop(['Created_DTTM_1','Created_DTTM_2'],axis=1)
assert False
# %% mapping stage
...
df_jobapplication_placement.to_csv(os.path.join(standard_file_upload, '7_jobapplication_placement_live.csv'), index=False)
df_jobapplication_other.to_csv(os.path.join(standard_file_upload, '7_jobapplication_other_live.csv'), index=False)