...
# %% pending
activity = pd.read_sql("""select * from activities""", engine_sqlite)
activity['position_external_id'] = activity['job'].apply(lambda x: str(x).split('.')[0] if x else x)
activity['company_external_id'] = activity['company'].apply(lambda x: str(x).split('.')[0] if x else x)
activity['contact_external_id'] = activity['person'].apply(lambda x: str(x).split('.')[0] if x else x)
activity['candidate_external_id'] = activity['person'].apply(lambda x: str(x).split('.')[0] if x else x)
assert False
# %%
activity['insert_timestamp'] = pd.to_datetime(activity['created'], format='%Y/%m/%d %H:%M:%S')
activity['owner'] = activity['createdBy']
activity['content'] = activity[['type','notes']]\
    .apply(lambda x: '\n'.join([': '.join([i for i in e if i]) for e in zip(['Type','Notes'], x) if e[1]]), axis=1)
