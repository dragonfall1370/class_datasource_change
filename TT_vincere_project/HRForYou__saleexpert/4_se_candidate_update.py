...
cur_emp.loc[cur_emp['current_job_title'].str.contains('\x01')]
a=cur_emp[28984:28985]
a.to_csv('jobtile.csv')
...
select concat('KL',u.user_id) as user_id,
       ...
       p.status1,
       p.anmerkungen,
       p.erfahrung_fuehrung_mitarbeiter,
       p.hobbies,
       p.profil_text_bewerber as ich_suche,
       p.eigener_pkw
from user_data u
left join user_profil_stellensuchender p on u.user_id = p.user_id
...
note1['eigener_pkw'] = note1['eigener_pkw'].apply(lambda x: 'Ja' if int(x) == 1 else '')

# assert False
note1['user_id'] = note1['user_id'].astype(str)
note1 = note1.fillna('')
note1['note'] = note1[['titel', 'geburtsort', 'anmerkungen',
                     'hobbies', 'ich_suche',
