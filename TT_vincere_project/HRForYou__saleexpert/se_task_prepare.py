...
# %%
cand = pd.read_sql("""
select ativities.*, l.reg_mail as owner
from
...
left join user_login l on ativities.user_id = l.user_id
...
# reminder = reminder.loc[reminder.faellig_am != '0000-00-00 00:00:00']
# reminder['insert_timestamp'] = pd.to_datetime(reminder['faellig_am'])
...
# reminder['cat_type'] = reminder['cat_type'].where(reminder['cat_type'].notnull(), 'Email')
...
cand = cand.loc[cand.faellig_am != '0000-00-00 00:00:00']
cand['insert_timestamp'] = pd.to_datetime(cand['faellig_am'])

cand['description'] = cand['description'].apply(lambda x: html_to_text(x))
cand['title'] = cand['titel'].where(cand['titel'].notnull(), 'Email')
cand['subject'] = cand['subject'].where(cand['subject'] != '', None)
cand['description'] = cand['description'].where(cand['description'] != '', None)
cand['faellig_am'] = cand['faellig_am'].astype(str)
cand['content'] = cand[['title','titel','subject','description', 'faellig_am']] \
    .apply(lambda x: '\n\n'.join([': '.join(e) for e in zip(['Art','Titel', 'Betreff', 'Beschreibung', 'Erstellt am'], x) if e[1]]), axis=1)
# %% load to temp db
...
re2 = vincere_activity.transform_tasks_temp(cand, conn_str_ddb, mylog)
