...
# %% job type
tem = candidate[['candidate_externalid', 'peo_division']].dropna().drop_duplicates()
tem['peo_division'].unique()
tem.loc[tem['peo_division']=='Healthcare', 'desired_job_type'] = 'temporary'
tem.loc[tem['peo_division']=='Permanent', 'desired_job_type'] = 'permanent'

tem2 = tem[['candidate_externalid', 'desired_job_type']].dropna().drop_duplicates()
cp = vcand.update_desired_job_type(tem2, mylog)
...
note['note'] = note[['candidate_externalid']].apply(lambda x: '\n'.join([': '.join(e) for e in zip(['Person ID'], x) if e[1]]), axis=1)
cp7 = vcand.update_note2(note, dest_db, mylog)
