...
select distinct position_description_id from position_candidate where id in (select position_candidate_id from invoice))""", connection)
cont_place['board'] = 4
cont_place['status'] = 1
...
select distinct position_description_id from position_candidate where id not in (select position_candidate_id from invoice))""", connection)
...
cont_no_job = pd.read_sql("""select id from contact where id not in (select distinct contact_id from position_description)""", connection)
cont_no_job['board'] = 2
