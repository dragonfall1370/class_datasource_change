...
type = pd.read_sql("""select CONCAT('',peo_no) as contact_externalid, description from peo_key pk
join (
select keyword_no, description
from diction d
left join class_ky c on d.class_no = c.class_no
where class_name in ('Contract type')) k on pk.keyword_no = k.keyword_no""",engine_mssql)
type = type.groupby('contact_externalid')['description'].apply(', '.join).reset_index()
note = note.merge(type,on='contact_externalid',how='left')
note = note.where(note.notnull(),None)
note['note'] = note[['peo_branch','peo_division','peo_cnt_comm','description']]\
    .apply(lambda x: '\n'.join([': '.join(e) for e in zip(['Office','Sector','Comments','Contract type'], x) if e[1]]), axis=1)
