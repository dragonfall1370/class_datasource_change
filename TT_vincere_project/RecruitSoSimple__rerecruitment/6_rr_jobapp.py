...
select concat('RSS',Candidate) as candidate_externalid
     , concat('RSS',Vacancy) as job_externalid, Status as application_stage
     , [Added Date], History
from Vacancies_Candidates
""", engine_mssql)
jobapp['stage'] = jobapp['History'].apply(lambda x: x.split('\n')[-1])
jobapp['date'] = jobapp['stage'].apply(lambda x: x.split(' ')[0])
jobapp['date'] = pd.to_datetime(jobapp['date'], format='%d/%m/%Y')
jobapp.loc[jobapp['candidate_externalid']=='RSS682']
assert False
# %% mapping stage
...
tem2['matcher'] = tem2['Status'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', x)).lower())
