...
assert False
# %%
log['content'] = log[['Type','Subject','Body']]\
    .apply(lambda x: '\n'.join([': '.join([i for i in e if i]) for e in zip(['Type','Subject','Content'], x) if e[1]]), axis=1)
log['insert_timestamp'] = pd.to_datetime(log['Date'], format='%d/%m/%Y %H:%M:%S')
log['matcher'] = log['Consultant'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', x)).lower() if x else x)
log = log.merge(user, on='matcher', how='left')
log = log.where(log.notnull(), None)
log['owner'] = log['UserEmail']
log['company_external_id'] = log['Clients'].apply(lambda x: 'RSS'+str(x) if x else x)
log['contact_external_id'] = log['Contacts'].apply(lambda x: 'RSS'+str(x) if x else x)
log['position_external_id'] = log['Vacancies'].apply(lambda x: 'RSS'+str(x) if x else x)
log['candidate_external_id'] = log['Candidates'].apply(lambda x: 'RSS'+str(x) if x else x)
...
re1.to_sql(con=engine_sqlite, name='vincere_activity_rss_prod', if_exists='replace', dtype=dtype, index=False)
