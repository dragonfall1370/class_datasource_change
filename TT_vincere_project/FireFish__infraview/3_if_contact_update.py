...
# %% owner
owner = pd.read_sql("""
select c.ID as contact_externalid, Username as email
from Person p
join Candidate c on p.ID = c.ID
left join SystemUser u on u.ID = p.CreatedUserID
where OwnerPersonID is null
""", engine_sqlite)
owner['contact_externalid'] = owner['contact_externalid'].apply(lambda x: str(x) if x else x)
owner = owner.loc[owner['email']!='sysadmin']
