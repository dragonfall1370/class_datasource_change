...
select p.ID as candidate_externalid
     ...
     , c.PersonalEMail
     , o.WorkEMail as owner, n.Value as notice, p.DirectDial, t.Value as salutation, p.ContactLastActionDate
from Person p
...
from Person p
join Candidate c on p.ID = c.ID
left join (select PersonID, EmailMarketing, SmsMarketing from PersonMarketing) mar on mar.PersonID = p.ID
...
note.loc[note['EmailMarketing'] == '0', 'EmailMarketing'] = 'No'
note.loc[note['EmailMarketing'] == '1', 'EmailMarketing'] = 'Yes'
note.loc[note['SmsMarketing'] == '0', 'SmsMarketing'] = 'No'
note.loc[note['SmsMarketing'] == '1', 'SmsMarketing'] = 'Yes'
note['note'] = note[['candidate_externalid', 'RecruiterSummary', 'EmailMarketing', 'SmsMarketing', 'permstatus']]\
    .apply(lambda x: '\n'.join([': '.join(e) for e in zip(['Infraview ID', 'Recruiter Summary', 'Email Marketing', 'Sms Marketing', 'Permanent Status'], x) if e[1]]), axis=1)
cp11 = vcand.update_note2(note, dest_db, mylog)

# %% last activity date
tem = candidate[['candidate_externalid', 'ContactLastActionDate']].dropna()
...
tem['currency'] = 'pound'
vcand.update_currency_type(tem, mylog)
...
quali_1 = quali_1.rename(columns={'quali':'experience'})
...
vcand.update_education_summary(tem, mylog)
...
vcand.update_active(tem2,mylog)
...
tem['notice'].unique()
tem.loc[tem['notice']=='4 Weeks', 'notice_period'] = 28
tem.loc[tem['notice']=='3 Months', 'notice_period'] = 90
