...
select concat('', csv.Person_Reference) as candidate_externalid
     , nullif(trim(cmv.Title),'') as Title
...
tem['gender_title'] = tem['gender_title'].str.strip()
...
cp = vcand.update_gender_title(tem, mylog)
...
candidate_info['Detail'].unique()
# %% gender
tem = candidate_info.query('Detail == "Gender"')
tem['Value'].unique()
tem.loc[tem['Value']=='Male', 'male'] = 1
tem.loc[tem['Value']=='Female', 'male'] = 0
tem2 = tem[['candidate_externalid','male']].dropna()
tem2['male'] = tem2['male'].astype(int)
cp = vcand.update_gender(tem2, mylog)

# %% current employer
cur_emp = candidate_info.query('Detail == "Job Title"')
cur_emp['current_employer'] = ''
cur_emp['current_job_title'] = cur_emp['Value'].str.strip()
...
select concat('', al.Reference) as candidate_externalid,nullif(trim(agroup),'') as agroup,nullif(trim(code),'') as code, Reference_Type from Analysis_Lookup al
...
where Reference_Type = 1""", engine_mssql)
analysis = analysis.drop_duplicates()
analysis = analysis.dropna()
analysis['analysis'] = analysis[['agroup', 'code']].apply(lambda x: ': '.join([e for e in x if e]), axis=1)
analysis = analysis.groupby('candidate_externalid')['analysis'].apply(lambda x: '\n'.join(x)).reset_index()
analysis = analysis.drop_duplicates()
analysis['analysis'] = '\n---Analysis---\n'+analysis['analysis']

ref = pd.read_sql("""
select concat('', Entity_Reference) as candidate_externalid, nullif(trim(Reference),'') as Reference from Reference_Codes where Entity_Type = 1""", engine_mssql)
ref = ref.dropna()
ref = ref.drop_duplicates()
ref = ref.groupby('candidate_externalid')['Reference'].apply(lambda x: ', '.join(x)).reset_index()
ref = ref.drop_duplicates()
...
tem1 = candidate_info[['candidate_externalid','Value','Detail']].query('Detail == "Candidate rating"').rename(columns={'Value':'Candidate rating'})
tem1 = tem1.groupby('candidate_externalid')['Candidate rating'].apply(lambda x: '\n'.join(x)).reset_index()
tem1 = tem1.drop_duplicates()

note4 = pd.read_sql("""
select concat('', kl.Reference) as candidate_externalid, keyword
...
note4 = note4.groupby('candidate_externalid')['keyword'].apply(lambda x: ', '.join(x)).reset_index()
...
note = ref.merge(analysis, on='candidate_externalid', how='left')
note = note.merge(note2, on='candidate_externalid', how='left')
note = note.merge(tem1, on='candidate_externalid', how='left')
note = note.merge(note4, on='candidate_externalid', how='left')
...
note['note'] = note[['Reference', 'Sort_Code','Account_Number','Account_Name', 'Building_Society_Number','analysis','keyword','Candidate rating']]\
    .apply(lambda x: '\n'.join([': '.join(e) for e in zip(['Reference', 'Sort Code','Account Number','Account Name', 'Building Society Number','','','Candidate Rating'], x) if e[1]]), axis=1)
