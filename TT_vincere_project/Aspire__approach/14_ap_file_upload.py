...
document_path = r'D:\Tony\File\approach'

# %% extract data
doc = pd.read_sql("""
select concat(Document_name,'.',d.Document_Type) as readable_filename, concat(d.Reference,'.',d.Document_Type) as file_name, Date_Added, dl.Reference,Document_name
from Documents d
...
temp_msg_metadata['uploaded_filename'] = temp_msg_metadata['Document_name']

assert False
# %% company
company_file = vincere_custom_migration.insert_candidate_documents_company(temp_msg_metadata, ddbconn, dest_db, mylog)
company_file = company_file.drop_duplicates()

# %% contact
contact_file = vincere_custom_migration.insert_candidate_documents_contact(temp_msg_metadata, ddbconn, dest_db, mylog)
contact_file = contact_file.drop_duplicates()

# %% job
job_file = vincere_custom_migration.insert_candidate_documents_job(temp_msg_metadata, ddbconn, dest_db, mylog)
