...
document_path = r'D:\Tony\Energize_file\EN_PROD\Files_PROD'
# %% extract data
file = pd.read_sql("""
select Id, Filename, DateAttached, Ref from Attachments where Filename is not null
""", engine_mssql)
file.loc[file['Id']=='391838-6909-1569']
temp_msg_metadata = vincere_common.get_folder_structure(document_path)
# assert False
file['matcher'] = file['Ref'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', x)).lower())
...
vindoc = vindoc[['id', 'uploaded_filename']].merge(temp_msg_metadata[['alter_file2', 'Filename']], left_on='uploaded_filename', right_on='alter_file2', how='left')
