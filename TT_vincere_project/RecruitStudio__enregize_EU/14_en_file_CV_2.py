...
document_path = r'D:\Tony\Energize_file\EN_PROD\TXT'
# %% extract data
file = pd.read_sql("""
select Id, Filename, DateAttached, Ref from Attachments where Filename is not null and Ref like '%.txt'
""", engine_mssql)
temp_msg_metadata = vincere_common.get_folder_structure(document_path)
file['matcher'] = file['Filename'].apply(lambda x: x[:-3]+'rtf')
assert False
file['matcher'] = file['matcher'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', x)).lower())
# temp_msg_metadata['matcher'] = temp_msg_metadata['file_fullpath'].apply(lambda x: x.split('Files_PROD')[1])
...
temp_msg_metadata['external_id'] = temp_msg_metadata['Id']
temp_msg_metadata['external_id'] = 'EUK'+temp_msg_metadata['external_id']
temp_msg_metadata = temp_msg_metadata.drop_duplicates()
temp_msg_metadata.loc[temp_msg_metadata['external_id']=='EUK586990-5646-1394']
assert False
# %% company
...
# company_file.loc[company_file['external_id']=='EUK786234-8214-10349']
# %% contact
...
contact_file.loc[contact_file['entity_id']=='EUK586990-5646-1394']
...
# candidate_file.loc[candidate_file['external_id']=='EUK391838-6909-1569']
# assert False
...
vindoc = vindoc[['id', 'uploaded_filename']].merge(temp_msg_metadata[['alter_file2', 'Filename']], left_on='uploaded_filename', right_on='alter_file2')
...
vindoc['uploaded_filename'] = vindoc['uploaded_filename'].apply(lambda x: x[:-3]+'rtf')
vincere_custom_migration.load_data_to_vincere(vindoc, dest_db, 'update', 'candidate_document', ['uploaded_filename'], ['id'], mylog)


tem = pd.concat([contact_file,job_file,candidate_file])
tem.to_csv('temp_msg_metadata.csv')