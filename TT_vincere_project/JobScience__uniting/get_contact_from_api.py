...
# %% connect db
cont = pd.read_csv('contact_add.csv')

df = []
def get_data(id):
    url = "https://unitingambition.my.salesforce.com/services/data/v47.0/sobjects/Contact/"+id
...
for index, row in cont.iterrows():
    print(row['ts2__Contact__c'])
    print(index)
    get_data(row['ts2__Contact__c'])

cont_add = pd.concat(df)
cont_add.to_csv('Contact_new.csv')
