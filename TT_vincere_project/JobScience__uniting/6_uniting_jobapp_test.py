...
def process_jobapp(job_app, file_name):
    # %% mapping stage
    ...
    # %%
    print('process map only: ' + file_name + ' ' + '------------------------------')
    ...
    # %%
    print('process mapping: ' + file_name + ' ' + '------------------------------')
    ...
    print('process to csv: ' + file_name + ' ' + '------------------------------')
    ...
    df_jobapplication_placement['application-actionedDate'] = df_jobapplication_placement[
        'application-actionedDate'].map(lambda x: str(x)[:10])
    df_jobapplication_other['application-actionedDate'] = df_jobapplication_other['application-actionedDate'].map(
        lambda x: str(x)[:10])

    df_jobapplication_placement.to_csv(
        os.path.join(standard_file_upload, '7_jobapplication_placement' + '_' + str(file_name) + '.csv'), index=False)
    df_jobapplication_other.to_csv(
        os.path.join(standard_file_upload, '7_jobapplication_other' + '_' + str(file_name) + '.csv'), index=False)


split_df_to_chunks1 = split_dataframe_to_chunks(job_application, 10000)
print('total ' + str(len(split_df_to_chunks1)))
for idx, val in enumerate(split_df_to_chunks1):
    print(idx)
    # process_jobapp(val, str(idx))
    pool.add_task(process_jobapp, val, str(idx))
