...
jobapp = pd.read_sql("""
select concat('',td.Entity_Reference) as job_externalid
     , concat('',td.Person_Reference) as candidate_externalid
     , Description as application_stage
     , Met_Date
     , Rejected_Date
from Tracker_Detail td
left join Tracker_Stage ts on td.Stage_Reference = ts.Reference
left join Tracker_Row tr on tr.Entity_Reference = td.Entity_Reference and tr.Person_Reference = td.Person_Reference
""", engine_mssql)
...
tem2['matcher'] = tem2['Aspire Stage'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', x)).lower())
...
tem4['timestamp'] = tem4['Met_Date'].astype(str)
