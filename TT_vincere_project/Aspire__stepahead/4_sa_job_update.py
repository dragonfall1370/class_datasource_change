...
# %% note
note6 = pd.read_sql("""
Select nullif(convert(nvarchar(max),an.Notes),'') as Notes
, concat('', dl.Reference) as company_externalid
from Notes an
...
tem = job[['job_externalid','Salary_Details', 'Job_Notes', 'Email_Address', 'Main_Telephone','company_externalid']]
tem['Other Contacts'] = 'No Data Row Found'
tem['Job Title'] = 'No Data Row Found'
tem['Start Date'] = 'No Data Row Found'
tem['Duration'] = 'No Data Row Found'
tem['Salary Amount From'] = 'No Data Row Found'
tem['Salary Amount To'] = 'No Data Row Found'
tem['Benefits'] = 'No Data Row Found'
tem = tem.merge(note6,on='company_externalid',how='left')
tem = tem.fillna('')
tem['note'] = tem[['Salary_Details', 'Job_Notes', 'Email_Address', 'Main_Telephone','Other Contacts','Job Title','Start Date','Duration','Salary Amount From','Salary Amount To','Benefits','Notes']]\
    .apply(lambda x: '\n'.join([': '.join(e) for e in zip(['Salary Details','Notes', 'Email', 'Phone','Other Contacts','Job Title','Start Date','Duration','Salary Amount From','Salary Amount To','Benefits','Client Notes'], x) if e[1]]), axis=1)
