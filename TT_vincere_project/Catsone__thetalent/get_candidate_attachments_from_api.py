...
def get_data(index):
    url = "https://api.catsone.com/v3/candidates/"+str(index)+"/attachments"
    ...
    if '_embedded' in json:
        print('True')
        df.append(json_normalize(json['_embedded']['attachments']))
    else:
        print('False')
assert False
...
df_to_csv.to_csv('candidates_attachments.csv', index=False)