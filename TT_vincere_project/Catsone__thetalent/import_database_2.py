...
csv_path = r'C:\Users\tony\Desktop\rytons\the talent\thetalent\thetalent_db\New folder'
...
for index, row in temp_msg_metadata.iterrows():
   ...
   for idx, val in enumerate(split_df_to_chunks):
      print(idx)
      if row['file_fullpath'] == 'C:\\Users\\tony\\Desktop\\rytons\\the talent\\thetalent\\thetalent_db\\jobs.csv':
         val['contact_id'] = val['contact_id'].apply(lambda x: str(x).split('.')[0] if x else x)
         val['recruiter_id'] = val['recruiter_id'].apply(lambda x: str(x).split('.')[0] if x else x)

      if row['file_fullpath'] == 'C:\\Users\\tony\\Desktop\\rytons\\the talent\\thetalent\\thetalent_db\\candidates.csv':
         val['owner_id'] = val['owner_id'].apply(lambda x: str(x).split('.')[0] if x else x)
