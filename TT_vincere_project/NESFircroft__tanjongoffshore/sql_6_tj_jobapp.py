...
cvsent = pd.read_sql("""
select CandidateID, EnquiryID, c.CreatedDate, cs.Description as application_stage
from CVsSent c
left join CVSentStatus cs on c.StatusID = cs.StatusID
where CandidateID is not null and EnquiryID is not null
""", engine_mssql)
cvsent['candidate_externalid'] = 'FC'+cvsent['CandidateID'].astype(str)
cvsent['job_externalid'] = 'FC'+cvsent['EnquiryID'].astype(str)

interview = pd.read_sql("""
select CandidateID, EnquiryID, i.CreatedDate, ins.Description as application_stage
from Interviews i
left join InterviewStatus ins on i.StatusID = ins.StatusID
where CandidateID is not null and EnquiryID is not null
""", engine_mssql)
interview['candidate_externalid'] = 'FC'+interview['CandidateID'].astype(str)
interview['job_externalid'] = 'FC'+interview['EnquiryID'].astype(str)

place = pd.read_sql("""
select CandidateID, EnquiryID, p.CreatedDate, ps.Status as application_stage
from Placements2 p
...
jobapp = pd.concat([cvsent, interview, place])
...
tem2['matcher'] = tem2['Fircroft Stage'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', x)).lower())
...
# %%
tem5 = tem4[['candidate_externalid', 'job_externalid', 'timestamp', 'vincere Stage','SUB STATUS (Require 1 Custom Script)']]
...
jobapp_result = ja.process_jobapp_v3(tem5,'SUB STATUS (Require 1 Custom Script)')
...
df_jobapplication_placement.to_csv(os.path.join(standard_file_upload, '7_jobapplication_placement_tj.csv'), index=False)
df_jobapplication_other.to_csv(os.path.join(standard_file_upload, '7_jobapplication_other_tj.csv'), index=False)