...
# %% sub status create
subss = pd.concat([
    pd.read_csv(os.path.join(standard_file_upload, '7_jobapplication_other_tj.csv')),
    pd.read_csv(os.path.join(standard_file_upload, '7_jobapplication_placement_tj.csv')),
    ])
subss['Comments'] = subss['SUB STATUS (Require 1 Custom Script)']
