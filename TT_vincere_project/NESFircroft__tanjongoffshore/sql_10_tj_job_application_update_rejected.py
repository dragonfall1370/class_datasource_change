...
# load placement
job_app_contains_rejected = pd.read_csv(os.path.join(standard_file_upload, '7_jobapplication_other_tj.csv'))
...
vjobapp = pd.read_sql("""
select rejected_date, pc.id, pd.external_id as jobextid, c.external_id as candidateextid, associated_date
...
job_app_contains_rejected['rejected_date'] = pd.to_datetime(job_app_contains_rejected['associated_date'])
