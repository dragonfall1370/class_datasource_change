...
select CandidateID, EnquiryID, t.ContactID, Comment, t.DateCreated, t.CreatedBy, ts.Description, ClientID
from Tracking t
left join TrackingStatus ts on t.Status = ts.StatusID
left join ClientContacts CC3 on t.ContactID = CC3.ContactID
""", engine_mssql)
activity = activity.drop_duplicates()
activity = activity.where(activity.notnull(),None)
activity['candidate_external_id'] = activity['CandidateID'].apply(lambda x: 'FC'+str(x).split('.')[0] if x else x)
activity['job_external_id'] = activity['EnquiryID'].apply(lambda x: 'FC'+str(x).split('.')[0] if x else x)
...
assert False
# %%
activity['insert_timestamp'] = pd.to_datetime(activity['DateCreated'], format='%Y/%m/%d %H:%M:%S')
activity = activity.merge(user, on='matcher', how='left')
activity = activity.where(activity.notnull(),None)
activity['owner'] = activity['Email Address']


activity['content'] = activity[['Description','Comment']]\
    .apply(lambda x: '\n'.join([': '.join([i for i in e if i]) for e in zip(['Status','Comment'], x) if e[1]]), axis=1)
...
re1.to_sql(con=engine_sqlite, name='vincere_activity_prod_sql', if_exists='replace', dtype=dtype, index=False)
