...
document_path = r'D:\Tony\File\tanjong'

# %% extract data
cand = pd.read_sql("""
select RMSCANDIDATEID, CV from Candidate
""", engine_sqlite)
cand['RMSCANDIDATEID'] = cand['RMSCANDIDATEID'].astype(str)
...
""", engine_mssql)
cand_sql['CandidateID'] = cand_sql['CandidateID'].astype(str)
cand_sql['AttachmentID'] = cand_sql['AttachmentID'].astype(str)
cand_sql = cand_sql.loc[cand_sql['CandidateID'].isin(cand['RMSCANDIDATEID'])]
# assert False
...
temp_msg_metadata_cand = temp_msg_metadata.merge(cand_sql, on='matcher')
