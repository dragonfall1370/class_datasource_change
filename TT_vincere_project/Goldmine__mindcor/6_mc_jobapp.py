...
jobapp = pd.read_sql("""
select OPID as job_externalid, ACCOUNTNO as candidate_externalid, name as application_stage from OPMGR where nullif(PRODUCTNAME, '') is null and nullif(NAME, '') is not null and RECTYPE <> 'O  '
""", engine_mssql)

jobapp_date = pd.read_sql("""
select OPID as job_externalid, STARTDATE from OPMGR where nullif(PRODUCTNAME, '') is not null and STARTDATE is not null and RECTYPE <> 'O  '
""", engine_mssql)

jobapp = jobapp.merge(jobapp_date, on='job_externalid', how='left')
jobapp.loc[jobapp['STARTDATE'].notnull()]
jobapp.loc[jobapp['job_externalid']=='EO5HFC8%&]6ZG2)']
# tem1 = jobapp.loc[jobapp['Rejected_Date'].notnull()]
# tem1['application_stage'] = tem1['application_stage']+'Rejected'
# tem2 = jobapp.loc[jobapp['Rejected_Date'].isnull()]
# jobapp = pd.concat([tem1,tem2])
assert False
# %% mapping stage
...
tem2 = pd.read_csv('jobapp_stage_mapping.csv',encoding='cp1252')
tem2['matcher'] = tem2['Goldmine'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', x)).lower())

tem3 = tem1.merge(tem2, on='matcher', suffixes=['', '_y'], how='outer', indicator=True)
tem3.loc[tem3['_merge']=='left_only']
tem3.loc[tem3['_merge']=='left_only', 'Vincere Stage'] = 'SHORTLISTED'
...
tem4['timestamp'] = tem4['STARTDATE'].astype(str)
