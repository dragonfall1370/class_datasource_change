...
select ClientID as company_external_id
     ...
     , nullif(convert(varchar,ClientNoteDescription),'') as ClientNoteDescription
...
select pn.PersonID as contact_external_id
     ...
     , nullif(convert(varchar,PersonNoteDescription),'') as PersonNoteDescription
     , at.DisplayName as action_type
     , nullif(convert(varchar,EffectiveDate),'') as EffectiveDate
     , a.AMEmail as owner
from PersonNote pn
...
select JobID as position_external_id
     ...
     , nullif(convert(varchar,JobNoteDescription),'') as JobNoteDescription
     , at.DisplayName as action_type
     , nullif(convert(varchar,EffectiveDate),'') as EffectiveDate
     , a.AMEmail as owner
from JobNote jn
...
select CandidateID as candidate_external_id
     ...
     , InternalInterviewDescription
     , InternalInterviewPlace
     , a.AMEmail as owner
...
select CompanyID as company_external_id
     ...
     , Subject, CreatedDate
     , nullif(convert(varchar,mt.Description),'') as Description
...
candidate_interview['content'] = candidate_interview[['type','InternalInterviewDescription', 'InternalInterviewPlace']]\
    .apply(lambda x: '\n'.join([': '.join([i for i in e if i]) for e in zip(['Type','Description', 'Place'], x) if e[1]]), axis=1)
