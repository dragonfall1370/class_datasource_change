...
jobapp_no_shortlisted = pd.read_sql("""
...
where application_stage <> 'Employer Interview'
""", engine_sqlite)
...
join (select i.JobID as job_externalid,
             ...
             i.CreatedDate,
             it.Description as application_stage
from Interview i
...
where stage = 'Employer Interview'
""", engine_sqlite)
jobapp = pd.concat([jobapp_no_shortlisted, jobapp_shortlisted])
assert False
# %% mapping stage
...
tem2['matcher'] = tem2['FireFish Job Stage'].apply(lambda x: ''.join(re.findall('[a-zA-Z0-9]', x)).lower())

tem3 = tem1.merge(tem2, on='matcher', suffixes=['', '_y'], how='outer', indicator=True)
tem3['Vincere Job Stage'].unique()
tem3 = tem3.loc[tem3['Vincere Job Stage'].notnull()]
tem4 = jobapp.merge(tem3, on='application_stage')
tem4['application_stage'].unique()
tem4['timestamp'] = tem4['CreatedDate'].astype(str)
tem4['vincere Stage'] = tem4['Vincere Job Stage'].astype(str)
# %%
