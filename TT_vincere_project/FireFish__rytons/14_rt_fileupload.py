...
document_path = r'D:\Tony\Rytons Associates_CSV_20190919\doc\doc\doc'
...
# %% extract data
# file = pd.read_sql("""
# select CoreObjectID, FileName, CreatedDate, Name
# from Document d
# left join DocumentCategory dc on dc.ID = d.DocumentTypeID
# """, engine_sqlite)
#
#
...
# # assert False
#
# temp_msg_metadata1 = temp_msg_metadata.merge(file, left_on='matcher', right_on='CoreObjectID')
# temp_msg_metadata1['external_id'] = temp_msg_metadata1['CoreObjectID'].map(lambda x: str(x))
...
# from Document d
# left join DocumentCategory dc on dc.ID = d.DocumentTypeID
# where Name not in ('Job', 'Company', 'Setting')
# """, engine_sqlite)
#
# temp_msg_metadata2 = temp_msg_metadata.merge(file2, left_on='matcher', right_on='CoreObjectID')
# temp_msg_metadata2['external_id'] = temp_msg_metadata2['CoreObjectID'].map(lambda x: str(x))
# temp_msg_metadata2['file_name'] = temp_msg_metadata2['alter_file2']

temp_msg_metadata = vincere_common.get_folder_structure(document_path)
temp_msg_metadata['matcher'] = temp_msg_metadata['root'].apply(lambda x: x.split('\\')[-2])
temp_msg_metadata['file_name'] = temp_msg_metadata['alter_file2']
temp_msg_metadata['external_id'] = temp_msg_metadata['matcher'].map(lambda x: str(x))
assert False
# %% company
# temp_msg_metadata_comp = temp_msg_metadata1.loc[temp_msg_metadata1['Name'] == 'Company']
...
vindoc = vindoc[['id', 'uploaded_filename', 'created']].merge(temp_msg_metadata[['alter_file2', 'file']], left_on='uploaded_filename', right_on='alter_file2', how='left')
