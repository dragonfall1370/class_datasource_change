...
document_path = cf['default'].get('upload_folder')
...
# %% extract data
file = pd.read_sql("""
SELECT Id || '.docx' as Id , ParentId as entityid, Title, CreatedDate from Note;
""", engine_sqlite)
...
temp_msg_metadata['extension_file'] = '.docx'
# temp_msg_metadata.loc[temp_msg_metadata['file_name']=='e2555008-ec39-459e-b538-128f7ad3e1e4.txt']
assert False
# %% transform data
...
vindoc = vindoc[['id', 'uploaded_filename', 'created']].merge(temp_msg_metadata[['alter_file2', 'Title','CreatedDate']], left_on='uploaded_filename', right_on='alter_file2')
vindoc.loc[vindoc['alter_file2'].notnull(), 'uploaded_filename'] = vindoc.loc[vindoc['alter_file2'].notnull(), 'Title']
vindoc['uploaded_filename'] = vindoc['uploaded_filename'].apply(lambda x: '%s.docx' % x)
