...
#%% job app
sql = """
select idassignment as positionExternalId
     , idperson as candidateExternalId
     , createdon as actionedDate
     , cp.value as application_stage
from assignmentcandidate ac
left join candidateprogress cp on ac.idcandidateprogress = cp.idcandidateprogress
...
"""
cand = pd.read_sql(sql, engine_sqlite)
cand['actionedDate'] = cand['actionedDate'].apply(lambda x: str(x).split(' ')[0])
cand = cand.rename(columns={'positionExternalId':'application-positionExternalId', 'candidateExternalId':'application-candidateExternalId'
    , 'actionedDate': 'application-actionedDate'})
cand['application_stage'] = 'SHORTLISTED'
cand.to_csv(os.path.join(standard_file_upload, '7_jobapplication_other_2.csv'), index=False)