...
left join "user" u on u.fullname = al.createdby
where ContextEntityNumber is not null
""", engine_sqlite)
...
left join "user" u on u.fullname = al.createdby
where ContextEntityNumber is not null
and candidate_externalid = '43ef2a0c-cbe5-4cd1-9b63-9bf052c4f90d'
""", engine_sqlite)
...
assert False
# %%
company_log['Description'] = company_log['Description'].apply(lambda x: x.replace('\\x0d\\x0a','\r\n') if x else x)
...
contact_log['Description'] = contact_log['Description'].apply(lambda x: x.replace('\\x0d\\x0a','\r\n') if x else x)
...
job_log['Description'] = job_log['Description'].apply(lambda x: x.replace('\\x0d\\x0a','\r\n') if x else x)
...
job_src['insert_timestamp'] = pd.to_datetime(job_src['CreatedOn'])
job_src['content'] = job_src[['FullName', 'Value', 'ContactedOn', 'ContactSubject']]\
    .apply(lambda x: '\n'.join([': '.join([i for i in e if i]) for e in zip(['Name','Source Progress', 'last Contact', 'Summary'], x) if e[1]]), axis=1)
job_src['content'] = '###SOURCES###\n'+job_src['content']
job_src['position_external_id'] = job_src['job_externalid']

candidate_log['Description'] = candidate_log['Description'].apply(lambda x: x.replace('\\x0d\\x0a','\r\n') if x else x)
...
re3 = vincere_activity.transform_activities_temp(job_src, conn_str_ddb, mylog)
