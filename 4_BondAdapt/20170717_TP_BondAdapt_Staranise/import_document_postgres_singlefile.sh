...
file=/tmp/DOCUMENT_CONTENT_FILE_encode.txt
...
do
	...
	sed "s/'/''/g" $i | sed "s:<feff>::g" | sed "s:<9c>^Z::g" | sed "s:^X::g" | sed "s:^@::g" | sed "s:<A1>::g" | sed "s:<A6>::g" | sed "s:¦       ^@^@::g" | sed "s:<80><99>::g" > /tmp/DOCUMENT_CONTENT_Note.txt
	...
	psql -h $host -U $user -p $port -d "$db" -c "SET CLIENT_ENCODING TO 'UTF8'; INSERT INTO position_candidate_feedback (doc_id, user_account_id, comment_body) VALUES ('$doc_id', '-10', '$prefix: $note');"
	echo "psql -h $host -U $user -p $port -d "$db" -c "SET CLIENT_ENCODING TO 'UTF8'';' INSERT INTO position_candidate_feedback '('doc_id, user_account_id, comment_body')' VALUES '(''$doc_id', '-10', '$prefix: $note'')'';'""
