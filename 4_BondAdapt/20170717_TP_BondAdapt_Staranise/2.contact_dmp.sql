...
------------
-- MAIL-----
, mail1 (userID,email) as (select REFERENCE, ltrim(rtrim(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(EMAIL_LINK,'/',''),'<',''),'>',''),'(',''),')',''),':',' '),'.@',''),'+',' '),CHAR(9),' '),'      ',''),'•',' '),'''',' '),'&',' '),';',' '),'@@','@'),'[',' '),']',' '))) as email from PROP_EMAIL where EMAIL_LINK is not null and EMAIL_LINK != '')
...
, mail2a (userID,String,rn) as (SELECT userID,iif(right(string,1) = '.',LEFT(String, len(String) -1),String),ROW_NUMBER() OVER(PARTITION BY userID ORDER BY userID DESC) AS rn FROM mail2 WHERE String like '%_@_%_.__%')
--select * from mail2a where userid in (590700,738164,827440) --in (396970,397884,397965,398036,406635,409577,410217,414232,415647,420063,436059,528026,550535,558455,567093,588173,588743,591415,592958,596988,599869,626195,629811,631637,659586,661339,676728,718815,728977,753998,760676,801076,814991)
, mail2b (userID,String,rn) as (SELECT userID,String,ROW_NUMBER() OVER(PARTITION BY String ORDER BY userID DESC) AS rn FROM mail2a WHERE rn > 1)
--, mail2c (userID,String) as (select userID,concat('DUPLICATED_',rn,'_',String) from mail2b where rn > 1) --userid in (397776,397113,479597,410025,397109)
--, mail2c (userID,String) as (select userID,String from mail2b where rn > 1)
--select count(*) from mail2c where userid = 470934
--select userID from mail2c group by userid having count(*) > 1
--SELECT userid, STUFF((SELECT DISTINCT ',' + String from mail2b WHERE rn > 1 and userid = a.USERID FOR XML PATH ('')), 1, 1, '')  AS URLList FROM mail2a AS a GROUP BY a.USERID
/*SELECT userid, STUFF((SELECT distinct ', ' + REPLACE(String COLLATE Latin1_General_BIN, char(26), '') --REPLACE(ISNULL(NOTE, ''), CHAR(26), '')
	   FROM mail2c where userid = a.userid and rn > 1
	   FOR XML PATH(''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '') As DESCRIPTION FROM mail2c as a GROUP BY a.userid
select * from mail2b where userid in (397776,397113,479597,410025,397109)*/

, mail3 (userID,String) as (SELECT userid, STUFF((SELECT DISTINCT ',' + String from mail2a where rn > 1 and userid = a.USERID FOR XML PATH ('')), 1, 1, '') AS URLList FROM mail2a AS a GROUP BY a.USERID)
--select * from mail3 where userid in (590700,738164,827440) -- and right(string,1) = '.'
...
select distinct 
	...
	, case when (de.String != '' or de.String is not null) then concat('DUPLICATED_',de.rn,'_',de.String) else pe.String end as 'contact-email'
	...
	, concat (
		 case when (oe.String != '' and oe.String is not null) then concat('Other email: ',oe.String,char(10)) else '' end
...
left join mail2b de ON ccc.CONTACT = de.userid -- DUPLICATED-EMAIL
