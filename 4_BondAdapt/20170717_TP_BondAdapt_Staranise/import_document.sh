...
#find /mnt/vmware/general_notes/ -name *.rtf -type f > /tmp/DOCUMENT_CONTENT_FILE.txt
...
do
	...
	sed "s/'/''/g" $i | sed "s:﻿::g" > /tmp/DOCUMENT_CONTENT_Note.txt
	#sed "s/'/''/g" $i | sed "s:<p>﻿</p>:<p></p>:g" > /tmp/DOCUMENT_CONTENT_Note.txt
	#dos2unix /tmp/DOCUMENT_CONTENT_Note.txt
