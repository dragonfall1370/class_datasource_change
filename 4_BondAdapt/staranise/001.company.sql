...
--, cg.STATUS as '(STATUS)'
	...
	--, cnt.DESCRIPTION as '(company-locationCountry)'
		...
		when cnt.DESCRIPTION like 'CAMBODI%' then 'KH'
		...
		when cnt.DESCRIPTION like 'HONG%' then 'HK'
		when cnt.DESCRIPTION like 'INDIA%' then 'IN'
		when cnt.DESCRIPTION like 'INDONES%' then 'ID'
		when cnt.DESCRIPTION like 'IRELAND%' then 'IE'
		when cnt.DESCRIPTION like 'ISLE%' then 'GB'
		when cnt.DESCRIPTION like 'ISRAEL%' then 'IL'
		when cnt.DESCRIPTION like 'ITALY%' then 'IT'
		when cnt.DESCRIPTION like 'JAPAN%' then 'JP'
		when cnt.DESCRIPTION like 'KAZAKHS%' then 'KZ'
		when cnt.DESCRIPTION like 'KOREA%' then 'KR'
		when cnt.DESCRIPTION like 'MACAU%' then 'MO'
		when cnt.DESCRIPTION like 'MALAYSI%' then 'MY'
		when cnt.DESCRIPTION like 'MAURITI%' then 'MU'
		when cnt.DESCRIPTION like 'MONGOLI%' then 'MN'
		when cnt.DESCRIPTION like 'MYANMAR%' then 'MM'
		when cnt.DESCRIPTION like 'NETHERL%' then 'NL'
		when cnt.DESCRIPTION like 'PHILIPP%' then 'PH'
		when cnt.DESCRIPTION like 'SINGAPO%' then 'SG'
		when cnt.DESCRIPTION like 'SWITZER%' then 'CH'
		when cnt.DESCRIPTION like 'TAIWAN%' then 'TW'
		when cnt.DESCRIPTION like 'THAILAN%' then 'TH'
...
--, cg.LOCATION as '(company-locationName)'
	...
	, owner.EMAIL_LINK as 'company-owners'
        , Stuff(  Coalesce('Company Owner: ' + NULLIF(owner.EMPLOYEE_NAME, '') + char(10), '')
...
--where cg.name like '%Orangefield%'
