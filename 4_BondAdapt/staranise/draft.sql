...
General Notes
select * from DOCUMENTS where DOC_CATEGORY = 6532839 and OWNER_ID = <<PROP_PERSON_GEN.REFERENCE>>
...
select * from tmp where ENTITY_ID = 395016
...
SELECT ENTITY_ID, JE.J_NOTES 
        FROM LK_ENTITIES_JOURNAL J INNER JOIN JOURNAL_ENTRIES JE ON JE.ID = J.JOURNAL_ID WHERE JE.J_NOTES != '' and JE.J_NOTES is not null --J.ENTITY_ID = <<PROP_JOB_GEN.REFERENCE>>
