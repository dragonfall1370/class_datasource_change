...
--, doc(OWNER_ID, DOC_ID) as (SELECT OWNER_ID, STRING_AGG( concat(cast(DOC_ID as varchar(max)),'.',FILE_EXTENSION),',') WITHIN GROUP (ORDER BY DOC_ID) DOC FROM DOCUMENTS where DOC_CATEGORY = 6532841 GROUP BY OWNER_ID)

, notes0 (owner_id, content, CREATED_DATE) as (SELECT owner_id, ltrim(replace(replace( [dbo].[udf_StripHTML](convert(varchar(max),convert(varbinary(max),DOCUMENT))) ,'Â',''),'ï»¿','')) as 'content', CREATED_DATE FROM DOCUMENTS where DOC_CATEGORY = 6532841)
, notes (owner_id, content) as (
       SELECT owner_id
       ...
       FROM notes0 where content <> '' GROUP BY owner_id
       )
--select owner_id from notes0 group by owner_id having count(*) > 1
--select owner_id,* from notes where owner_id in (81913, 500551)


--select count(*)
select --top 5
--select --distinct
	  ccc.CONTACT as 'contact-externalId', pg.person_id as '#person_id'
...
--            + Coalesce('Recruits For: ' + NULLIF(cast(pg. as nvarchar(max)), '') + char(10), '')
              ...
              + Coalesce(char(10) + 'NOTES: ' + char(10) + NULLIF(cast(notes.content as nvarchar(max)), '') + char(10), '')
              , 1, 0, '') as 'contact-note'
...
left join ( 
       ...
       from (
              select REFERENCE, STREET1, STREET2, LOCALITY, TOWN, county, POST_CODE, COUNTRY, MN.DESCRIPTION ,CONFIG_NAME, rn = ROW_NUMBER() OVER (PARTITION BY REFERENCE ORDER BY VALID desc, CONFIG_NAME asc)
              ...
              left JOIN (select * from MD_MULTI_NAMES where LANGUAGE = 10010) MN ON MN.ID = ADDRESS.COUNTRY
...
left join (select * from tel where CONFIG_NAME = 'Home') telhome on ccc.CONTACT = telhome.REFERENCE --candidate-homePhone
left join (select * from tel where CONFIG_NAME = 'Mobile') telmobile on ccc.CONTACT = telmobile.REFERENCE --candidate-mobile
left join (select * from tel where CONFIG_NAME = 'Work') telwork on ccc.CONTACT = telwork.REFERENCE --candidate-phone & candidate-workPhone
...
left join notes on notes.OWNER_ID = ccc.CONTACT
--where pg.person_id in (1094197,1107874)
--or pg.REFERENCE in (81913, 500551)
