...
with
-- EMAIL
  mail1 (ID,email) as (select UC.userID, replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(UC.email,',',UC.email2),'/',' '),'<',' '),'>',' '),'(',' '),')',' '),':',' '),'.@','@'),'@.','@'),'+',' '),'&',' '),'[',' '),']',' '),'?',' '),'''',' '),';',' '),'•',' '),CHAR(9),' ') as mail from bullhorn1.BH_UserContact UC left join bullhorn1.Candidate C on C.userID = UC.UserID where UC.email like '%_@_%.__%' or UC.email2 like '%_@_%.__%' and C.isPrimaryOwner = 1 )
...
, mail4 (ID,email,rn) as ( SELECT ID, email = ltrim(rtrim(CONVERT(NVARCHAR(MAX), email))), r1 = ROW_NUMBER() OVER (PARTITION BY ID ORDER BY ID desc) FROM mail2 )
...
, note as (
	SELECT CA.userID
		 , Stuff( Coalesce('Candidate ID: ' + NULLIF(cast(CA.userID as varchar(max)), '') + char(10), '')
                        ...
                        + Coalesce('Opted In - SMS Messages: ' + NULLIF(cast(UC1.smsOptIn as varchar(max)), '') + char(10), '')
                        --+ Coalesce('CV Text: ' + NULLIF(cast(UC1.description as varchar(max)), '') + char(10), '')
                        + Coalesce('CV Text: ' + NULLIF([dbo].[udf_StripHTML](UC1.description), '') + char(10), '')
                        /*
                        ...
                        , 1, 0, '') as note
	-- select top 100 *
	--select top 200 [dbo].[udf_StripHTML](UC1.description) --REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE([dbo].[udf_StripHTML](UC1.description),'d&#39;', ''),'l&#39;',''),'&hellip;',''),'&bull;',''),'&mdash;',''),'&Oslash;',''),'&#39;',''),'qu&#39;','')
	from bullhorn1.Candidate CA
	left join bullhorn1.BH_UserContact UC1 ON UC1.userID = CA.userID
...
--                , iif(C.userID in bl.userid,Coalesce(NULLIF(replace(C.LastName,'?',''), ''), concat('Lastname-',C.candidateID,'-BlackList-',bl.ClientCorporationID))
	...
	---left join BH_ClientCorporationBlackList bl on bl.userid = C.userid
	where C.isPrimaryOwner = 1
	--and c.candidateID = 300
	--and C.FirstName = 'Lynette'
