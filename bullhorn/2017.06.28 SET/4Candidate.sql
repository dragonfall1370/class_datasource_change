...
, mail5 (ID, email1, email2) as (
		...
		where pe.rn = 1 )
--select * from mail5
--, pe as (select ID, email from mail4 where rn = 1)
--, we as (select ID, email from mail4 where rn = 2)
, oe1 as (select ID, email from mail4 where rn = 3)
--, oe2 as (select ID, email from mail4 where rn = 4)
...
, note as (
	SELECT CA.userID
		 , Stuff( Coalesce('Candidate ID: ' + NULLIF(cast(CA.candidateID as varchar(max)), '') + char(10), '')
		        --+ Coalesce('Other email: ' + NULLIF(cast(mail5.email3 as varchar(max)), '') + char(10), '')
...
--, t1 as (
	select
		...
		--, Coalesce(NULLIF(pe.email,''), concat('candidate-',cast(C.userID as varchar(max)),'@noemail.com') ) as 'candidate-email'
		--, we.email as 'candidate-workEmail'
	...
	--left join pe on C.userID = pe.ID
	--left join we on C.userID = we.ID
