...
, note as (
	...
	+ Coalesce('Benefits: ' + NULLIF(cast(JP.benefits as varchar(max)), '') + char(10), '')
       ...
       + Coalesce('Interview Required?: ' + NULLIF( cast( iif(JP.isInterviewRequired in (null,0),'No','Yes') as varchar(max)), '') + char(10), '')       	 	
       ...
       + Coalesce('Visa Sponsorship Provided: ' + NULLIF( cast( iif(JP.willSponsor in (-1,0),'No','Yes') as varchar(max)), '') + char(10), '')       	 	
...
select --top 100 
	...
	--, cast(a.description as varchar(max)) as 'position-internalDescription'
       , ltrim(Stuff(
                            Coalesce('Required Skills: ' + NULLIF(convert(varchar(max),a.skills), '') + char(10), '')
                        + Coalesce('Certification Requirements: ' + NULLIF(convert(varchar(max),a.certifications), '') + char(10), '')
                        + Coalesce('Degree Requirements: ' + NULLIF(convert(varchar(max),a.degreeList), '') + char(10), '')
                        + Coalesce('Description: ' + NULLIF(char(10) + cast(a.description as varchar(max)), '') + char(10), '')
	...
	--, a.address, a.city, a.state, a.zip, replace(coalesce(NULLIF(tc.abbreviation, ''), ''),'NULL','') as 'Country'
-- select distinct employmentType -- select distinct Type -- select distinct status -- select distinct customtext1 -- select count(*) --2574 -- select top 100 startDate  -- select top 10 *
...
with t as (
       SELECT
               ...
               , cast(1006 as int) as form_id
               , cast(1016 as int) as field_id
               , a.skills as field_value
       from bullhorn1.BH_JobPosting a
       where convert(varchar(max),a.skills) <> '' )
select * from t 
*/