...
, int as (
        select jobPostingID
        , Stuff(  Coalesce('Travel Requirements: ' + NULLIF(cast(a.customText2 as varchar(max)), '') + char(10), '')
                + Coalesce('Additional Travel/Location Details: ' + NULLIF(cast(a.customTextBlock1 as varchar(max)), '') + char(10), '')
                + Coalesce('Will Relocate ?: ' + NULLIF(cast(a.willRelocate as varchar(max)), '') + char(10), '')
                + Coalesce('Visa Sponsorship Provided: ' + NULLIF(cast(a.willSponsor as varchar(max)), '') + char(10), '')
                + Coalesce('Shift/Hours Details: ' + NULLIF(cast(a.correlatedCustomText10 as varchar(max)), '') + char(10), '')
                + Coalesce('Placement/Start Date: ' + NULLIF(cast(a.correlatedCustomDate1 as varchar(max)), '') + char(10), '')
                + Coalesce('Placement Salary or Contract Rate: ' + NULLIF(cast(a.correlatedCustomInt1 as varchar(max)), '') + char(10), '')
                + Coalesce('Job Description: ' + char(10) + NULLIF(cast(a.description as varchar(max)), '') + char(10), '')
                --+ Coalesce('Custom Component 1: ' + NULLIF(cast(a.CustomComponent1 as varchar(max)), '') + char(10), '')
                --+ Coalesce('Custom Component 2: ' + NULLIF(cast(a.CustomComponent2 as varchar(max)), '') + char(10), '')
                --+ Coalesce('Custom Component 3: ' + NULLIF(cast(a.CustomComponent3 as varchar(max)), '') + char(10), '')
                , 1, 0, '') as 'position-internalDescription' 
        -- select JP.skills, JP.yearsRequired, JP.degreeList, JP.certifications, JP.benefits
        from bullhorn1.BH_JobPosting a --where JP.numOpenings is not null
        )
--select * from int 
...
+ Coalesce('Assignment Type: ' + NULLIF(cast(JP.customText11 as varchar(max)), '') + char(10), '')
+ Coalesce('Status: ' + NULLIF(cast(JP.status as varchar(max)), '') + char(10), '')
+ Coalesce('Priority: ' + NULLIF(cast(JP.type as varchar(max)), '') + char(10), '')
+ Coalesce('Reporting to (Contact): ' + NULLIF(cast(UC.name as varchar(max)), '') + char(10), '')
+ Coalesce('Reporting to (Job Title): ' + NULLIF(cast(JP.reportTo as varchar(max)), '') + char(10), '')
+ Coalesce('Source: ' + NULLIF(cast(JP.source as varchar(max)), '') + char(10), '')
+ Coalesce('Superannuation Rate: ' + NULLIF(cast(JP.customInt1 as varchar(max)), '') + char(10), '')
+ Coalesce('Benefits: ' + NULLIF(cast(JP.benefits as varchar(max)), '') + char(10), '')
+ Coalesce('Benefit Details: ' + NULLIF(cast(JP.customTextBlock2 as varchar(max)), '') + char(10), '')
+ Coalesce('Perm Fee (%): ' + NULLIF(cast(JP.feeArrangement as varchar(max)), '') + char(10), '')
+ Coalesce('PO Number: ' + NULLIF(cast(JP.customText3 as varchar(max)), '') + char(10), '')
+ Coalesce('# required: ' + NULLIF(cast(JP.numOpenings as varchar(max)), '') + char(10), '')
--+ Coalesce('Skills / Experience: ' + NULLIF(cast(JP.skillsInfoHeader as varchar(max)), '') + char(10), '')
+ Coalesce('Skills / Experience: ' + NULLIF(cast(JP.skills as varchar(max)), '') + char(10), '')
+ Coalesce('Preferred Work Experience: ' + NULLIF(cast(JP.customText4 as varchar(max)), '') + char(10), '')
...
+ Coalesce('Job Duration: ' + NULLIF(cast(JP.durationWeeks as varchar(max)), '') + char(10), '')
...
+ Coalesce('Days per week: ' + NULLIF(cast(JP.hoursPerWeek as varchar(max)), '') + char(10), '')
...
+ Coalesce('Country: ' + NULLIF(cast(tmp_country.country as varchar(max)), '') + char(10), '')
	        , 1, 0, '') as note
        -- select top 50 * -- select count(*)
        from bullhorn1.BH_JobPosting JP --where cast(skills as varchar(max)) <> ''
        left join (select bh_usercontact.userid , bh_usercontact.name from bullhorn1.BH_UserContact) UC on JP.clientuserid = UC.userID
...
select --top 100 
	...
	, case when a.employmentType is null then 'PERMANENT'
	       ...
	       when a.employmentType = 'Contract To Perm' then 'PERMANENT'
	       else ''
	...
	, int.[position-internalDescription] as 'position-internalDescription' --, cast(a.description as varchar(max)) as 'position-internalDescription'
