...
, note as (
	...
	, Stuff( Coalesce('Client Corporation ID: ' + NULLIF(convert(varchar(max),CC.clientCorporationID), '') + char(10), '')
                        ...
                        + Coalesce('Ownership: ' + NULLIF(convert(varchar(max),CC.ownership), '') + char(10), '')
                        + Coalesce('Company Overview: ' + NULLIF(ltrim(rtrim(cast(CC.notes as varchar(max)))), '') + char(10), '')                     
                        + Coalesce('Key Selling Points of Company: ' + NULLIF([dbo].[fn_ConvertHTMLToText](cast(CC.companyDescription as varchar(max))),'') + char(10), '')
                        + Coalesce('Standard Perm Fee (%): ' + NULLIF(cast(CC.feeArrangement as varchar(max)), '') + char(10), '')
                        + Coalesce('Special Terms Agreed: ' + NULLIF(cast(CC.customTextBlock1 as varchar(max)), '') + char(10), '')
                        ...
                        --+ Coalesce('Billing Contact: ' + NULLIF(cast(CC.CustomComponent1 as varchar(max)), '') + char(10), '')
                        --+ Coalesce('Billing Phone: ' + NULLIF(CC.CustomComponent2, '') + char(10), '')
                        --+ Coalesce('Billing Email: ' + NULLIF(cast(CC.CustomComponent3 as varchar(max)), '') + char(10), '')
...
select --top 100 
	...
	, headquarter.name as 'company-headquarter'
        , ltrim(Stuff( Coalesce(' ' + NULLIF(CC.address1, ''), '') + Coalesce(', ' + NULLIF(CC.address2, ''), '') + Coalesce(', ' + NULLIF(CC.city, ''), '') + Coalesce(', ' + NULLIF(CC.state, ''), '') + Coalesce(', ' + NULLIF(tc.country, ''), '') , 1, 1, '') ) as 'company-locationName'
        , ltrim(Stuff( Coalesce(' ' + NULLIF(CC.address1, ''), '') + Coalesce(', ' + NULLIF(CC.address2, ''), '') + Coalesce(', ' + NULLIF(CC.city, ''), '') + Coalesce(', ' + NULLIF(CC.state, ''), '') + Coalesce(', ' + NULLIF(tc.country, ''), '') , 1, 1, '') ) as 'company-locationAddress'
...
-- select *  -- select count (*) --560 -- select distinct CC.ownership
from bullhorn1.BH_ClientCorporation CC
