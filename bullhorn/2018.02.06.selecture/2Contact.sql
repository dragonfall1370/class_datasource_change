...
, note as (
	select UC.userID
	, Stuff(  Coalesce('BH Contact ID: ' + NULLIF(cast(Cl.UserID as varchar(max)), '') + char(10), '')
		  ...
		  + Coalesce('Desired Levels: ' + NULLIF(cast(Cl.desiredSpecialties as varchar(max)), '') + char(10), '')
		  + Coalesce('Desired Job Sub-Category: ' + NULLIF(cast(Cl.desiredSkills as varchar(max)), '') + char(10), '')
		...
		*/
		--+ Coalesce('Last Note: ' + NULLIF( replace(replace(replace(replace(replace(replace([dbo].[udf_StripHTML](UC.lastNote_denormalized),'&nbsp;',''),'&amp;','&'),'&#39;',''''),'&ldquo;','"'),'&rdquo;','"'),'&gt;','') , '') + char(10), '') --cast(UC.lastNote_denormalized as varchar(max))
                + Coalesce('Last Note: ' + NULLIF(ltrim(rtrim([dbo].[fn_ConvertHTMLToText](cast(UC.lastNote_denormalized as varchar(max)) ))), '') + char(10), '')
                , 1, 0, '') as note
        ...
        left join BusinessSector BS on UC.userID = BS.userId
...
select    UC.clientCorporationID as 'contact-companyId'
	...
	--, UC.namePrefix as 'Title' --VC gender_title: Mr. Mrs. Ms. Miss. Dr. Captain
	, case UC.namePrefix
                when 'Dr' then 'Dr.'
                when 'Dr.' then 'Dr.'
                when 'Micmr' then ''
...
with t as (
select     Cl.clientID as 'externalId'
