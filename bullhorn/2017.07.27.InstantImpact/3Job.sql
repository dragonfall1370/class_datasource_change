...
--DOCUMENT
, doc as (select a.jobPostingID, concat(a.jobPostingFileID,a.fileExtension) as jobFile from bullhorn1.View_JobPostingFile a where fileExtension in ('.doc','.docx','.pdf','.xls','.xlsx','.rtf','.html'))
, jobdoc as (SELECT jobPostingID, STUFF((SELECT DISTINCT ',' + jobFile from doc WHERE jobPostingID = a.jobPostingID FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '')  AS jobFile FROM doc AS a GROUP BY a.jobPostingID)
...
select a.jobPostingID as 'position-externalId' 
	...
	, case when a.employmentType = 'Permanent' then 'PERMANENT'
	       when a.employmentType = 'Intern to Perm' then 'TEMPORARY_TO_PERMANENT'
	       when a.employmentType = 'Intern only' then 'CONTRACT'
