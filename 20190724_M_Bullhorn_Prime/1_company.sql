...
--            + coalesce('Date Added: ' + nullif(cast(CC.dateAdded as varchar(max)), '') + char(10), '')
              ...
              + coalesce('Company Overview: ' + nullif(cast(CC.notes as varchar(max)), '') + char(10), '')
...
, doc (clientCorporationID,ResumeId) as 
			( SELECT clientCorporationID
			, STRING_AGG(cast(concat(clientCorporationFileID,fileExtension) as nvarchar(max)),',' ) 
								WITHIN GROUP (ORDER BY clientCorporationFileID) att 
			from bullhorn1.BH_ClientCorporationFile where isdeleted <> 1 
			/*and fileExtension in ('.doc','.docx','.pdf','.rtf','.xls','.xlsx','.htm', '.html', '.msg', '.txt')*/ 
...
, dup as (SELECT clientCorporationID,ltrim(rtrim(name)) as name
...
, owner0 as (select distinct C.clientCorporationID, C.recruiterUserID, UC.firstName, UC.lastname, UC.email /*, UC.email2, UC.email3, UC.email_old*/ 
			FROM bullhorn1.BH_Client C left join bullhorn1.BH_UserContact UC on UC.userid = C.recruiterUserID 
			where UC.email like '%_@_%.__%' /*UC.email is not null and UC.email <> ''*/ )

, owner (clientCorporationID,owners) as (SELECT clientCorporationID, STRING_AGG( email,',' ) WITHIN GROUP (ORDER BY email) att 
...
select --top 10
         concat('PR', CC.clientCorporationID) as 'company-externalId'
...
select 'PR999999999','Default company','','','','','','','','','','','','This is default company from data migration'