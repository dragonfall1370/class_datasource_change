...
, trim(@NewLineChar from concat(
	iif(len(trim(isnull(convert(varchar(50), x.ActivityDate, 111), ''))) > 0,  @NewLineChar + 'Activity Date: ' + trim(isnull(convert(varchar(50), x.ActivityDate, 111), '')), '')
	...
	, iif(len(trim(isnull(x.Completed__c, ''))) > 0, @NewLineChar + 'Completed: ' + trim(isnull(x.Completed__c, '')), '')
	...
	, iif(len(trim(isnull(x.EmailMessageId, ''))) > 0, @NewLineChar + 'EmailMessageId: ' + trim(isnull(x.EmailMessageId, '')), '')
...
from [Task] x
left join TaskRelation rel on rel.TaskId = x.Id
