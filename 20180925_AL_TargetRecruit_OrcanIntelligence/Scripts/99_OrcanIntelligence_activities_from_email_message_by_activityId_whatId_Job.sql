...
with
Tasks as (
	select x.Id as TaskId, x.WhatId as JobId, j.Name as JobName
	from Task x
	--join Account a on x.WhatId = a.Id
	join AVTRRT__Job__c j on x.WhatId = j.Id
...
, trim(@NewLineChar from concat(
	@DoubleNewLine + 'Related To: ' + concat('Job #', trim(isnull(t.JobName, '')), ' (', t.JobId, ')')
