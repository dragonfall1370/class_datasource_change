...
, person_document as (select uniqueid
	, string_agg(replace(right("relative document path", position(E'\\' in reverse("relative document path")) - 1), 'txt', 'rtf'), ',' order by to_date(date, 'DD/MM/YY') desc) as person_document
...
, c."4 ref no numeric"
, coalesce(nullif(c."185 surname alphanumeric", ''), 'No last name') as "contact-lastName"
, c."186 forenames alphanumeric" as "contact-firstName"
