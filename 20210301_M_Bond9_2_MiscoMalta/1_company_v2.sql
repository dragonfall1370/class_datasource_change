...
, com_document as (select uniqueid
	, string_agg(replace(right("relative document path", position(E'\\' in reverse("relative document path")) - 1), 'txt', 'rtf'), ',' order by to_date(date, 'DD/MM/YY') desc) as com_document
...
, replace(concat_ws(', '
	, coalesce(nullif(split_part(c."25 address alphanumeric", '~', 1), ''), NULL)
	, coalesce(nullif(split_part(c."25 address alphanumeric", '~', 2), ''), NULL)
	, coalesce(nullif(split_part(c."25 address alphanumeric", '~', 3), ''), NULL)
	, coalesce(nullif(split_part(c."25 address alphanumeric", '~', 4), ''), NULL)
	, coalesce(nullif(split_part(c."25 address alphanumeric", '~', 5), ''), NULL)
