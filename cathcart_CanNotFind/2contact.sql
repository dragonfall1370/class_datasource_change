...
select
        ...
        , UPPER(replace(c.Title,'SIR','MR')) as 'contact-title' -- INJECTION
...
-- select count(*) --12336 -- select top 10 * 
from contacts c --where c.Surname like '%Muhammad%' and c.Forename like '%Adnan%'
left join Clients cl on cl.MainSiteUnique = c.SiteUnique
...
--where cl.UniqueID <> cl.MainSiteUnique
--e1.email like '%zanroo%' or c.WebAddress like '%zanroo%'
--where c.SiteUnique = '0'
--or c.SiteUnique = '2106'
--where c.ContactUniqueID in ('2456','995','2154')
--where c.AccountManager <> c.CreatingUser
--where SiteUnique = '0'
where (c.Surname like '%Muhammad%' and c.Forename like '%Adnan%')

select count(*) --12336 -- select top 100  cl.UniqueID, cl.MainSiteUnique, c.SiteUnique
from contacts c
left join Clients cl on cl.UniqueID = c.SiteUnique where cl.UniqueID is not null and cl.UniqueID <> cl.MainSiteUnique
--left join Clients cl on cl.MainSiteUnique = c.SiteUnique where cl.MainSiteUnique is not null --2752
--left join Clients cl on cl.UniqueID = c.SiteUnique where cl.UniqueID is not null
--left join Clients cl on cl.UniqueID = c.SiteUnique where c.SiteUnique <> '0'
...
with t as (
        select
                ...
                , case c.Title
                        ...
                        end as 'contact-title' -- INJECTION
                , replace(c.HomeEmail,char(9),'') as 'contact-personalemail' -- INJECTION
        from contacts c )
--select count(*) from t --10594
select * from t
--select distinct [contact-title] from t
where
--   ([contact-mobile] is not null and [contact-mobile] <> '')
--or ([contact-title] is not null  and [contact-title] <> '')
 ([contact-personalemail] like '%@%' and [contact-personalemail] is not null and [contact-personalemail] <> '')
and [contact-externalid] = '3974'
