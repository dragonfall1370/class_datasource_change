...
, cte_contact AS (
	...
	--, CASE WHEN px.emailwork IS NOT NULL 
	--		THEN ROW_NUMBER() OVER(PARTITION BY LOWER(px.emailwork) 
	--		ORDER BY cp.sortorder ASC, cp.employmentfrom DESC, cp.isdefaultrole = '1') ELSE 1
	--	END rn_email
...
----Additional info
	...
	LEFT JOIN contact_benefit cb on cb.idcompany_person = cp.idcompany_person
	LEFT JOIN candidate_document cd on cd.idperson = cp.idperson
	LEFT JOIN contact_language cl on cl.idperson = cp.idperson
----Additional joins	
