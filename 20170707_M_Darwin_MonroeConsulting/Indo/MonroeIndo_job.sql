...
select 
case 
	when (V.ContactID = '' or V.ContactID is NULL) and V.CompanyID in (select CompanyID from ContactMaxID) then concat('MCIndo',CM.ContactMaxID)
	when (V.ContactID = '' or V.ContactID is NULL) and V.CompanyID not in (select CompanyID from ContactMaxID) then 'MCIndo9999999'
	when V.ContactID is NULL and V.CompanyID is NULL then 'MCIndo9999999'
	else concat('MCIndo',V.ContactID) end as 'position-contactId'
, V.CompanyId as 'CompanyID'
, V.ContactId as 'ContactID'
, concat('MCIndo',V.DynamicDataId) as 'position-externalId'
, V.JobTitle as 'position-title(old)'
, iif(V.DynamicDataId in (select DynamicDataId from dup where dup.rn > 1)
	, iif(dup.JobTitle = '' or dup.JobTitle is NULL,concat('No job title-',dup.DynamicDataId),concat(dup.JobTitle,'-',dup.DynamicDataId))
