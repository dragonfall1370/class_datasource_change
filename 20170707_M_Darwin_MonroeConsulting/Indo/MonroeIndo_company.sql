...
--MAIN SCRIPT
select concat('MCIndo',C.DynamicDataId) as 'company-externalId'
, C.CompanyName as '(OriginalName)'
, iif(C.DynamicDataId in (select DynamicDataId from dup where dup.rn > 1)
	, iif(dup.CompanyName = '' or dup.CompanyName is NULL,concat('Default Company-',dup.DynamicDataId,' [ID]'),concat(dup.CompanyName,'-DUPLICATE-',dup.DynamicDataId,' [ID]'))
	, iif(C.CompanyName = '' or C.CompanyName is null,concat('Default Company-',dup.DynamicDataId,' [ID]'),concat(C.CompanyName,' [ID]'))) as 'company-name'
...
, iif(C.LastUpdatedByuserId = '' or C.LastUpdatedByuserId is NULL,'',concat('Last Updated By: ',U.EmailAddress,char(10)))
...
left join Users U on C.LastUpdatedByuserId = U.Id
