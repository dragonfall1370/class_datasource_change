...
select
    ...
    , concat('Candidate External ID: BNS_',tc.id,char(10),
         ...
         if(tce.id = '' or tce.id is NULL,'',Concat(char(10), 'Original Email Address: ', tce.email_address, char(10))),
         ...
         if(cc.opleidingsniveau_c = '' or cc.opleidingsniveau_c is NULL,'',Concat(char(10), 'Education (Opleidings niveau): ', cc.opleidingsniveau_c, char(10))),
         if(cc.carriere_niveau_c = '' or cc.carriere_niveau_c is NULL,'',Concat(char(10), 'Career Level (Carriere niveau): ', cc.carriere_niveau_c, char(10))),
...
from Temp_Candidates tc left join contacts_cstm cc on tc.id = cc.id_c
                ...
                left join Temp_Can_Emails2 tce on tc.id = tce.id
                -- where tc.id = '13132785-ab9c-ce3f-17be-530733dd850b'
...
-- where concat(tc.first_name, ' ', tc.last_name) = 'Ingrid Hendriks'
