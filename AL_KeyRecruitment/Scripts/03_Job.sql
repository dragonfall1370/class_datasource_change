...
, note as (
	...
	, Stuff(
		...
		+ Coalesce('Priority: ' + NULLIF(cast(JP.CustomText2 as varchar(max)), '') + char(10), '')
...
/*       , Stuff( Coalesce('Location Requirements: ' + NULLIF(cast(a.onSite as varchar(max)), '') + char(10), '')
	...
	, convert(varchar(10),
		case a.isOpen
			when 1 then dateadd(year, 1, getdate())
			when 0 then iif(a.dateClosed is null, getdate() - 1, a.dateClosed)
		end
		,120
	) as 'position-endDate'
	...
	, trim(isnull(doc.files, '')) as 'position-document'
	--, concat(note.note,placementnote.note) as 'position-note' --left(,32000)
	, trim(isnull(note.note, '')) as 'position-note'
	--, skills as 'skills'
...
and a.employmentType <> 'Opportunity'
