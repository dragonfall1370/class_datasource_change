...
WITH main_company AS (
	SELECT *,
	ROW_NUMBER() OVER(PARTITION BY LOWER(TRIM(name)) ORDER BY CASE WHEN record_status IN ('L', 'D', 'Z') THEN 1 ELSE 2 END, create_timestamp) rn
	FROM organisation
),
company_phone AS (
	SELECT *,
...
c.create_timestamp::TIMESTAMP AS registration_date,
COALESCE(
	CASE
		WHEN c.rn = 1 THEN TRIM(c.name)
		ELSE CONCAT(TRIM(c.name), ' [Duplicate ', LPAD((c.rn - 1)::TEXT, 2, '0'), ']')
	END, 
'[No Company Name]'
) AS "company-name",
...
CONCAT_WS(
	...
	CASE WHEN c.annual_report IS NOT NULL THEN 'Annual report: Yes' ELSE 'Annual report: No' END,
	CASE WHEN c.annual_report_year IS NOT NULL THEN CONCAT('Annual report year: ', c.annual_report_year) END,
