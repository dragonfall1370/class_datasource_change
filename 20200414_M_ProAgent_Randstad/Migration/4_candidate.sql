...
--MAIN SCRIPT
select top 10 c.[PANO ] as [candidate-externalId] --#CF inject then
	...
	, case when len([氏名]) <= 2 then trim(left([氏名], 1))
		...
		else trim(left([氏名], 2)) end as [candidate-firstName]
	, case when len([氏名]) <= 2 then trim(right([氏名], 1))
		when charindex(' ', trim([氏名])) > 0 then trim(right([氏名], len([氏名]) - charindex(' ', [氏名])))
		else coalesce(nullif(trim(right([氏名], len([氏名]) - len(left([氏名], 2)))),''),'Last name') end as [candidate-Lastname]
	, [フリガナ] as [candidate-firstNameKana]
	, [フリガナ] as [candidate-lastNameKana]
/* USING PYTHON TO TRANSFORM TO ROMAJI
	, [フリガナ]
...
*/
	...
	, u.EmailAddress as [candidate-owners]
