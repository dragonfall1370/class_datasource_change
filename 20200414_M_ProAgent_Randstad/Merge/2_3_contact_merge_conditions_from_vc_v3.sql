...
, vc_contact as (select c.company_id 
		...
		--	else NULL end as vc_contact_email --only used in review2 for alternative email
		, case when c.email like '%_@_%.__%' then c.email else NULL end as vc_contact_email --used on production
		, c.insert_timestamp
...
, pa_vc_email_name as (select pa.company_id
	...
	) --select * from pa_vc_email_name --1736 rows
...
, pa_vc_email_kana as (select pa.company_id
	...
	) --select * from pa_vc_email_kana --2 rows
...
, pa_vc_name_kana as (select pa.company_id
	...
	where pa.company_id = vc.company_id
	and pa.contact_id not in (select contact_id from pa_vc_email_name)
	) --select * from pa_vc_name_kana --32 rows
...
, pa_vc_name as (select pa.company_id
	...
	where pa.company_id = vc.company_id
	and pa.contact_id not in (select contact_id from pa_vc_email_name)
	) --select * from pa_vc_name --792 rows
...
, pa_vc_name_rn as (select *
	...
	from pa_vc_name) --select * from pa_vc_name_rn --792
