...
where m.vc_pa_company_id not in (select vc_pa_company_id from mike_tmp_company_dup_check)
...
join company_location cl on cl.company_id = m.vc_pa_company_id
where m.vc_pa_company_id not in (select vc_pa_company_id from mike_tmp_company_dup_check) --1569 rows