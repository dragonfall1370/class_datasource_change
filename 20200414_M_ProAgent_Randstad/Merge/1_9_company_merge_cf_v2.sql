...
with latest_company as (select *
	...
	, 1001 form_id
	, 11275 field_id
	from (select * from mike_tmp_company_dup_check) a
	where 1=1
	and rn = 1
...
with latest_company as (select m.vc_company_id
	...
	where 1=1
	and rn = 1
	and coalesce(vc_pa_update_date, vc_pa_reg_date) > coalesce(vc_latest_date, vc_reg_date, '1900-01-01') -- rows
	and a.field_value is not NULL and a.field_value <> ''
)
...
with latest_company as (select m.vc_company_id
	...
	, 'add_com_info' additional_type
	, 1001 form_id
	, 1011 field_id --replace with 1004, 1114, 1007, 1008, 1010, 1011, 11331
	from mike_tmp_company_dup_check m
	join (select * from additional_form_values where form_id = 1001 and field_id = 1011) a on a.additional_id = m.vc_pa_company_id
	where 1=1
	and rn = 1
	and coalesce(vc_pa_update_date, vc_pa_reg_date) > coalesce(vc_latest_date, vc_reg_date, '1900-01-01') -- rows
	and a.field_value is not NULL and a.field_value <> ''
