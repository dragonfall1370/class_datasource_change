...
from candidate c
join cand_brand cb on cb.candidate_id = c.id --general condition
...
where 1=1
and c.active in (0, 1) --status: active or passive --general condition
...
--and c.id in (select distinct candidate_id from int_1month) --12-filter conditions
and c.id in (select distinct candidate_id from placed_condition) --13-filter conditions
order by c.id