...
, placements as (select id
	...
	, row_number() over(partition by position_candidate_id order by insert_timestamp, valid desc, id) as rn --get the oldest | changed req on 2020-10-30
	from offer)
...
, job_placed as (select pc.id as job_app_id
		...
		, pc.placed_date
		, row_number() over(partition by pc.position_description_id order by pc.placed_date asc) as rn --get the oldest
...
, job_placed_3day as (select *
		...
		--and (placed_date + interval '12 days')::date = now()::date --test export date
...
, con_owner as (select c.id as contact_id
	...
	from job_placed_3day ji --using filter table instead
...
--, j.placement_id as "Placement ID"
--, j.placed_date as "Placement Booked Date"
--, c.ownerId
, nps.code_split as "NPS_Branch_Code"
--, nps.team_name --test team name
--, nps.name --test consultant name
--, nps.rn
from con_owner_rn c
join (select * from user_team_nps where rn=5) nps on nps.user_id = c.user_id --NPS Event 5 with rn=5
where 1=1
