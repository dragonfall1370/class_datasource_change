...
, placement_profit as (select pd.company_id
	, sum(coalesce(o.projected_profit, o.profit, 0))
	, round(sum(coalesce(o.projected_profit, o.profit, 0)::int), 1) as placement_profit --sum up based on job
	from offer o
...
select 
	...
	, coalesce(pp.placement_profit, 0)::float as "Total client fee amount (GP)"
from company c
