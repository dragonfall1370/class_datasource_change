...
--EXCLUDED BRAND: Staffing
, contact_brand_ex as (select tgc.contact_id
	, tg.name
	from team_group_contact tgc
	left join (select * from team_group where group_type = 'BRAND') tg on tg.id = tgc.team_group_id
	where tg.name = 'Staffing'
	)
...
, case when cf_11344.field_value = '1' then 'YES'
		when cf_11344.field_value = '2' then 'NO'
		else '' end as "Survey Completed"
, cf_11345.field_date_value "Survey Completed On"
, cf_11346_value "1. コロナ禍が貴社の事業全体にもたらした影響"
, cf_11347.field_value "Survey Completed By (consultant email address)"
, cf_11348_value "2. コロナ禍の影響をうけ、現在、貴社が直面し"
, cf_11361.field_value "2. その他（自由回答）"
, cf_11350_value "3. コロナ禍により生じた財政的な課題を解決す"
, cf_11362.field_value "3. その他（自由回答）"
, cf_11353_value "4. 新型コロナウイルス感染拡大をうけ発令され"
, cf_11363.field_value "4. その他（自由回答）"
, cf_11357_value "5. 様々な制限が緩和され、ニューノーマルに基"
, cf_11364.field_value "5. その他（自由回答）"
, cf_11354_value "６．今後の見通しについてお尋ねします。 　"
, cf_11355_value "６－２．採用等により、人員の補充が必要(見"
, cf_11356_value "６－３．採用等により、人員の補充が必要(見"
, cf_11358_value "６－４　採用または補充予定の人数総数はどの"
, cf_11351_value "7. 回復・成長期の人材活用において、課題とし"
, cf_11352.field_value "7.  その他　（自由回答）"
, cf_11349_value "8. 現在の課題を解決するサポートとして、ラン"
...
left join (select * from additional_form_values where field_id = 11344) cf_11344 on cf_11344.additional_id = c.id --not filter with survey completed
where c.deleted_timestamp is NULL
and c.id not in (select contact_id from contact_brand_ex) --exclude Staffing brand
and c.insert_timestamp::date >= '2019-07-01'
--and c.id = 15810
order by c.id