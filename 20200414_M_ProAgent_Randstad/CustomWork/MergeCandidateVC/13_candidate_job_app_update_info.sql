...
, update_info as (select pc.id
	...
	where pc.status >=200
	and pc.candidate_id in (select candidate_id from cand_merged) --check all applications from slave/master candidates
...
where 1=1 --limit the number of offers from update_info


---UPDATE OFFER_PERSONAL_INFO IF ANY CHANGES ON JOB/COMPANY/CONTACT
