 drop table if exists VCConACEmails
			...
			SELECT userMessageID
				, STUFF(
					(
						...
						order by [dbo].[ufn_RemoveForXMLUnsupportedCharacters](name) asc
						FOR XML PATH (''), TYPE
...
select --top 200
        ...
        , concat('From: ' , convert(nvarchar(max), externalFrom, 120) , char(10) ,
                   ...
                   'Body: ' , cast(email_content as varchar(max)) , char(10) ,
                   'Comments: ' , cast(comments as varchar(max)) , char(10) ,
...
from EmailBody eb
left join bullhorn1.BH_UserContact o ON o.userID = eb.sendingUserID
--where comments <> ''
