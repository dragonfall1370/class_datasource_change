...
, note as (
	SELECT
		...
		, Stuff(
            ...
            + Coalesce('LTD Name: ' + NULLIF(cast(ca.customText3 as varchar(max)), '') + char(10), '')
			+ Coalesce('Status: ' + NULLIF(cast(ca.status as varchar(max)), '') + char(10), '')
			+ Coalesce('GitHub Contact: ' + NULLIF(cast(ca.customText14 as varchar(max)), '') + char(10), '')
			+ Coalesce('General Comments: ' + NULLIF(convert(varchar(max), CA.comments), '') + char(10), '')
			+ Coalesce('Latest Comment: ' + NULLIF(convert(varchar(max), lc.comments), '') + char(10), '')
	...
	left join ( select userid, firstname, lastname from bullhorn1.BH_UserContact )UC ON UC.userID = CA.referredByUserID
       ...
       left join (select * from VC_Countries) t on CA.secondaryCountryID = t.code
	--left join VC_Countries on cast(CA.desiredLocations as varchar(2)) = VC_Countries.ABBREVIATION
...
select --top 5
	...
	, concat('', CONVERT(VARCHAR(10),C.dateOfBirth,120)) as 'candidate-dob'
	, iif(ed.rn > 1,concat(ed.rn, '-', ed.email), iif(ed.email = '' or ed.email is null, concat('candidate_',cast(C.userID as varchar(max)),'@noemailaddress.co'),ed.email) ) as 'candidate-email'
	, e2.email as 'candidate-PersonalEmail' -->>
	
	, trim(isnull(C.mobile, '')) as 'candidate-phone'
	, trim(isnull(C.mobile, '')) as 'candidate-mobile'
	, trim(isnull(C.phone, '')) as 'candidate-homePhone'	
    ...
    , isnull(ltrim(Stuff( Coalesce(NULLIF(SN.SkillName, '') + char(10), '')
	...
	, isnull(eh.eh, '') as 'candidate-workHistory'
	, isnull(es.es, '') as 'candidate-education'
...
left join VC_Countries tc ON c.countryID = tc.code
...
OFFSET     6000 ROWS       -- skip N rows
FETCH NEXT 2000 ROWS ONLY; -- take M rows
