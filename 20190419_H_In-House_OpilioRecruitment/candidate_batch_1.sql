...
candidate_position AS (
	SELECT
	DISTINCT person_id candidate_id
	FROM alpha_position
	WHERE jobFunction_id IS NOT NULL
)
...
-- 	ROW_NUMBER() OVER(PARTITION BY c.person_id, p.lastName, c.email ORDER BY c.person_id) rn,
	...
	COALESCE(
		CASE 
			WHEN a.countryCode IN ('SCT', 'WLS', 'UK') THEN 'GB'
			WHEN a.countryCode = 'NIR' THEN 'IE'
		END, 
...
JOIN candidate_position cp ON c.person_id = cp.candidate_id
...
LEFT JOIN candidate_second_nationality sn ON c.id = sn.candidate_id
