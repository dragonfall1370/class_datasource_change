...
select
        ...
        , FromSalary as 'FromSalary' -- INJECTION
        , ToSalary as 'ToSalary' -- INJECTION
        ...
        , Stuff( 
                ...
                + Coalesce('Stage Reached: ' + NULLIF(cast(sr.Description as varchar(max)), '') + char(10), '') --v.StageReached
                , 1, 0, '') as 'contact-note'        
-- select count(*) --2658 -- select distinct v.VacancyType --select distinct v.Currency -- select top 1000 * 
from vacancies v
...
left join (select Code, Description from CodeTables where Code in ('1st','2nd','3rd','CVS','FINT','MAT','OFF','PLA') and TabName = 'Match Status') sr on sr.Code = v.StageReached

/*
select  v.UniqueID as 'position-externalId' 
        , FromSalary as 'FromSalary' -- INJECTION
        , ToSalary as 'ToSalary' -- INJECTION
from vacancies v
*/
