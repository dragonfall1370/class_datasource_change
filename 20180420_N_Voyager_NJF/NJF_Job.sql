...
, iif(j.intJobId in (select intJobId from dup where dup.rn > 1)
	, iif(dup.vchClientJobTitle = '' or dup.vchClientJobTitle is NULL,concat('NJF Search - No job title - ID',dup.intJobId),concat(dup.vchClientJobTitle,' - ', dup.intJobId))
	, iif(j.vchClientJobTitle = '' or j.vchClientJobTitle is null,concat ('NJF Search - No job title - ID',j.intJobId),j.vchClientJobTitle)) as 'position-title'
...
, left(
	concat('Job External ID: NJFS',j.intJobId,char(10)
