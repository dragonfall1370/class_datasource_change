with tempLocation as (select ct.intCompanyTierId
	...
	, Stuff(
			...
			, 1, 1, '') as 'fullAddress'
from dCompanyTier ct left join refCountry rc on ct.sintCountryId = rc.sintCountryId)
...
select concat('NJFS',intCompanyId) as externalID
		...
		, ltrim(concat(replace(replace(replace(replace(fullAddress,',,',','),'Lo, ndon','London'),'"',''),'  ',''),iif(Num = '' or Num is null, '',iif(fullAddress = '' or fullAddress is null,concat('Ph. ',Num),concat(' - Ph. ',Num))))) as address
		...
		, concat('NJFS',tl.intCompanyTierId) as intCompanyTierId
		, vchTown as City
		, current_timestamp as insertTimestamp
from tempLocation tl left join TierPhone tp on tl.intCompanyTierId = tp.intCompanyTierId--where fullAddress is not null
--where intCompanyId in (2,455)--,1887,4546,6397,1436,6251,1670,2054,6275,499,5081,6676,5508,592,6504,6803,6488,2)
order by intCompanyId