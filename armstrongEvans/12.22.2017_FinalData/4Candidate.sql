...
, his as (
        ...
        , ch.Address, ch.Phone, ch.Email, ch.Checked, ch.current_
...
--select * from his where convert(varchar(max),[candidate_externalId]) = '10030'
, whis as (
        SELECT cast([candidate_externalId] as varchar(max)) as 'candidate_externalId'
                , whis = STUFF(( 
                                SELECT char(10) + 'Date Range: ' + cast([Date Range] as varchar(max)) + char(10) --Coalesce('Date Range: ' + NULLIF(cast([Date Range] as nvarchar(max)) , ''), '') 
                                                + 'To: ' + replace(cast([To] as varchar(max)),'1900-01-01','')  + char(10) --Coalesce('To: ' + NULLIF(cast([To] as nvarchar(max)) , ''), '') 
                                ...
                                FROM his b 
                                WHERE cast([candidate_externalId] as varchar(max)) = cast(a.[candidate_externalId] as varchar(max)) and b.rn > 3 order by b.rn asc --and cast([candidate_externalId] as varchar(max)) <> '' 
...
, his1.JobTitle as 'candidate-jobTitle1' , his1.EmployerName as 'candidate-employer1', his1.EmployerName as 'candidate-company1', replace(his1.[Date Range],'1900-01-01','') as 'candidate-startDate1' ,replace(cast(his1.[To] as varchar(max)),'1900-01-01','') as 'candidate-enddate1'
, his2.JobTitle as 'candidate-jobTitle2' , his2.EmployerName as 'candidate-employer2', his2.EmployerName as 'candidate-company2', replace(his2.[Date Range],'1900-01-01','') as 'candidate-startDate2', replace(cast(his2.[To] as varchar(max)),'1900-01-01','') as 'candidate-enddate2'
, his3.JobTitle as 'candidate-jobTitle3' , his3.EmployerName as 'candidate-employer3', his3.EmployerName as 'candidate-company3', replace(his3.[Date Range],'1900-01-01','') as 'candidate-startDate3', replace(cast(his3.[To] as varchar(max)),'1900-01-01','')  as 'candidate-enddate3'
, whis.whis as 'candidate-workhistory'
	-- select count (*) --12791
	-- select distinct convert(varchar(max),c.candidate_Country)
	...
	-- select distinct convert(varchar(max),c.source)
	-- select distinct convert(varchar(max),c.status)
