...
-- MAIL
------------
  mail1 (ID,email) as (select cast(c.contact_externalId as varchar(100)), replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace( convert(varchar(100),lower(c.contact_email)),'/',' '),'<',' '),'>',' '),'(',' '),')',' '),':',' '),'.@','@'),'@.','@'),'+',' '),'&',' '),'[',' '),']',' '),'?',' '),'''',' '),';',' ') as email from ContactsImportAutomappingTemplate c where convert(varchar(100),c.contact_email) <> '')
--select * from mail1
...
, mail4 (ID,email,rn) as ( SELECT ID, email = ltrim(rtrim(CONVERT(varchar(100), email))), r1 = ROW_NUMBER() OVER (PARTITION BY ID ORDER BY ID desc) FROM mail3 )
...
--select * from mail1 where email like '%cleehill%'
select * from ed where email like '%cleehill%'
select contact_externalId from ContactsImportAutomappingTemplate where email like '%cleehill%'
select contact_externalId from ContactsImportAutomappingTemplate c group by c.contact_externalId having count(*) > 1
*/
...
-- select count(*) --4389 -- select top 10 * -- select distinct convert(varchar,c.contact_owners) -- select distinct convert(varchar,c.contact_email)
from ContactsImportAutomappingTemplate c --where  convert(varchar,c.contact_email) <> '' --3769
