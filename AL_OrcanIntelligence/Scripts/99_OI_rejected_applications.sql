drop table if exists [dbo].[VCRejectedApplications];

with abc as (
	select [application-candidateExternalId], [application-positionExternalId], [application-stage], aso.[Order] as StageOrder
		, apps.RejectedDate, apps.AssociatedDate, LastModifiedById from (
		select
		...
		case(upper(trim(isnull(ja.AVTRRT__Stage__c, ''))))
			...
			when upper('Rejected by Account Manager') then 'SHORTLISTED'--'SHORTLISTED > REJECTED'
			when upper('Rejected by Candidate') then 'SHORTLISTED'--'SHORTLISTED > REJECTED'
			when upper('Rejected by Hiring Manager') then 'SHORTLISTED'--'SHORTLISTED > REJECTED'
		...
		, cast(ja.LastModifiedDate as datetime) as RejectedDate
		, cast(ja.CreatedDate as datetime) as AssociatedDate
		, ja.LastModifiedById
		--, row_number() over(partition by 

		from
		AVTRRT__Job_Applicant__c ja
		where upper(trim(isnull(ja.AVTRRT__Stage__c, ''))) in (
...
select distinct t1.[application-candidateExternalId], t1.[application-positionExternalId], t1.[application-stage]
	, t1.AssociatedDate, t1.RejectedDate, t1.LastModifiedById

into [dbo].[VCRejectedApplications]
...
select * from [dbo].[VCRejectedApplications]
