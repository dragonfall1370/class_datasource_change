...
--MAIN SCRIPT
select 
iif(C.CompanyID = '' or C.CompanyID is NULL,'MCIndo9999999',concat('MCIndo',C.CompanyID)) as 'contact-companyId'
, C.CompanyID as '(OriginalCompanyID)'
, C.Company as '(OriginalCompanyName)'
, concat('MCIndo',C.DynamicDataId) as 'contact-externalId'
...
, stuff((coalesce(', ' + C.Tel,'') + coalesce(', ' + C.Mobile,'') + coalesce(', ' + C.Mobile2,'')),1,1,'') as 'contact-phone'
