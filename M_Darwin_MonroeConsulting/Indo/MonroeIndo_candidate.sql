...
--MAIN SCRIPT
select concat('MCIndo',C.DynamicDataId) as 'candidate-externalId'
...
, case
	...
	else concat('candidate_ID-',C.DynamicDataId,'@noemail.com') end as 'candidate-email'
, CWE.CandidateEmail as 'candidate-workEmail'
, convert(varchar(10),C.DateofBirth,120) as 'candidate-dob'
, C.Mobile2 as 'candidate-phone'
, C.Mobile2 as 'candidate-mobile'
...
, concat(coalesce('Current Company: ' + C.CurrentCompany + char(10),''),coalesce('Current JobTitle: ' + C.CurrentJobTitle + char(10),'')
	, coalesce('Current salary: ' + cast(C.Salary as varchar(max)) + char(10),'')
