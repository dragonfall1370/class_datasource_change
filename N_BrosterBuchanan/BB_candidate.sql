...
from PROP_EDU_ESTAB)
 ...
 , tempEducation1 as(
	...
	, iif(From_Date is NULL,'',concat(char(10),'From Date: ',convert(varchar(10),From_Date, 120)))
	, iif(To_Date is NULL,'',concat(char(10),'To Date: ',convert(varchar(10),To_Date, 120)))
...
from  PROP_PERSON_GEN pg left join PROP_ASSIG_GEN ag on pg.EMPLOYMENT = ag.REFERENCE
							...
							left join ENTITY_TABLE et on cp.REFERENCE = et.ENTITY_ID
...
where ag.REFERENCE is not null and cp.REFERENCE is not null and et.CREATEDDATE > '2017-04-30')

-------------------------------------------------MAIN SCRIPT
select
	-- pg.REFERENCE as 'candidate-externalId'
	concat('BB',cp.REFERENCE) as 'candidate-externalId'
	...
	, iif(cem.email = '' or cem.email is null, concat('CandidateID-',cast(cp.REFERENCE  as varchar(max)),'@noemail.com'),cem.email) as 'candidate-email'
...
from PROP_CAND_PREF cp left join PROP_PERSON_GEN pg on cp.REFERENCE = pg.REFERENCE--23170 rows
left join PROP_CAND_GEN cg on cp.REFERENCE = cg.REFERENCE
left join title on cp.REFERENCE = title.REFERENCE
left join gender on cg.REFERENCE = gender.REFERENCE
--left join dob on cp.REFERENCE = dob.REFERENCE
left join WorkEmail we on cp.REFERENCE = we.REFERENCE
left join homeEmail he on cp.REFERENCE = he.REFERENCE
left join CandEmail cem on cp.REFERENCE = cem.REFERENCE
left join e1 ON cp.REFERENCE = e1.ID -- candidate-email
left join e2 ON cp.REFERENCE = e2.ID -- candidate-email
left join e3 ON cp.REFERENCE = e3.ID -- candidate-email
left join e4 ON cp.REFERENCE = e4.ID -- candidate-email
left join (SELECT REFERENCE, EMAIL_ADD from PROP_EMAIL where OCC_ID = 100000217) emailh2 on cp.REFERENCE = emailh2.REFERENCE
left join owner ON cp.REFERENCE = owner.PROP_PERSON_GEN_REFERENCE
...
left join doc on cp.REFERENCE = doc.OWNER_ID
...
left join address1 ON cp.REFERENCE = address1.REFERENCE
left join address2 ON cp.REFERENCE = address2.REFERENCE
...
left join (select REFERENCE, TEL_NUMBER from PROP_TELEPHONE TEL INNER JOIN MD_NAMED_OCCS OCC ON OCC.OCC_ID= TEL.OCC_ID where CONFIG_NAME = 'Home') telhome on cp.REFERENCE = telhome.REFERENCE --candidate-homePhone
left join (select REFERENCE, TEL_NUMBER from PROP_TELEPHONE TEL INNER JOIN MD_NAMED_OCCS OCC ON OCC.OCC_ID= TEL.OCC_ID where CONFIG_NAME = 'Mobile') telmobile on cp.REFERENCE = telmobile.REFERENCE --candidate-mobile
left join (select REFERENCE, TEL_NUMBER from PROP_TELEPHONE TEL INNER JOIN MD_NAMED_OCCS OCC ON OCC.OCC_ID= TEL.OCC_ID where CONFIG_NAME = 'Work') telwork on cp.REFERENCE = telwork.REFERENCE --candidate-phone & candidate-workPhone
left join (select PERSON_GEN.REFERENCE, EMPLOYEE.NAME as ConsName from PROP_PERSON_GEN PERSON_GEN INNER JOIN PROP_OWN_CONS CONS ON PERSON_GEN.REFERENCE = CONS.REFERENCE INNER JOIN MD_NAMED_OCCS OCC ON OCC.OCC_ID= CONS.OCC_ID INNER JOIN PROP_EMPLOYEE_GEN EMPLOYEE ON EMPLOYEE.USER_REF = CONS.CONSULTANT where CONFIG_NAME = 'Permanent') consName on cp.REFERENCE = consname.REFERENCE
...
left join skills ON cp.REFERENCE = skills.REFERENCE -- candidate-email & candidate-workEmail
...
left join Nationality nal on cp.REFERENCE = nal.REFERENCE
...
left join createdUpdatedInfo et on cp.REFERENCE = et.ENTITY_ID
left join contactedInfo ci on cp.REFERENCE = ci.REFERENCE 
left join curEmployer ce on cp.REFERENCE = ce.REFERENCE
left join employment emp on cp.REFERENCE = emp.canID
left join education e on cp.REFERENCE = e.REFERENCE
