...
, Documents as (select candidate_id, doc_id, concat(doc_id,'_',replace(replace(doc_name,',',''),'.',''),rtrim(ltrim(doc_ext))) as docfilename 
	...
	-- and doc_ext not like '%msg%'
	) -- 12972 rows without extension
...
, CandidateWorkHistory as (select job_candidate, job_id
	, concat(
--	  coalesce('From: ' + nullif(ltrim(rtrim(convert(varchar(10),job_from,120))),'') + char(10),NULL)
...
--EMAIL DUPLICATION
, CandidateEmail as (SELECT cn_id, replace(replace(ltrim(rtrim(CONVERT(NVARCHAR(MAX), REPLACE(cn_cont_email, char(9), '')))),CHAR(13),''),CHAR(10),'') as CandidateEmail
	, ROW_NUMBER() OVER (PARTITION BY replace(replace(ltrim(rtrim(CONVERT(NVARCHAR(MAX), REPLACE(cn_cont_email, char(9), '')))),CHAR(13),''),CHAR(10),'') ORDER BY cn_id desc) as rn
	FROM tblCandidate
...
, candidate_duplication_info as (
	select
  cn_id
  , cn_fname
  , cn_lname
  ,  CONVERT(VARCHAR(19), cn_datecreated, 120) as cn_datecreated
from tblCandidate where cn_id in (
  SELECT
    cn_duplicate_id
  FROM tblCandidate
  WHERE cn_isduplicate > 0
)
)

--MAIN SCRIPT
select --top 100
	concat('RLC',c.cn_id) as 'candidate-externalId'
	...
	, case 
		...
		else NULL end as 'candidate-citizenship' -- TODO: check if any nation name has not listed here 
	, n.nat_name as OriginalNationality -------------------------------------- this field is not in the requirement, why it can be here, the import wizard will ignore ??
	...
	, coalesce(rtrim(ltrim(c.cn_present_position)),rtrim(ltrim(c.cn_latest_position))) as 'candidate-jobTitle1' -- Current Job Title
	, ltrim(rtrim(c.cn_cont_mobile)) as 'candidate-phone' --Personal Phone, Primary Phone
	, ltrim(rtrim(c.cn_cont_mobile)) as 'candidate-mobile' --mobile phone
	, ltrim(rtrim(c.cn_cont_home)) as 'candidate-homePhone'
	
	-- TODO: convert to string_agg
	, ltrim(stuff((
	  coalesce(' ' + nullif(rtrim(ltrim(c.cn_address_street)),''),'') + coalesce(', ' + nullif(rtrim(ltrim(c.cn_address_subdist)),''),'') 
		+ coalesce(', ' + nullif(rtrim(ltrim(c.cn_address_dist)),''),'') + coalesce(', ' + nullif(rtrim(ltrim(p.pro_name)),''),'') 
		+ coalesce(', ' + nullif(rtrim(ltrim(left(replace(cn_address_postalcode, char(13), ''), 5))),''),'')),1,1,'')) as 'candidate-address'
	, nullif(rtrim(ltrim(left(replace(cn_address_postalcode, char(13), ''), 5))),'') as 'candidate-zipCode'
...
--	, nullif(rtrim(ltrim(c.cn_address_postalcode)),'') as 'candidate-zipCode'
	...
	, case 
		...
		,concat('candidate-',c.cn_id,'@noemail.com')) 
		end as 'candidate-email'
	, nullif(ltrim(rtrim(c.cn_cont_email2)),'') as 'candidate-workEmail'
		-- =========================================================================> TODO: REVIEW TO HERE
	
, ltrim(rtrim(
case when c.cn_import_email in (select usr_email from tblUser) then c.cn_import_email else null end
)) as 'candidate-owners'
...
, ct.CandidateTags as 'candidate-skills'
...
--NOTES
, concat('External ID: ',c.cn_id,char(10)
	, coalesce('Duplicate External ID: ' + nullif(ltrim(rtrim(cddupif.cn_id)),'') + char(10),'')
	, coalesce('Duplicate Name: ' + concat(nullif(ltrim(rtrim(cddupif.cn_fname)),''), nullif(ltrim(rtrim(cddupif.cn_lname)),'')) + char(10),'')
	, coalesce('Duplicate Candidate Created Date: ' + nullif(ltrim(rtrim(cddupif.cn_datecreated)),'') + char(10),'')
	) as 'candidate-note'
from tblCandidate c
...
left join candidate_duplication_info cddupif on c.cn_id=cddupif.cn_id
where c.cn_show = 'Y'
order by c.cn_id
