...
select
  concat('NJFGTP',C.intCompanyId) as 'company-externalId'
, C.vchCompanyName as '(OriginalName)'
, iif(C.intCompanyId in (select intCompanyId from dup where dup.rn > 1)
	, iif(dup.vchCompanyName = '' or dup.vchCompanyName is NULL,concat('NJF GTP - Default Company - ID',dup.intCompanyId),concat('NJFGTP-',dup.rn,' - ',dup.vchCompanyName))
	, iif(C.vchCompanyName = '' or C.vchCompanyName is null,concat('NJF GTP - Default Company - ID',C.intCompanyId),C.vchCompanyName)) as 'company-name'
...
, ca.companyAttachments as 'company-document'
, left(Concat(
			'Company External ID: NJFGTP', C.intCompanyId,char(10),
...
UNION ALL
select 'NJFGTP9999999','','NJF GTP - Default Company','','','','','','','This is Default Company from Data Import'