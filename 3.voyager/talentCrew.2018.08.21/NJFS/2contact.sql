...
with 
tempContacts as (select c.intContactId, c.vchStandardRefCode, c.intPersonId, intCompanyTierContactId, ctc.intCompanyTierId, ct.intCompanyId, ctc.vchJobTitle, c.datLastContacted
		, ctc.vchNote, ctc.bitActive, ctc.intPreferredTelecomId 
		, ctc.tintContactPriorityId , ct.vchTown
		, ROW_NUMBER() OVER(PARTITION BY ctc.intContactId ORDER BY intCompanyId ASC) AS rn
...
--select count(*) from tempContacts --2566
--select top 10 * from tempContacts tc where tc.intCompanyId in (5559,6254) or tc.intCompanyTierContactId in (33316,80764,42925,63799,66515,64760,64117,63444,22063,65710,71772,72976,72624,73326,76366,79166,74484,73031,83887)
...
, contactConsultant0 (intCompanyTierContactId, consultantEmail) as (
       select intCompanyTierContactId, u.vchEmail as consultantEmail
...
--, contactConsultant_rn (intCompanyTierContactId,consultantEmail,rn) as ( SELECT intCompanyTierContactId, consultantEmail = ltrim(rtrim(CONVERT(NVARCHAR(MAX), consultantEmail))), r1 = ROW_NUMBER() OVER (PARTITION BY intCompanyTierContactId ORDER BY intCompanyTierContactId desc) FROM contactConsultant )
--select * from contactConsultant_rn where intCompanyTierContactId = 694

, contactConsultant (intCompanyTierContactId, consultantEmail) as (
       SELECT a.intCompanyTierContactId, STUFF((
                     select
                     stuff(
                       Coalesce(',' + NULLIF(cast(consultantEmail as varchar(max)), ''), '')
                     , 1, 0, '') as wh
                     from contactConsultant0
       WHERE intCompanyTierContactId = a.intCompanyTierContactId
       FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '') AS es 
       FROM contactConsultant0 as a
       GROUP BY a.intCompanyTierContactId 
       )
-- select * from contactConsultant where intCompanyTierContactId = 694
...
---- WORK HISTORY
, wh (intPersonId, wh) as (
       SELECT a.intPersonId, STUFF(( 
                     select char(10) + 
                     stuff(
                       Coalesce('Line Manager: ' + NULLIF(cast(vchLineManager as varchar(max)), '') + char(10), '')
                     + Coalesce('Job Type: ' + NULLIF(cast(tintJobType as varchar(max)), '') + char(10), '')
                     + Coalesce('Start Date: ' + NULLIF( iif( cast(datStartDate as varchar(50)) = '1900-01-01', '', cast(datStartDate as varchar(50)) ), '') + char(10), '')
                     + Coalesce('End Date: ' + NULLIF(cast(datEndDate as varchar(max)), '') + char(10), '')
                     + Coalesce('Salary Currency: ' + NULLIF(cast(tintSalaryCurrencyId as varchar(max)), '') + char(10), '')
                     + Coalesce('Notes: ' + NULLIF(cast(vchNote as varchar(max)), '') + char(10), '')
                     , 1, 0, '') as wh
                     from dWorkHistory
       WHERE intPersonId = a.intPersonId order by datStartDate desc
       FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '') AS es 
       FROM dWorkHistory as a
       --where intPersonId in (54)
       GROUP BY a.intPersonId 
       )
--select * from wh where intPersonId in (11);
--select * from wh where intPersonId in (164043);
--select * from dWorkHistory


, dWorkHistory_decSalary_note (intPersonId, sal) as (
       SELECT a.intPersonId, STUFF(( 
                     select
                     stuff( Coalesce('Salary: ' + NULLIF(cast(decSalary as varchar(max)), '') + char(10), '') , 1, 0, '') as sal
                     from dWorkHistory
       WHERE intPersonId = a.intPersonId order by datStartDate desc
       FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 0, '') AS es 
       FROM dWorkHistory as a
       --where intPersonId in (54)
       GROUP BY a.intPersonId 
)
--select intPersonId, sal from dWorkHistory_decSalary_note where intPersonId = 202 group by intPersonId having count(*)


, dCompanyTierContactTelecom_note (intContactId,note) as (
       SELECT a.intContactId, STUFF(( 
                     select char(10) + 
                     stuff(
                          Coalesce('Telecoms - Value: ' + NULLIF(cast(vchValue as varchar(max)), '') + char(10), '')
                       + Coalesce('Telecoms - Ext: ' + NULLIF(cast(vchExtension as varchar(max)), ''), '')
                     , 1, 0, '') as sal
                     from dCompanyTierContactTelecom
       WHERE intContactId = a.intContactId
       FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '') AS note
       FROM dCompanyTierContactTelecom as a
       --where intPersonId in (54)
       GROUP BY a.intContactId 
)
--select * from dCompanyTierContactTelecom_note where intContactId = 202
...
select --top 50
       ...
       , iif(p.vchForename = '' or p.vchForename is NULL,'Firstname',p.vchForename) as 'contact-firstName'
       , iif(p.vchSurname = '' or p.vchSurname is NULL,concat('LastName-', tc.intCompanyTierContactId),p.vchSurname) as 'contact-lastName'
       ...
       , cm.mobile as 'contact-mobilephone'
...
, Stuff( 
       +  Coalesce('Contact Priority (PTY): ' + NULLIF(cast(tc.tintContactPriorityId as varchar(max)), '') + char(10), '')
       +  Coalesce('Location ID: ' + NULLIF(cast(tc.intCompanyTierId as varchar(max)), '') + char(10), '')
       +  Coalesce('Location City: ' + NULLIF(cast(tc.vchTown as varchar(max)), '') + char(10), '')
       +  Coalesce('Last Contacted Date: ' + NULLIF(cast(tc.datLastContacted as varchar(max)), '') + char(10), '')
       +  Coalesce('Known As: ' + NULLIF(cast(p.vchKnownAs as varchar(max)), '') + char(10), '')
       +  Coalesce(NULLIF(cast(ctctn.note as varchar(max)), '') + char(10), '')
      +  Coalesce(NULLIF(cast(whdn.sal as varchar(max)), '') + char(10), '')       
      +  Coalesce('Work History: ' + NULLIF(cast(wh.wh as varchar(max)), '') + char(10), '')       
, 1, 0, '') as 'contact-note'
/*, left(
...
from tempContacts tc left join dPerson p on tc.intPersonId = p.intPersonId
					 ...
					 left join wh on wh.intPersonId= tc.intPersonId
					 left join dCompanyTierContactTelecom_note ctctn on ctctn.intContactId = tc.intContactId
					 left join dWorkHistory_decSalary_note whdn on whdn.intPersonId = tc.intPersonId
--where len(cam.contactAttachments) > 32000
--where tc.intCompanyTierContactId = 694-- and
...
/*
50	lEventCompanyTierContact	vchEventDetail	EVENTS > Details	YES	Note
52	lAttachmentEvent	intAttachmentId	EVENTS > Attachments	YES	Note

select 
c.intCandidateId
--ctc.intCompanyTierContactId, ae.intAttachmentId
, e.vchEventDetail
, concat(a.vchAttachmentName,a.vchFileType) as att
-- select top 10 *
from lEventCompanyTierContact ectc 
--left join dCandidate c on ectc.intCandidateId = c.intCandidateId
left join dEvent e on ectc.intEventId = e.intEventId
left join lCompanyTierContact ctc on ectc.intContactId = ctc.intContactId and ectc.intCompanyTierId = ctc.intCompanyTierId
left join lAttachmentEvent ae on ectc.intEventId = ae.intEventId
left join dAttachment a on ae.intAttachmentId = a.intAttachmentId
--left join email em on ae.intAttachmentId = em.AttachmentID
