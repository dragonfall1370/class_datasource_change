...
, tphone1 as (
       select intCompanyId, vchValue, replace(iif(vchExtension <> '', concat(vchNumber,vchExtension), vchNumber), char(0x0003),'') as vchNumber
...
, tphone2 as (
       select ct.intCompanyId, vchValue, iif(vchExtension <> '', concat(vchNumber,vchExtension, ' (',ct.vchCompanyTierName, ')'), concat(vchNumber, ' (', ct.vchCompanyTierName, ')')) as vchNumber --ctt.intCompanyTierId, intCompanyTierTelecomId
...
, mainTierPhone as (
       SELECT intCompanyId
       , STUFF( (SELECT ',' + vchNumber from  tphone2 WHERE intCompanyId = T.intCompanyId order by intCompanyId asc FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '')  AS vchNumber
       FROM tphone2 as T
       GROUP BY T.intCompanyId)

, Phone as (
       select c.intCompanyId, coalesce(t1.vchNumber, mtp.vchNumber) as Num
...
--GET FAX
, Fax as (select intCompanyId, vchValue as fax from dCompanyTelecom where vchDescription = 'Fax')

--Duplicate Company Name
, dup as (select intCompanyId, ltrim(rtrim( iif(vchCompanyName = '' or vchCompanyName is null,'No CompanyName',vchCompanyName) )) as vchCompanyName , ROW_NUMBER() OVER(PARTITION BY ltrim(rtrim( iif(vchCompanyName = '' or vchCompanyName is null,'No CompanyName',vchCompanyName) )) ORDER BY intCompanyId ASC) AS rn from dCompany)
--select * from dup
--select count(distinct intCompanyId) from dCompany
...
, TierPhone as (
       ...
       , STUFF( (SELECT ',' + vchNumber from  TierPhone1 WHERE intCompanyTierId = T.intCompanyTierId order by intCompanyTierId asc FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '')  AS Num
       FROM TierPhone1 as T
       GROUP BY T.intCompanyTierId)

, tempLocationNote as (
       select ct.intCompanyTierId, ct.intCompanyId--, ct.vchCompanyTierName, ct.vchNote as Description, replace(vchNumber, char(0x0003),'')
		, concat(
...
, LocationNote as (
       SELECT intCompanyId, 
            STUFF(
                (SELECT char(10) + char (10) + replace(replace(replace(replace(locationName, char(0x0017),''),char(0x0003),''),char(0x001B),''),char(0x0002),'') from  tempLocationNote WHERE intCompanyId =tln.intCompanyId order by intCompanyId asc FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'),2,1, '')  AS locationNote
...
, tempConsultant as (select cc.intConsultantId, cc.intCompanyId, u.vchEmail from lconsultantCompany cc left join dUser u on cc.intConsultantId = u.intUserId)

, CompOwner as (
       select intCompanyId,
		STUFF( (SELECT ',' + vchEmail from  tempConsultant WHERE intCompanyId =tc.intCompanyId order by intCompanyId asc FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'),1,1, '')  AS ownerEmails
...
, note as (
       select
         C.intCompanyId as 'intCompanyId'
       , Stuff( 
       ...
       + Coalesce('Company Invoice Contact: ' + NULLIF(cast(C.intInvoiceContactId as varchar(max)), '') + char(10), '')
...
---Main Script---
--insert into importCompany--Have to empty the table before re-insert
select --top 100
       ...
       , iif(dup.rn > 1,concat(dup.vchCompanyName,' ',dup.rn), dup.vchCompanyName) as 'company-name'
...
from dCompany C --1145
			...
			left join note on note.intCompanyId = C.intCompanyId
