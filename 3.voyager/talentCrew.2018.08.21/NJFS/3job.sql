...
, case when j.tintJobStatusId in (5, 6, 101, 102) then dateadd(day,-1, cast(getdate() as date)) else dateadd(day,+10000, cast(getdate() as date)) end as 'position-endDate' --, j.datEndDate as 'position-endDate' --, iif(j.datEndDate is not null, j.datEndDate,iif(j.tintJobStatusId in (105,101,107,108,6,5,109,110,111,102,106), DATEADD(m, 1, j.datStartDate), null)) as 'position-endDate'
...
, left(
	concat('ID: ',j.intJobId,char(10)
	, iif(jl.jobLocation = '' or jl.jobLocation is NULL,'',concat('Job Location: ',jl.jobLocation,char(10),char(10)))
	--, iif(j.datStartDate = '','',concat('Job Start Date: ',char(10),j.datStartDate)) -- Removed for Production
	--, iif(j.bitASAP = '','',concat('ASAP Flag: ',char(10),j.bitASAP)) -- Removed for Production
	, iif( j.intCandidateReportsToId = '' or j.intCandidateReportsToId  is null,'',concat('Reports To: ',jo2.jobOwner,char(10))) -- Removed for Production
...
, j.tintJobStatusId as 'Custom Field > Job Status' -->>
--, j.decBasicMinSalary as 'postion-salaryFrom' -->> -- Removed for Production
--, j.decBasicMaxSalary as 'position-salaryTo' -->> -- Removed for Production
-- select count(*) --1502 -- select distinct intCandidateReportsToId --datStartDate --tintJobStatusId, decBasicMinSalary, decBasicMaxSalary -- select distinct j.tintJobStatusId as 'Custom Field > Job Status' -->> dbo.refJobStatus
