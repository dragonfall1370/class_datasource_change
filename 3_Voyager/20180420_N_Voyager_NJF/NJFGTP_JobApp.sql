...
--select * from refInterviewLevel
--select * from refInterviewStatus
...
select concat('NJFGTP',intJobId) as 'application-positionExternalId',
		concat('NJFGTP',intCandidateId) as 'application-candidateExternalId',
		--*,
		...
		iif(tintCVSubmit = 1 or tintSummarySubmit = 1, 'SENT','SHORTLISTED'))))))) as 'application-Stage'
from  jobApp --where intInterviewId <> 14264 and intInterviewId <> 13470 and CompID in (2,455)--and intInterviewId = 7583
...
--select * from dJob where intJobId = 4221
