...
select
  concat('NJFC',C.intCompanyId) as 'company-externalId'
, C.vchCompanyName as '(OriginalName)'
, iif(C.intCompanyId in (select intCompanyId from dup where dup.rn > 1)
	, iif(dup.vchCompanyName = '' or dup.vchCompanyName is NULL,concat('NJF Contract - Default Company - ID',dup.intCompanyId),concat('NJFC-',dup.rn,' - ',dup.vchCompanyName))
	, iif(C.vchCompanyName = '' or C.vchCompanyName is null,concat('NJF Contract - Default Company - ID',C.intCompanyId),C.vchCompanyName)) as 'company-name'
...
, left(Concat(
			'Company External ID: NJFC', C.intCompanyId,char(10),
...
UNION ALL
select 'NJFC9999999','','NJF Contract - Default Company','','','','','','','This is Default Company from Data Import'
