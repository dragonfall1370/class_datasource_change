select concat('NJFS',intCompanyTierContactId) as ContactExternalId, -10 as userId
		...
		, concat('-----MIGRATED FROM EMAILS-----',
				...
				iif(actc.dtInserted = '' or actc.dtInserted is null, '', concat(char(10),'Inserted Date: ',convert(varchar(20),actc.dtInserted,120))),
				concat(char(10),'Email ID: ', actc.intAttachmentId)
				) as commentContent
