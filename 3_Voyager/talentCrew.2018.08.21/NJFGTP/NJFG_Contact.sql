...
, tempContactEmail1 as (SELECT intContactId, 
     STUFF(
         (SELECT ',' + replace(replace(replace(vchValue,'&',','),'/',','),' ','')--replace & and / with , on NJF GTP, replace space with , on NJF Contracts
...
, temptempcontactEmail4_1 as (select *
	,replace(replace(replace(replace(iif(CHARINDEX('[',vchValue)>0,right(vchValue,len(vchValue)-CHARINDEX('[',vchValue)),vchValue),']',''),'mailto:',''),'<',''),'>','') as vchValue1 
...
----RECOGNITE CONTACTS HAVE THE SAME EMAIL ADDRESSES
--, tempContactEmail5 as (select tc.intCompanyTierContactId, tce.intContactId, tce.vchValue as email, ROW_NUMBER() OVER(PARTITION BY tce.vchValue ORDER BY tce.intContactId ASC) AS rn
--from tempcontactEmail4 tce left join tempContacts tc on tce.intContactId = tc.intContactId)

, ContactEmail as (select intCompanyTierContactId, intContactId,
case 
when rn=1 then email
else concat('NJFGTP_',rn,'_',(email))
end as Email
...
select 
iif(tc.intCompanyId = '' or tc.intCompanyId is NULL,'NJFGTP9999999',concat('NJFGTP',tc.intCompanyId)) as 'contact-companyId'
, tc.intCompanyId as '(OriginalCompanyID)'
, com.vchCompanyName as '(OriginalCompanyName)'
, concat('NJFGTP',tc.intCompanyTierContactId) as 'contact-externalId'
...
, left(
	concat('Contact External ID: NJFGTP',tc.intCompanyTierContactId,char(10)
...
UNION ALL
select 'NJFGTP9999999','','','NJFGTP9999999','NJF GTP - Default','Contact','','','','','','','','','This is default contact from Data Import'

--select * from importContact where len(contactdocument)> 32000