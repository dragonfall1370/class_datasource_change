...
select
        ...
        --, case when c.SiteUnique <> '0' then c.SiteUnique else 'defaultcompany' end as 'contact-companyId' 
        , cl.AccountName as 'company-name#'
...
-- select count(*) --12336 -- select top 10 * from Sites
from contacts c
left join Sites s on s.SiteUniqueID = c.MainSiteUnique
left join Clients cl on cl.MainSiteUnique = s.SiteUniqueID
...
where cl.UniqueID = '2002'
where cl.AccountName like '%MINDTERRA%'
where cl.MainSiteUnique = c.SiteUnique and  cl.AccountName is not null
--and c.ContactUniqueID in ('5319','8176')
...
left join Sites s on s.SiteUniqueID = c.MainSiteUnique
left join Clients cl on cl.MainSiteUnique = s.SiteUniqueID
--left join Clients cl on cl.AccountCode = s.AccountCode
--where s.SiteUniqueID = '1857'
-- select count(*) from Clients cl where cl.UniqueID = '0'
where cl.AccountName like '%MINDTERRA%'
where cl.MainSiteUnique = c.SiteUnique
where cl.MainSiteUnique <> cl.UniqueID --98
where cl.MainSiteUnique = cl.UniqueID --1541
...
where cl.UniqueID is not null and cl.UniqueID <> cl.MainSiteUnique
