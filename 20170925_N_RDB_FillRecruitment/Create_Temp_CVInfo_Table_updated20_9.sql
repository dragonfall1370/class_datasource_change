...
, TempCV1 as(select ApplicantId, 
	...
	, iif(UpdatedOn = '' or UpdatedOn is NULL,'',concat(char(10),'Updated on: ',UpdatedOn,char(10)))))) as CVInfo
	, rn
	from TempCV)
...
insert into Temp_CVInfo SELECT ApplicantId, 
     STUFF(
         (SELECT char(10) + CVInfo
          from  TempCV1
          WHERE ApplicantId = tcv.ApplicantId
