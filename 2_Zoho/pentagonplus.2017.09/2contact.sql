...
select
        ...
        Stuff(Coalesce(' ' + NULLIF(c.Mobile, ''), '') + Coalesce(', ' + NULLIF(c.WorkPhone, ''), ''), 1, 1, '') as 'contact-phone',
...
--left join (SELECT b.parentid, STUFF((SELECT DISTINCT ',' + replace(replace(a.FileName,',',''),'''','') from attachments a left join clients c on a.parentid = c.ClientId WHERE a.parentid = b.parentid FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '')  AS docs FROM attachments AS b GROUP BY b.parentid) doc on doc.parentid = c.ContactId
left join (SELECT b.parentid, STUFF((SELECT DISTINCT ',' + replace(replace(a.FileName,',',''),'''','') from attachments a left join contacts c on a.parentid = c.contactId WHERE a.parentid = b.parentid FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '')  AS docs FROM attachments AS b GROUP BY b.parentid) doc on doc.parentid = c.ContactId
where
...
--select count(*) --7177
from notes t
left join contacts c on c.ContactId = t.parentid
...
--select count(*) --2
from calls t
left join contacts c on c.ContactId = t.RelatedTo
left join (select userid, email, firstname, lastname from users) u1 on u1.userid = t.CreatedBy
left join (select userid, email, firstname, lastname from users) u2 on u2.userid = t.ModifiedBy
where c.ContactId is not null
