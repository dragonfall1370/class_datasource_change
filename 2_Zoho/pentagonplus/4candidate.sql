...
*/
        , Stuff(  Coalesce('Supply Chain & Logistics Skills: ' + NULLIF(c.SupplyChainLogisticsSkills_ifapplicable, '') + char(10), '')
                ...
                + Coalesce('Industry: ' + NULLIF(c.Industrymaxselect2, '') + char(10), '')
...
*/
        , Stuff(
                ...
                + Coalesce('Total Work exp (month): ' + NULLIF(c.Totalworkexpmonth, '') + char(10), '')
        ...
        , Stuff(  Coalesce(NULLIF(c.CurrentJobTitle, ''), '')
                + Coalesce(', ' + NULLIF(c.CurrentJobTitle_, ''), '')
                , 1, 0, '') as 'candidate-jobTitle1'
...
left join (SELECT b.parentid, STUFF((SELECT DISTINCT ',' + replace(replace(a.FileName,',',''),'''','') from attachments a left join client c on a.parentid = c.ClientId WHERE a.parentid = b.parentid FOR XML PATH (''), TYPE).value('.', 'nvarchar(MAX)'), 1, 1, '')  AS docs FROM attachments AS b GROUP BY b.parentid) doc on doc.parentid = c.candidateID
