...
if __name__ == '__main__':
    ...
    for dirpath, dirnames, filenames in os.walk(locator):
        csvfs.extend([os.path.join(dirpath, name) for name in filenames if name.startswith('edenscott_cand_location_')])
    ...
    vincere_custom_migration.psycopg2_bulk_update(df, ddbconn, ['latitude', 'longitude'], ['id',], 'common_location')

    df = pd.read_sql("select id, address from common_location where longitude is null and address is not null;", ddbconn)
    dfs = vincere_common.df_split_to_listofdfs(df, 25000)
    for idx, d in enumerate(dfs):
        d.to_csv('geo_____{}.csv'.format(idx), index=False)
